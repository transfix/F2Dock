...
void ScriptHandler::setupStartScript(int id) {
  ...
  if (!stOutFP) {
    ...
    stOutFP << "print \"\n\nStarting script\n\n\";" << std::endl;
    stOutFP << "$dummyFile = qq(" << DataPath << "/" << id << "_start.txt);"
            << std::endl;
...
void ScriptHandler::setupFinishScript(std::string id) {
  ...
  if (!stOutFP) {
    ...
    stOutFP << "#!/lusr/bin/perl" << std::endl;
    stOutFP << "print \"\n\nStarting script\n\n\";" << std::endl;
    stOutFP << "$dummyFile = qq(" << DataPath << "/" << id << "_done.txt);"
            << std::endl;
    stOutFP << "open OUT, \"> $dummyFile\" or die \"\nError: Cannot create "
               "dummy file!\n\n\"; "
            << std::endl;
...
void ScriptHandler::initScript(std::ofstream *script) {
  if (dockJob->platform == PRISM2) {
    ...
    (*script)
        << "export "
           "LD_LIBRARY_PATH=/home/docking/F2DockServer:$LD_LIBRARY_PATH\n";
  }
...
void ScriptHandler::preparePQRGenScript(std::ofstream *script, bool receptor) {
  ...
  std::cout << "python " << PDB2PQRPath << " --ff=amber "
            << dockJob->receptorPDBName << " " << dockJob->receptorPQRName
            << "\n";

  if (receptor)
    (*script) << "python " << PDB2PQRPath << " --ff=amber "
              << dockJob->receptorPDBName << " " << dockJob->receptorPQRName
              << "\n";
  else
    (*script) << "python " << PDB2PQRPath << " --ff=amber "
              << dockJob->ligandPDBName << " " << dockJob->ligandPQRName
              << "\n";
...
void ScriptHandler::prepareF2dGenScript(std::ofstream *script) {
  ...
  //(*script) << "../Python/python ./f2dGenerator.py "<< dockJob->ligandPQRName
  //<< " " << dockJob->receptorPQRName << " " << dockJob->f2dDim <<"\n";
  (*script) << MolSurfPath << " -surfaceAtoms " << dockJob->ligandPQRName << " "
            << dockJob->ligandXYZName << "\n";
  (*script) << MolSurfPath << " -generateF2d " << dockJob->ligandPQRName << " "
            << dockJob->ligandXYZName << " " << dockJob->ligandF2dName << " 0 0"
            << "\n";
  (*script) << MolSurfPath << " -populateSASUsingMesh "
            << dockJob->receptorPQRName << " " << dockJob->receptorRAWNName
            << " " << dockJob->receptorXYZName << " 1.1 0.6 1.1 0" << "\n";
  (*script) << MolSurfPath << " -generateF2d " << dockJob->receptorPQRName
            << " " << dockJob->receptorXYZName << " "
            << dockJob->receptorF2dName << " 1 0" << "\n";
...
void ScriptHandler::prepareRAWNGenScript(std::ofstream *script, bool receptor) {
  ...
  if (receptor) {
    (*script) << MolSurfPath << " -surfaceUsingAdaptiveGrid "
              << dockJob->receptorPQRName << " " << dockJob->receptorRAWName
              << " " << dockJob->quadDim << "\n";
    (*script) << MolSurfPath << " -qualityImprove " << dockJob->receptorRAWName
              << " " << dockJob->receptorIRAWName << "\n";
    (*script) << MolSurfPath << " -normals -average "
              << dockJob->receptorIRAWName << " " << dockJob->receptorRAWNName
              << "\n";
  } else {
    (*script) << MolSurfPath << " -surfaceUsingAdaptiveGrid "
              << dockJob->ligandPQRName << " " << dockJob->ligandRAWName << " "
              << dockJob->quadDim << "\n";
    (*script) << MolSurfPath << " -qualityImprove " << dockJob->ligandRAWName
              << " " << dockJob->ligandIRAWName << "\n";
    (*script) << MolSurfPath << " -normals -average " << dockJob->ligandIRAWName
              << " " << dockJob->ligandRAWNName << "\n";
  }
...
void ScriptHandler::prepareQuadGenScript(std::ofstream *script, bool receptor) {
  ...
  if (receptor)
    (*script) << MolSurfPath << " -aSpline -quad " << dockJob->receptorRAWNName
              << " gaussian 1 " << dockJob->receptorQuadName << "\n";
  else
    (*script) << MolSurfPath << " -aSpline -quad " << dockJob->ligandRAWNName
              << " gaussian 1 " << dockJob->ligandQuadName << "\n";
