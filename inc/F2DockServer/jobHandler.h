...
JobHandler::JobHandler(DockingJobParams *dj) {
  ...
  dockJob->rerankingInputFileName =
      sh->getDataPath() + "/" + pid + "_rerank.inp";
  dockJob->rerankingOutputFileName =
      sh->getDataPath() + "/" + pid + "_rerank.out";

  dockJob->f2dGenInputFileName = sh->getDataPath() + "/" + pid + "_f2dgen.inp";
  dockJob->quadGenInputFileName =
      sh->getDataPath() + "/" + pid + "_quadgen.inp";

  if (dockJob->platform == PRISM2) {
    dockJob->scriptFileName =
        "/home/docking/F2DockServer/data/Jobs/job" + pid + ".scr";
  } else {
...
void JobHandler::saveJob() {
  ...
  std::string jobDescriptionFileName =
      sh->getDataPath() + "/" + "job" + pid + ".txt";
...
std::string JobHandler::getFileName(int index) {
  if (index == RECEPTOR_PQR)
    return dockJob->receptorPQRName;
  else if (index == LIGAND_PQR)
    return dockJob->ligandPQRName;

  else if (index == RECEPTOR_F2D)
    return dockJob->receptorF2dName;
  else if (index == LIGAND_F2D)
    return dockJob->ligandF2dName;

  else if (index == RECEPTOR_RAWN)
    return dockJob->receptorRAWNName;
  else if (index == LIGAND_RAWN)
    return dockJob->ligandRAWNName;

  else if (index == RECEPTOR_QUAD)
    return dockJob->receptorQuadName;
  else if (index == LIGAND_QUAD)
    return dockJob->ligandQuadName;

  else if (index == DOCKING_OUT)
    return dockJob->dockingOutputFileName;
  else if (index == RERANKING_OUT)
    return dockJob->rerankingOutputFileName;
...
bool JobHandler::submitJob() {
  ...
  std::cout << "Script file name: " << dockJob->scriptFileName.c_str()
            << std::endl;

  std::ofstream jobscript(
      dockJob->scriptFileName.c_str()); // initializing the job script
  sh->initScript(&jobscript);

  std::cout << " script initialized " << std::endl;

  if (type == DOCKING) {
    if (!submitDockingJob(&jobscript))
      return false;
  } else if (type == RERANKING) {
    if (!submitRerankingJob(&jobscript))
      return false;
  } else if (type == F2DGEN) {
    if (!submitF2dGenJob(&jobscript))
      return false;
  } else if (type == QUADGEN) {
    if (!submitQuadGenJob(&jobscript, true))
      return false;
    if (!submitQuadGenJob(&jobscript, false))
      return false;
  }
...
bool JobHandler::submitDockingJob(
    std::ofstream *script) // Check if sufficient data is available. Prepare the
                           // script for docking job. Make sure to include the
                           // generation of intermediate files
{
  ...
  if (!(rpdb || rpqr && (rraw || rqua)))
    return false;
  if (!(lpdb || lpqr && (lraw || lqua)))
    return false;
...
bool JobHandler::submitRerankingJob(std::ofstream *script) {
  ...
  if (!dockJob->isDockingOutputAvailable)
    return false;
...
bool JobHandler::submitF2dGenJob(std::ofstream *script) {
  if (!dockJob->isReceptorPDBAvailable || !dockJob->isLigandPDBAvailable)
    return false;
...
void JobHandler::processDockingInput() {
  std::ofstream input(dockJob->dockingInputFileName.c_str(),
                      std::ios_base::app);
...
void JobHandler::processRerankingInput() {
  std::ofstream input(dockJob->rerankingInputFileName.c_str(),
                      std::ios_base::app);

  std::string inputstring =
      "staticMoleculePQR " + dockJob->receptorPQRName + "\n";
  inputstring += "movingMoleculePQR " + dockJob->ligandPQRName + "\n";
  inputstring += "staticMoleculeQUAD " + dockJob->receptorQuadName + "\n";
  inputstring += "movingMoleculeQUAD " + dockJob->ligandQuadName + "\n";
  inputstring += "F2DockOutputFile " + dockJob->dockingOutputFileName + "\n";
  inputstring +=
      "rerankedOutputFile " + dockJob->rerankingOutputFileName + "\n";
