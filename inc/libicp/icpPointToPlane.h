...
#ifndef LIBICP_ICP_POINT_TO_PLANE_H
#define LIBICP_ICP_POINT_TO_PLANE_H
...
public:
  IcpPointToPlane(double *M, const int32_t M_num, const int32_t dim,
                  const int32_t num_neighbors = 10, const double flatness = 5.0)
      : Icp(M, M_num, dim) {
    M_normal = computeNormals(num_neighbors, flatness);
  }

  virtual ~IcpPointToPlane() { free(M_normal); }
...
private:
  ...
  void computeNormal(const kdtree::KDTreeResultVector &neighbors,
...
#endif // LIBICP_ICP_POINT_TO_PLANE_H
