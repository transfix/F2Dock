...
private:
  ...
  inline void transformPoint(double x, double y, double z, double *transMat,
                             double *nx, double *ny, double *nz);
  inline void transformPoint(Point p, double *transMat, Point *np);
  double computeXlateForPG(int numStQPoints, QPOINT *stQPoints,
  ...
  bool copyAtomsFromArray(int numAtoms, double *atms, int *nAtoms,
                          ATOM **atoms);
  bool readAtoms(char *atomsFile, int *nAtoms, ATOM **atoms);
  void initFreeNodeServer(int numNodes);
  int nextFreeNode(void);
  void countAtomsOctreeNodesAndSortAtoms(ATOM *atoms, int atomsStartID,
  ...
  bool buildAtomsOctree(int nAtoms, ATOM *atoms, int *numAtomsOctreeNodes,
                        ATOMS_OCTREE_NODE **atomsOctree, int *atomsOctreeRoot);
  void countQPointsOctreeNodesAndSortQPoints(QPOINT *qPts, int qPtsStartID,
                                             int qPtsEndID, QPOINT *qPtsT,
                                             int *numNodes);
  int constructQPointsOctree(int qPtsStartID, int qPtsEndID, QPOINT *qPts,
                             QPOINTS_OCTREE_NODE *qPointsOctree);
  bool buildQPointsOctree(int nQPoints, QPOINT *qPoints,
                          int *numQPointsOctreeNodes,
                          QPOINTS_OCTREE_NODE **qPointsOctree,
                          int *qPointsOctreeRoot);
  void assignHydrophobicityToQPoints(ATOMS_OCTREE_NODE *atomsOctree, int nodeA,
                                     ATOM *atoms,
                                     QPOINTS_OCTREE_NODE *qPointsOctree,
                                     int nodeQ, QPOINT *qPoints, double farDist,
                                     double rangeExt);
  void initOctreeFlags(int threadID);
  void collectPseudoGsol(int threadID, double *trans, double *transI,
                         double *pGsol, double *pGsolHStaticPos,
                         double *pGsolHStaticNeg, double *pGsolHMovingPos,
                         double *pGsolHMovingNeg);
  void markPotentialQPoints(int threadID, int nodeS, int nodeM, double *trans);

public:
  pseudoGsol(char *paramFile, int nStAtoms, double *stAtoms, int nMvAtoms,
             double *mvAtoms, int nThreads);
  ...
  void getPseudoGsol(int threadID, double *pGsol, double *pGsolHStaticPos,
                     double *pGsolHStaticNeg, double *pGsolHMovingPos,
                     double *pGsolHMovingNeg);
  void getPseudoGsol(int threadID, double *trans, double *pGsol,
                     double *pGsolH);
