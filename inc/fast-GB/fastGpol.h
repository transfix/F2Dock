...
private:
  ...
  void generateJobs(OCTREE_NODE *octree, int nodeI, int nodeJ, double eps,
                    std::vector<JOB> &leafJobs, std::vector<JOB> &nonLeafJobs);

  inline float invSqrt(float x);
  inline float fastExp(float x);

#ifdef USE_SSE
  inline float vectorGB(v4sf xi, v4sf yi, v4sf zi, v4sf Ri, v4sf qi, v4sf xj,
                        v4sf yj, v4sf zj, v4sf Rj, v4sf qj);
  ...
  void computeThreadedFastGpol(int threadID, OCTREE_NODE *octree, double *qSum,
                               double *approxR, int numGroupR, ATOM *atom,
                               double eps, double *Gpol);

  static void *startThreadedFastGpol(void *v) {
    fastGB::fastGpol::PARAMS *pr = (fastGB::fastGpol::PARAMS *)v;

    pr->thisPtr->computeThreadedFastGpol(pr->threadID, pr->octree, pr->qSum,
                                         pr->approxR, pr->numGroupR, pr->atom,
                                         pr->eps, &(pr->Gpol));
    return nullptr;
  }
