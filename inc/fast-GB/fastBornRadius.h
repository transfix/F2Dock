...
#ifdef BRR4
#define COMBINE_INTEGRALS(a, b)                                                \
  ((1 - INV_SQRT_TWO) * (a) / (4 * M_PI) + pow((b) / (16 * M_PI), 0.5))
#else
//   #define COMBINE_INTEGRALS( a, b ) ( pow( ( 3 * ( a ) ) / ( 20 * M_PI ), 1.0
//   / 3.0 ) )
...
private:
  ...
  void pushIntegralsAndFillAtomsSubtreeRootsArray(int nodeID, double intVal,
                                                  double intCVal,
                                                  int maxNodesInLevel);
...
public:
  ...
  fastBornRadius(int numQPoints, double *qPts, int numAtoms, double *atms,
                 bool printStat);
  ~fastBornRadius();
