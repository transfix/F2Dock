...
struct ball {
  ...
  ball(Point p, float r) {
    ...
    radius = r;
    semi_side = r / 1.73205;
    //   exposed[0] = exposed[1] = exposed[2] = exposed[3] = exposed[4] =
    //   exposed[5] = true;
  }
  bool equals(ball *b) { return (radius == b->radius && center == b->center); }
  /*  bool isBuried(void) {
      return !(exposed[0] || exposed[1] || exposed[2] || exposed[3] ||
    exposed[4] || exposed[5]);
