...
template <typename T> struct RangeTuple {
  int id = -1;
  T ptr{};
...
template <class T> class IntegerRange {
  ...
  void printReps() {
    ...
    for (int i = 0; i < num_clusters; i++)
      std::cout << "Representative for tree # " << i + 1 << " is " << repMap[i]
                << std::endl;
  ...
  */
  ...
  void splitBST(int index) {
    ...
    if (s > SPLIT_THRESH) {
      RangeTuple<T> notfound(-1, NULL);
      BinarySearchTree<RangeTuple<T>> *t =
          new BinarySearchTree<RangeTuple<T>>(notfound);
      BST[index]->split(t);
      ...
      typename std::vector<BinarySearchTree<RangeTuple<T>> *>::iterator q =
          BST.begin();
      q = q + index + 1;
      BST.insert(q, t);
  ...
  */
  ...
  int mergeBST(int index) {
    ...
    if (s <= MERGE_THRESH) {
      ...
      typename std::vector<BinarySearchTree<RangeTuple<T>> *>::iterator p =
          BST.begin();
      p = p + merge_with;
      BST.erase(p);
