...
extern sparse3DFFT_time sparse3DFFT_gettimeofday_time_diff(sparse3DFFT_time t1,
                                                           sparse3DFFT_time t2);
...
#define sparse3DFFT_time_to_sec(t)                                             \
  ((double)(t).tv_sec + (double)(t).tv_usec * 1.0E-6)
...
#define TIME_FFT(fft, t, tmin)                                                 \
  {                                                                            \
    ...
    for (tfft_repeat = 0; tfft_repeat < SPARSE3DFFT_TIME_REPEAT;               \
         ++tfft_repeat) {                                                      \
      do {                                                                     \
        ts = sparse3DFFT_get_time();                                           \
        for (tfft_iter = 0; tfft_iter < tfft_iters; ++tfft_iter) {             \
          fft;                                                                 \
        }                                                                      \
        te = sparse3DFFT_get_time();                                           \
        tcur = (total_t =                                                      \
                    sparse3DFFT_time_to_sec(sparse3DFFT_time_diff(te, ts))) /  \
               tfft_iters;                                                     \
        tfft_iters *= 2;                                                       \
      } while (total_t < tmin);                                                \
      ...
      if (tcur < tbest)                                                        \
        tbest = tcur;                                                          \
      if (real_total_t > SPARSE3DFFT_TIME_LIMIT)                               \
        break;                                                                 \
    }                                                                          \
    t = tbest;                                                                 \
  }
