...
void read_par(char *prmfile, int nat, char *atnam, int nb, int *bonds,
              float **kbond, float **lbond, int nang, int *angles,
...
/**
        return *pptext, a pointer to the n-th word (starting from 0) in a string
   ptext with a delimiter ch
*/
...
void read_ff_charmm(const char *psffile, char *prmfile, char *rtffile,
                    struct atomgrp *ag);
/**
       update fixed atom index and bonded active structures. list contains
