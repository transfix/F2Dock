...
namespace LibMol {
typedef struct {
  double lx, ly, lz; /* the corner of the cell cube with the minimum x, y, z
                        co-ordinates */
  double dim;        /* dimension of the cube */

#ifdef ADD_ATTR
  double sx, sy,
      sz; /* sums of the x, y and z co-ordinates of the atoms under this node */
  ...
  int p_ptr;    /* index of (ptr to) the parent node in the octree (-1 for the
                   parent of the root) */
  int c_ptr[8]; /* indices of (ptrs to) child nodes */

  int leaf;     /* 1 if this node is a leaf (i.e., no further subdivision), 0
                   otherwise */
  int id_cap;   /* capacity of the indices array */
  int id_num;   /* number of items in the indices array (used for temporary
                   storage in non-leafs) */
  int *indices; /* indices to the items/atoms in this node (fixed items are
                   listed first) */
...
typedef struct {
  ...
  int free_node_ptr; /* index of the first free node in the nodes array, -1 if
                        array if full */
} OCTREE;
...
inline void transform_point(double x, double y, double z, double *trans_mat,
...
inline double min_pt2bx_dist2(double bx, double by, double bz, double dim,
...
inline int within_distance_cutoff(double x1, double y1, double z1, double dim1,
                                  double x2, double y2, double z2, double dim2,
                                  double ext) {
  double d1 = dim1 + ext, d2 = -(dim2 + ext);

  if ((x2 - x1 >= d1) || (x2 - x1 <= d2) || (y2 - y1 >= d1) ||
      (y2 - y1 <= d2) || (z2 - z1 >= d1) || (z2 - z1 <= d2))
    return 0;

  d1 = 0.5 * (dim2 - dim1);

  d2 = (x2 - x1 + d1) * (x2 - x1 + d1) + (y2 - y1 + d1) * (y2 - y1 + d1) +
       (z2 - z1 + d1) * (z2 - z1 + d1);
