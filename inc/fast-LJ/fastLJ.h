...
private:
  ...
  typedef struct {
    ...
    bool leaf;
    int cPtr[8];
    int numAtoms[numAtomType], atomsStartID[numAtomType],
        atomsEndID[numAtomType];
  } ATOMS_OCTREE_NODE;
  ...
  bool getParamsFromFile(char *paramFile, char **staticAtomsFile,
  ...
  bool copyAtomsFromArray(int numAtomsSrc, double *atmsSrcXYZ,
...
#ifdef USE_SSE
  inline float vectorLJ(v4sf xi, v4sf yi, v4sf zi, v4sf xj, v4sf yj, v4sf zj,
                        v4sf Aij, v4sf Bij);
#endif

  void approximatePotential(int threadID, double *transMat, int nodeS,
                            int nodeM, double *LJPot);

public:
  fastLJ(int numStaticAtoms, double *stAtomsXYZ, char *stAtomsType,
         int numMovingAtoms, double *mvAtomsXYZ, char *mvAtomsType,
         int numThreads, bool printStat);
  fastLJ(char *staticAtomsFile, char *movingAtomsFile, int numThreads,
         bool printStat);
