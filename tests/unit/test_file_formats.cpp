...
// ────────────────────────────────────────────────────── PQR format per
// documentation: standard PDB ATOM records with charge and radius replacing the
// occupancy and B-factor columns. Format: ATOM  serial name resName chainID
// resSeq x y z charge radius
...
protected:
  ...
  bool parsePQRLine(const std::string &line, PQRAtom &atom) {
    ...
    int n = sscanf(line.c_str(), "%6s %d %4s %3s %c %d %lf %lf %lf %lf %lf",
                   record, &atom.serial, atom.name, atom.resName, &atom.chainID,
                   &atom.resSeq, &atom.x, &atom.y, &atom.z, &atom.charge,
                   &atom.radius);
    return (n >= 11);
  }
};
...
// ────────────────────────────────────────────────────── F2D format per
// documentation: like PQR but with I/E type column at the end. I = interior
// (core), E = exterior (skin) May also contain HETATM records for "grown" skin
// atoms.
...
TEST_F(F2DFormatTest, F2DAtomHasExpectedColumns) {
  ...
  while (std::getline(f, line) && checked < 5) {
    ...
    int n = sscanf(line.c_str(), "%6s %d %4s %3s %d %lf %lf %lf %lf %lf %1s",
                   record, &serial, name, resName, &resSeq, &x, &y, &z, &charge,
                   &radius, type);
...
// ───────────────────────────────────────────────────── QUAD format per
// documentation: 7 floats per line:
//   x y z nx ny nz weight
// where (x,y,z) = quadrature point position, (nx,ny,nz) = surface normal,
...
TEST_F(QUADFormatTest, QUADFileHasSevenColumnsPerLine) {
  ...
  while (std::getline(f, line) && lineCount < 10) {
    ...
    int n = sscanf(line.c_str(), "%lf %lf %lf %lf %lf %lf %lf", &x, &y, &z, &nx,
                   &ny, &nz, &w);
...
TEST_F(QUADFormatTest, QUADNormalsAreUnitLength) {
  ...
  while (std::getline(f, line) && checked < 20) {
    double x, y, z, nx, ny, nz, w;
    if (sscanf(line.c_str(), "%lf %lf %lf %lf %lf %lf %lf", &x, &y, &z, &nx,
               &ny, &nz, &w) != 7)
      continue;
...
TEST_F(QUADFormatTest, QUADWeightsArePositive) {
  ...
  while (std::getline(f, line) && checked < 50) {
    ...
    EXPECT_GT(w, 0.0) << "QUAD weight should be positive, got " << w;
    checked++;
  }
}

// ── Parameter file (.inp) format validation
// ──────────────────────────────────── The parameter file is a key-value text
// file with space-separated pairs. Lines starting with '#' are comments. Lines
// with <3 chars are skipped.
...
TEST_F(ParamFileTest, INPFileHasMandatoryParams) {
  ...
  while (std::getline(f, line)) {
    ...
    if (key == "staticMolecule")
      hasStaticMol = true;
    else if (key == "movingMolecule")
      hasMovingMol = true;
    else if (key == "staticMoleculePQR")
      hasStaticPQR = true;
    else if (key == "movingMoleculePQR")
      hasMovingPQR = true;
    else if (key == "staticMoleculeQUAD")
      hasStaticQUAD = true;
    else if (key == "movingMoleculeQUAD")
      hasMovingQUAD = true;
    else if (key == "outFile")
      hasOutFile = true;
  }
...
// ──────────────────────────────────── These tests verify the documented
// defaults from params.pdf and dockingTutorial.pdf. Since PARAMS_IN
// initialization is embedded in main(), we verify the constants themselves as
// documented.
...
TEST_F(ParamFileTest, INPReferencedFilesExist) {
  ...
  // We verify that the corresponding files exist locally in our tests/
  // directory.
  std::vector<std::string> expectedFiles = {
      "1A2K_R_U_1.7.f2d", "1A2K_L_U.f2d",  "1A2K_R_U.pqr",
      "1A2K_L_U.pqr",     "1A2K_R_U.quad", "1A2K_L_U.quad"};
...
TEST_F(ParamFileTest, AllTestComplexesHaveRequiredFiles) {
  ...
  for (const auto &pdb : complexes) {
    ...
    EXPECT_TRUE(std::ifstream(rpqr).is_open())
        << "Missing: " << pdb << "_R_U.pqr";
    EXPECT_TRUE(std::ifstream(lpqr).is_open())
        << "Missing: " << pdb << "_L_U.pqr";
    EXPECT_TRUE(std::ifstream(rquad).is_open())
        << "Missing: " << pdb << "_R_U.quad";
    EXPECT_TRUE(std::ifstream(lquad).is_open())
        << "Missing: " << pdb << "_L_U.quad";
  }
}
