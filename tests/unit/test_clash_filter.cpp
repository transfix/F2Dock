 ...
 * atoms using an octree-based approximation scheme (per Appendix B §Clash
 * Filter).
 *
...
makeAtomArray(const std::vector<std::array<double, 5>> &atoms) {
...
TEST(ClashFilter, ConstructWithAtoms) {
  ...
  auto staticAtoms = makeAtomArray({{0.0, 0.0, 0.0, 0.0, 1.5}});
  auto movingAtoms = makeAtomArray({{100.0, 100.0, 100.0, 0.0, 1.5}});
...
TEST(ClashFilter, NoClashesWhenFarApart) {
  auto staticAtoms = makeAtomArray({{0.0, 0.0, 0.0, 0.0, 1.8},
                                    {3.0, 0.0, 0.0, 0.0, 1.8},
                                    {6.0, 0.0, 0.0, 0.0, 1.8}});
  auto movingAtoms =
      makeAtomArray({{0.0, 50.0, 0.0, 0.0, 1.8}, {3.0, 50.0, 0.0, 0.0, 1.8}});
...
TEST(ClashFilter, DetectsOverlapNaively) {
  ...
  auto staticAtoms = makeAtomArray({{0.0, 0.0, 0.0, 0.0, 1.8}});
  auto movingAtoms = makeAtomArray({{0.0, 0.0, 0.0, 0.0, 1.8}});
  ...
  Matrix identity;
  identity.reset();
  bool ok = cf.computeInteractionsNaively(identity, &nClashes, &nSevere,
                                          &interaction);
  EXPECT_TRUE(ok);
...
TEST(ClashFilter, ApproximateMatchesNaive) {
  ...
  cf.computeInteractionsNaively(identity, &nClashNaive, &nSevereNaive,
                                &interNaive);
