...
/*
 * Unit tests for CCVOpenGLMath::Matrix, CCVOpenGLMath::Vector,
 * CCVOpenGLMath::Tuple
 *
...
// ─────────────────── Per the documentation, F2Dock output contains a 3×4
// transformation matrix (mat1..mat12) that transforms the moving molecule. This
// test verifies that a composed rotation+translation can round-trip through
// inverse.

TEST(Matrix, DockingTransformRoundTrip) {
  // Simulate a docking result: rotation then translation
  Matrix rot = Matrix::rotationX(0.3f) * Matrix::rotationY(0.6f) *
               Matrix::rotationZ(0.9f);
