...
static std::vector<Point *> bruteForceRange(std::vector<Point *> &pts, Point *q,
                                            double radius) {
...
protected:
  ...
  void TearDown() override { delete pg; }
};
...
TEST_F(PGRangeMultiTest, RangeQueryMatchesBruteForce) {
  ...
  // (PG may include points slightly beyond the exact radius due to cell
  // granularity)
