...
Concrete tasks:
1. ~~**NFFT acceleration plumbing** — surface an `nfftAccelerate` input-file flag (default off) parsed into `PARAMS_IN`; print a one-line startup diagnostic (`NFFT acceleration: requested=on/off available=yes/no active=...`); reject `nfftAccelerate true` with a clear message when `F2DOCK_HAVE_NFFT` is not defined at compile time. No scoring-path change yet.~~ **Landed in [#31](https://github.com/transfix/F2Dock/pull/31).** `bool nfftAccelerate` field on `PARAMS_IN`, default `false`; parser branch rejects `true` at parse time when the build is missing `F2DOCK_HAVE_NFFT`; stdout-only startup diagnostic preserves all existing `.out` baselines. `src/f2dock/CMakeLists.txt` links the optional `f3dock_science_stack` INTERFACE target.
2. ~~**`f3dock-nfft` fast Gaussian sum wrapper** — new optional static library exposing a Gaussian-sum evaluator backed by NFFT3's fastsum module. Deterministic unit tests compare against the direct O(N·M) sum.~~ **Landed in [#32](https://github.com/transfix/F2Dock/pull/32).** Library `f3dock-nfft` exposes `f3dock::nfft::FastGaussianSum` with two evaluation paths: `evaluate_direct` (always-on O(N·M) reference) and `evaluate` (selects the NFFT3 fastsum fast path when `is_accelerated()` returns true, otherwise delegates to the reference). The actual NFFT3 fastsum hookup is currently a no-op fallback — the library compiles and runs uniformly on every CI platform — and lands in a follow-up patch once libnfft3 is available in CI. 7 gtest cases lock in API and direct-evaluator correctness.
3. **NFFT in the electrostatics gridding path** — *partially unblocked*. `libcvc-deps` v1.1.0 ships `libnfft3` + `libfftw3` + `nfft3.pc`, and F2Dock CI now pulls that bundle on Linux via `.github/actions/fetch-libcvc-deps` (PR to land alongside this note). `pkg_check_modules(NFFT)` activates `F2DOCK_HAVE_NFFT=1` on the Linux runners; the configure step logs `F2Dock: NFFT3 <version> detected via pkg-config; F2DOCK_HAVE_NFFT enabled` so CI logs make this observable. macOS continues to use brew's FFTW/HDF5/Boost without the bundle (the macos-arm64 bundle's vendored Boost shadows brew's and breaks libcvc's `multi_array` consumers; re-enable once that is resolved upstream). The **remaining sub-blocker** is that NFFT3's `fastsum` *applications* module — which `FastGaussianSum::evaluate` was designed to delegate to — is not built/installed into `libnfft3.so` by upstream NFFT3 (it lives under `applications/fastsum/` in NFFT3's source tree). Per the project owner's "use libcvc, don't vendor" principle, the next step is an upstream issue against `transfix/libcvc-deps` requesting that the fastsum applications also be packaged into the bundle. Once that ships: (a) finish the NFFT3 fastsum hookup in `FastGaussianSum::evaluate`, (b) introduce a guarded call site that routes the gridding loop through `FastGaussianSum` when `pr.nfftAccelerate && FastGaussianSum::is_accelerated()`, and (c) add a `1A2K_nfft_smoke` integration baseline (loosened tolerance — the two algorithms are not bit-identical).
4. ~~**libcvc geometry consumer** — audit which concrete F2Dock pipelines still rely on legacy `Blurmaps` / `GOA` / `PRGN` semantics (surface generation, signed-distance fields, mesh I/O) and replace each with a call into libcvc.~~ **Complete.** Audit on master `2dc94f6` found **zero** Blurmaps / GOA / PRGN consumers in F2Dock's `src/` or `inc/` trees (`rg -i 'blurmap|\bGOA\b|PRGN'` returns no matches). The only references to those names are in the legacy upstream `F3Dock/src/f2dock/CMakeLists.txt` (the migration *source*, not the destination). The libcvc-consumption path is therefore the established convention going forward: when future F2Dock work needs surface generation, signed-distance fields, or related geometry primitives, the call goes through `find_package(cvc CONFIG)` and any missing capability is filed upstream against libcvc rather than vendored locally.

Risk: medium. Task 3's CI plumbing is now in place; the algorithmic remainder is gated on NFFT3's fastsum applications being packaged into `libcvc-deps`. Tasks 1, 2, and 4 are landed.
