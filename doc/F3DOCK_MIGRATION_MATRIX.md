...
3. **NFFT in the electrostatics gridding path** — *deferred*. Wiring `FastGaussianSum` into the rasterize-then-FFT path in `src/fft-utils/fastfft.cpp::gridding()` (or the call sites in `Docking.cpp`) is meaningful only when `FastGaussianSum::is_accelerated()` returns true on at least one CI platform, which requires libnfft3 to be present in the runner image. Once that is in place this task will (a) finish the NFFT3 fastsum hookup in `FastGaussianSum::evaluate`, (b) introduce a guarded call site that routes the gridding loop through `FastGaussianSum` when `pr.nfftAccelerate && FastGaussianSum::is_accelerated()`, and (c) add an `1A2K_nfft_smoke` integration baseline (loosened tolerance — the two algorithms are not bit-identical).
...
Risk: medium. Task 3 is the only one remaining and is gated on a CI prerequisite (libnfft3 in the runner image). Tasks 1, 2, and 4 are landed.
