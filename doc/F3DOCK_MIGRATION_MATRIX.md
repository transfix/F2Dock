...
|---|---|---:|---:|---|
| Phase 3 follow-ups: real `.f2d` integration test, B-side gridding hoist | Medium | Low-Medium | Low | **Phase 3 close-out (in progress)**: real-complex flex integration test **landed** on `feature/f3dock-phase3-followups` (`tests/integration/inputs/1A2K_flex_smoke.inp` + baseline, 2-state hinge over `1A2K`, also fixes a latent param-parser bug that dropped tokens past the first); B-side hoist still pending \u2014 factor state-invariant ligand-side work (FFT-Kernel construction, `transformAndNormalize` on B, gridding / `griddingElec` for B, `getCenterFrequencies` on B, ligand-side filter object construction) out of `dockingMain` so the outer multi-state loop only repeats the A-side (receptor, flex-perturbed) work, saving O(N_states - 1) redundant ligand passes |
