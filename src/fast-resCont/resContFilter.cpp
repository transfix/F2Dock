...
void resContFilter::printCurrentSettings(void) {
  ...
  printf("\tminRadius = %lf, maxLeafSize = %d, distCutoff = %lf, numThreads = "
         "%d\n",
         minRadius, maxLeafSize, distCutoff, numThreads);
}

resContFilter::resContFilter(int numStaticAtoms, double *stAtoms,
                             int numMovingAtoms, double *mvAtoms, char *intFile,
                             bool printStat) {
  setDefaults();
  ...
  } else if (!readInteractionMatrix(intFile))
    exit(1);
...
bool resContFilter::readInteractionMatrix(char *intFile) {
  ...
  if (fp == NULL) {
    printError(
        std::format("Failed to open interaction matrix file ({})!", intFile));
    return false;
  }
...
bool resContFilter::copyAtomsFromArray(int numAtomsSrc, double *atmsSrc,
...
void resContFilter::countAtomsOctreeNodesAndSortAtoms(ATOM *atoms,
                                                      int atomsStartID,
                                                      int atomsEndID,
                                                      ATOM *atomsT,
                                                      int *numNodes) {
...
int resContFilter::constructAtomsOctree(int atomsStartID, int atomsEndID,
                                        ATOM *atoms,
...
void resContFilter::fillMovingAtomsSubtreeRootsArray(int nodeID,
                                                     int maxNodesInLevel) {
  if (nodeID < 0)
    return;
...
bool resContFilter::buildOctrees(void) {
  if ((minRadius != minRadiusUsed) || (maxLeafSize != maxLeafSizeUsed) ||
      (distCutoff != distCutoffUsed) || (!staticAtomsOctreeBuilt))
...
void resContFilter::computeResResInteractions(Matrix transMat, int nodeS,
                                              int nodeM,
                                              double *interactionValuePos,
  ...
  if (d2 > (sumRad + distCutoff) * (sumRad + distCutoff))
    farEnough = true;
    ...
    if (staticAtomsOctree[nodeS].leaf && movingAtomsOctree[nodeM].leaf) {
      for (int i = movingAtomsOctree[nodeM].atomsStartID;
        ...
        for (int j = staticAtomsOctree[nodeS].atomsStartID;
          ...
          if (d2 <= distCutoffSQ) {
            if (intVal[rIDM][rIDS] > 0.0)
              (*interactionValuePos) += intVal[rIDM][rIDS];
            else if (intVal[rIDM][rIDS] < -3.0)
              (*interactionValueNeg) += (-intVal[rIDM][rIDS]);
          }
        }
      }
        ...
        if (staticAtomsOctree[nodeS].cPtr[i] >= 0)
          for (int j = 0; j < 8; j++)
            if (movingAtomsOctree[nodeM].cPtr[j] >= 0) {
              ...
              computeResResInteractions(
                  transMat, staticAtomsOctree[nodeS].cPtr[i],
                  movingAtomsOctree[nodeM].cPtr[j], &intValP, &intValN);
      ...
      for (int i = 0; i < 8; i++)
        if (staticAtomsOctree[nodeS].cPtr[i] >= 0) {
          ...
          computeResResInteractions(transMat, staticAtomsOctree[nodeS].cPtr[i],
                                    nodeM, &intValP, &intValN);
      ...
      for (int j = 0; j < 8; j++)
        if (movingAtomsOctree[nodeM].cPtr[j] >= 0) {
          ...
          computeResResInteractions(transMat, nodeS,
                                    movingAtomsOctree[nodeM].cPtr[j], &intValP,
                                    &intValN);
...
bool resContFilter::computeInteractions(Matrix transMat,
                                        double *interactionValuePos,
                                        double *interactionValueNeg) {
  ...
  if (numThreads == 1)
    computeResResInteractions(transMat, staticAtomsOctreeRoot,
                              movingAtomsOctreeRoot, interactionValuePos,
                              interactionValueNeg);
  else {
    ...
    for (int i = 0; i < numThreads; i++) {
      ...
      pthread_create(&p[i], NULL, computeResResInteractionsThread,
...
bool resContFilter::computeInteractions(double *interactionValuePos,
                                        double *interactionValueNeg) {
  return computeInteractions(transMatrix, interactionValuePos,
                             interactionValueNeg);
}

bool resContFilter::computeInteractionsNaively(Matrix transMat,
                                               double *interactionValuePos,
                                               double *interactionValueNeg) {
  if (printStatus)
...
bool resContFilter::computeInteractionsNaively(double *interactionValuePos,
                                               double *interactionValueNeg) {
  return computeInteractionsNaively(transMatrix, interactionValuePos,
                                    interactionValueNeg);
}
