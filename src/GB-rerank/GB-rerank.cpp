...
void getAtomsQuadsAndGpol(PARAMS_IN *params, char *pqrFile, char *quadFile,
                          int *numAtoms, double **atomsPQR, int *numQPoints,
                          double **qPoints, double *Gpol) {
...
void getGpol(PARAMS_IN *params, int numAtoms, double *atomsPQR, int numQPoints,
             double *qPoints, double *Gpol) {
...
void transformAndCopyMovingMolecule(double *atomsPQR, double *qPoints,
                                    double *transMat, MOLECULE_INFO *staticMol,
                                    MOLECULE_INFO *movingMol) {
  for (int i = 0; i < movingMol->numAtoms; i++) {
    int j = staticMol->numAtoms + i;

    double x = movingMol->atoms[5 * i + 0], y = movingMol->atoms[5 * i + 1],
           z = movingMol->atoms[5 * i + 2], q = movingMol->atoms[5 * i + 3],
           r = movingMol->atoms[5 * i + 4];

    double xx = transMat[0] * x + transMat[1] * y + transMat[2] * z +
                transMat[3],
           yy = transMat[4] * x + transMat[5] * y + transMat[6] * z +
                transMat[7],
           zz = transMat[8] * x + transMat[9] * y + transMat[10] * z +
                transMat[11];
  ...
  for (int i = 0; i < movingMol->numQPoints; i++) {
    double x = movingMol->qPoints[7 * i + 0], y = movingMol->qPoints[7 * i + 1],
    ...
    double nxx = (transMat[0] * (x + nx) + transMat[1] * (y + ny) +
                  transMat[2] * (z + nz) + transMat[3]) -
                 xx,
           nyy = (transMat[4] * (x + nx) + transMat[5] * (y + ny) +
                  transMat[6] * (z + nz) + transMat[7]) -
                 yy,
           nzz = (transMat[8] * (x + nx) + transMat[9] * (y + ny) +
                  transMat[10] * (z + nz) + transMat[11]) -
                 zz;
...
double computeXlateForPG(MOLECULE_INFO *staticMol, MOLECULE_INFO *movingMol) {
  ...
  for (int i = 0; i < staticMol->numQPoints; i++) {
    if (i == 0)
      minXYZ = staticMol->qPoints[5 * i + 0];
...
void printRerankingParamters(PARAMS_IN *p, FILE *fp) {
  ...
  for (int i = 0; i < p->numBands; i++)
    fprintf(fp, "%d:%d%s",
            (i > 0) ? (p->bands[2 * i] - p->bands[2 * i - 2]) : p->bands[2 * i],
            p->bands[2 * i + 1], (i < p->numBands - 1) ? "-" : "");
...
bool rerankF2DockOutput(PARAMS_IN *params, MOLECULE_INFO *staticMol,
                        MOLECULE_INFO *movingMol) {
  ...
  if (ifp == NULL) {
    printError(std::format("Failed to open F2Dock output file ( {} )!",
                           params->F2DockOutputFile));
    return false;
  }
  ...
  if (ofp == NULL) {
    printError(std::format("Failed to create reranked output file ( {} )!",
                           params->rerankedOutputFile));
    freeClose1();
    return false;
  }
  ...
  while (fgets(s, 1999, ifp) != NULL) {
    if (skipInitial(s, (char *)"# OUTPUT FORMAT:", t)) {
      ...
      if (sscanf(t, "%d", &m) != 1) {
        printError(std::format("Error reading F2Dock output file ( {} )!",
                               params->F2DockOutputFile));
        freeClose2();
        return false;
      }
      ...
      // clang-format off
      fprintf(ofp, "#	 COLNAME new_rank int rank of this docking result "
                   "after GB based reranking\n");
      fprintf(ofp, "#	 COLNAME new_score float new score computed from "
                   "delgpol and original F2Dock score\n");
      fprintf(ofp, "#	 COLNAME delgpol float reduction in polarization "
                   "energy in kcal/mol\n");
      fprintf(ofp, "#	 COLNAME areaprop float approximation to constant * "
                   "surfaceArea \n");
      ...
      while (fgets(s, 1999, ifp) != NULL) {
        ...
        if (skipInitial(s, (char *)"#	 COLNAME mat1 ", t))
          matLoc = m;
        ...
        m++;
        if ((matLoc > 0) && (rmsdLoc > 0) && (scoreLoc > 0))
          break;
      }
    } else if (skipInitial(s, (char *)"# START PEAKS", t)) {
      if ((matLoc <= 0) || (rmsdLoc <= 0)) {
        printError(
            std::format("Missing format data in F2Dock output file ( {} )!",
                        params->F2DockOutputFile));
        freeClose2();
        return false;
      }
      ...
      if ((matLoc <= rmsdLoc) && (matLoc + 12 > rmsdLoc)) {
        printError(
            std::format("Invalid format data in F2Dock output file ( {} )!",
                        params->F2DockOutputFile));
        freeClose2();
        return false;
      }
      ...
      double *atomsPQR = (double *)malloc(
          5 * (staticMol->numAtoms + movingMol->numAtoms) * sizeof(double));
      double *qPoints = (double *)malloc(
          7 * (staticMol->numQPoints + movingMol->numQPoints) * sizeof(double));
      ...
      Point *staticQPoints =
          (Point *)malloc(staticMol->numQPoints * sizeof(Point));
      Point *movingQPoints =
          (Point *)malloc(movingMol->numQPoints * sizeof(Point));
      ...
      while (fgets(s, 1999, ifp) != NULL) {
        ...
        if (!gotNumSol) {
          ...
          if ((params->numSol > 0) && (numSol > params->numSol))
            numSol = params->numSol;

          solInfo =
              (SOLUTION_INFO *)malloc((numSol + 1) * sizeof(SOLUTION_INFO));
        ...
        if (solIndex-- > numSol) {
          fprintf(ofp, "%6d %16.5lf %16.5lf %16.5lf %s", solIndex + 1,
                  (double)0.0, (double)0.0, (double)0.0, s);
        ...
        if (!getDoublesInRange(s, matLoc, matLoc + 11, trans) ||
            !getDoublesInRange(s, rmsdLoc, rmsdLoc, &rmsd)) {
          printError(std::format("Error reading F2Dock output file ( {} )!",
                                 params->F2DockOutputFile));
          freeClose6();
          return false;
        ...
        transformAndCopyMovingMolecule(atomsPQR, qPoints, trans, staticMol,
                                       movingMol);
        ...
        for (int i = 0; i < movingMol->numQPoints; i++) {
          if (!staticPG->pointsWithinRange(&movingQPoints[i],
            ...
            totalQPointsWeight += qPoints[7 * i + 6];
        }

        for (int i = 0; i < staticMol->numQPoints; i++) {
          if (!movingPG->pointsWithinRange(&staticQPoints[i],
                                           params->distanceCutoff)) {
        ...
        //                   for ( int i = 0; i < movingMol->numQPoints; i++ )
        //                     {
        //                       if ( !staticPG->pointsWithinRange(
        //                       &movingQPoints[ i ], params->distanceCutoff ) )
        //                         {
        //   //                           totalQPointsWeight +=
        //   movingMol->qPoints[ 7 * i + 6 ];
        //
        //                           for ( int j = 0; j < 7; j++ )
        //                             qPoints[ 7 * totalQPoints + j ] =
        //                             movingMol->qPoints[ 7 * i + j ];
        //
        //                           totalQPoints++;
        //                         }
        //                       else totalQPointsWeight += movingMol->qPoints[
        //                       7 * i + 6 ];
        //                     }
        ...
        if (params->GpolWeight != 0)
          getGpol(params, staticMol->numAtoms + movingMol->numAtoms, atomsPQR,
                  totalQPoints, qPoints, &Gpol);
        ...
        solInfo[i].newScore = params->GnonpolWeight * solInfo[i].areaProp +
                              params->GpolWeight * solInfo[i].DelGpol +
                              params->F2DockScoreWeight * origScore;
      ...
      for (int j = 0, k = 0; j < params->numBands; j++) {
        ...
        int maxK =
            ((params->bands[2 * j] > numSol) ? numSol : params->bands[2 * j]);

        //                   printf( "\n j = %d, params->bands[ 2 * j ] = %d,
        //                   params->bands[ 2 * j + 1 ] = %d, maxK = %d\n",
        //                               j, params->bands[ 2 * j ],
        //                               params->bands[ 2 * j + 1 ], maxK );
        //                   fflush( stdout );
        //
        //                   printf( "curIndex = %d, prevIndex = %d, nextIndex =
        //                   %d\n", curIndex, prevIndex, nextIndex ); fflush(
        //                   stdout );
        ...
        while (k < maxK) {
          while (solInfo[nextIndex].F2DockRank > params->bands[2 * j + 1]) {
            ...
            //                           printf( "prevIndex = %d, nextIndex =
            //                           %d\n", prevIndex, nextIndex ); fflush(
            //                           stdout );
          }
          ...
          //                       printf( "k = %d, curIndex = %d, prevIndex =
          //                       %d, nextIndex = %d\n", k, curIndex,
          //                       prevIndex, nextIndex ); fflush( stdout );

          k++;
        }

        //                   printf( "(outside) curIndex = %d, prevIndex = %d,
      ...
      while (curIndex != numSol) {
        ...
        fprintf(ofp, "%6d %16.5lf %16.5lf %16.5lf %s", i, solInfo[k].newScore,
                solInfo[k].DelGpol, solInfo[k].areaProp, solInfo[k].sol);
      ...
      /*               while ( i-- )
                       {
                         ...
                         pairH->Delete_Min( k, d );
                         fprintf( ofp, "%6d %16.5lf %16.5lf %16.5lf %s", i + 1,
         solInfo[ k ].newScore, solInfo[ k ].DelGpol, solInfo[ k ].areaProp,
         solInfo[ k ].sol );

                         if ( ( indexMinRMSD == -1 ) || ( solInfo[ k ].rmsd <
         minRMSD ) )
                           {
                             indexMinRMSD = k;
                             minRMSD = solInfo[ k ].rmsd;
      ...
      */
      ...
      while (fgets(s, 1999, ifp) != NULL) {
        if (!skipInitial(s, (char *)"# [", t))
          break;
      }

      f_printf(ofp, s);
    } else if (skipInitial(s, (char *)"# good peaks under ", t)) {
      f_printf(ofp, std::format("# good peaks under {:.0f} A: count = {} "
                                "highest rank = {} min RMSD = {:f}\n",
      ...
      f_printf(ofp, std::format(
                        "# best peak: rmsd = {:f} rank = {} delgpol = {:f} {}",
                        minRMSD, rankMinRMSD,
                        (indexMinRMSD < 0) ? ((double)0.0)
                                           : solInfo[indexMinRMSD].DelGpol,
                        t + i));
...
#define returnSpectrumError1()                                                 \
  {                                                                            \
    printError(std::format("Invalid spectrum ( {} )!", spectrum));             \
    return false;                                                              \
  }
...
bool getParamsFromFile(PARAMS_IN *p, char *paramFile) {
  ...
  while (fgets(s, 1999, fp) != NULL) {
    ...
    if (!strcasecmp(key, "staticMoleculePQR"))
      p->staticMoleculePQR = strdup(val);
    else if (!strcasecmp(key, "movingMoleculePQR"))
      p->movingMoleculePQR = strdup(val);
    else if (!strcasecmp(key, "staticMoleculeQUAD"))
      p->staticMoleculeQUAD = strdup(val);
    else if (!strcasecmp(key, "movingMoleculeQUAD"))
      p->movingMoleculeQUAD = strdup(val);
    else if (!strcasecmp(key, "F2DockOutputFile"))
      p->F2DockOutputFile = strdup(val);
    else if (!strcasecmp(key, "rerankedOutputFile"))
      p->rerankedOutputFile = strdup(val);
    else if (!strcasecmp(key, "F2DockScoreWeight"))
      p->F2DockScoreWeight = atof(val);
    else if (!strcasecmp(key, "GpolWeight"))
      p->GpolWeight = atof(val);
    else if (!strcasecmp(key, "GnonpolWeight"))
      p->GnonpolWeight = atof(val);
      ...
      p->numSol = v;
    } else if (!strcasecmp(key, "spectrum"))
      spectrum = strdup(val);
  }
...
void getMoleculeInformation(PARAMS_IN *params, MOLECULE_INFO *staticMol,
                            MOLECULE_INFO *movingMol) {
  getAtomsQuadsAndGpol(params, params->staticMoleculePQR,
                       params->staticMoleculeQUAD, &(staticMol->numAtoms),
                       &(staticMol->atoms), &(staticMol->numQPoints),
                       &(staticMol->qPoints), &(staticMol->Gpol));

  getAtomsQuadsAndGpol(params, params->movingMoleculePQR,
                       params->movingMoleculeQUAD, &(movingMol->numAtoms),
                       &(movingMol->atoms), &(movingMol->numQPoints),
                       &(movingMol->qPoints), &(movingMol->Gpol));
}
...
int main(int argc, char *argv[]) {
  ...
  if (!rerankF2DockOutput(&params, &staticMol, &movingMol))
    return 1;
