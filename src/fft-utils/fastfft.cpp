...
/*
   ...
   The following function performs STEP 1 (gridding) of algorithm 1.2 in
   section 1.2 of the following paper:

   Potts, D., Steidl G., and Tasche M.
   Fast Fourier transforms for nonequispaced data: A tutorial.
   in: Modern Sampling Theory: Mathematics and Applications, J.J. Benedetto and
   P. Ferreira (Eds.), Chapter 12, pages 249-274, 1998.

   We are given M atoms, with the center of atom s \in [0, M - 1] in < x[ s ],
   y[ s ], z[ s ] >, radius in r[ s ], and the weight at the atom center in f[ s
   ]. Each center < x[ s ], y[ s ], z[ s ] > \in ( < -1/2 + m / n, -1/2 + m / n,
   -1/2 + m / n >, < 1/2 - m / n, 1/2 - m / n, 1/2 - m / n > ), where m is large
   enough so that m > n * max_s{ r[ s ] }, and n^3 is the size of the 3D output
   grid gHat. The smoothingFunction object gives access to precomputed Gaussian
   bell function Phi (which is actually truncated to [-m, m], and is referred to
   as Psi in the paper).

   We compute gHat( l ) = \sum_{ s \in [0, M): l - m <= n * w_s <= l + m }{ f_s
   * Phi( w_s - l / n ) }, where l \in [ - n/2, n/2 )^3, w_s = < x[ s ], y[ s ],
   z[ s ] >, f_s = exp( - blobbiness * r[ s ] * r[ s ] ) * f[ s ], and Phi( ) is
   the truncated [-m, m] Gaussian bell function used for smoothing. Actually,
   the gHat array computed below by the gridding function is shifted by ( n/2,
   n/2, n/2 ), that is, the origin is at ( n/2, n/2, n/2 ), and not at ( 0, 0, 0
   ).
*/
...
void gridding(int M, double *x, double *y, double *z, float *r, char *type,
  ...
  // printf("gridding %d atoms: smoothSkin %d, spreadSkin %d numfreq %d m
  // %d\n",M,  smoothSkin, spreadSkin, n, m);

  for (int s = 0; s < M; s++) {
    // map from ( < -1/2 + m / n, -1/2 + m / n, -1/2 + m / n >, < 1/2 - m / n,
    // 1/2 - m / n, 1/2 - m / n > )
      ...
      for (int yt = yL; yt <= yU; yt++)
        for (int xt = xL; xt <= xU; xt++) {
          ...
          if (smoothSkin) {
            ...
            // value of the smoothing function at [ xt + n / 2, yt + n / 2, zt +
            // n / 2 ]
            double bellVal =
                smoothingFunction->getPhi(((zt + ofs) - zc) /*/ rc*/) *
                smoothingFunction->getPhi(((yt + ofs) - yc) /*/ rc*/) *
                smoothingFunction->getPhi(((xt + ofs) - xc) /*/ rc*/);

            // add atom s's contribution to gHat[ xt + n / 2, yt + n / 2, zt + n
            // / 2 ]
            gHat[index][0] +=
                f[s][0] * bellVal; // * exp( -blobbiness * rc * rc );
            gHat[index][1] +=
                f[s][1] * bellVal; // * exp( -blobbiness * rc * rc );
          } else {
            if (d2 > rc2)
              continue;
  ...
  if (spreadSkin) {
    ...
    for (int s = 0; s < M; s++)
      if (type[s] == 'E') {
        // map from ( < -1/2 + m / n, -1/2 + m / n, -1/2 + m / n >, < 1/2 - m /
        // n, 1/2 - m / n, 1/2 - m / n > )
        //       to ( < -n/2 + m, -n/2 + m, -n/2 + m >, < n/2 - m, n/2 - m, n/2
        //       - m > )
        double xc = x[s] * (n - 1), yc = y[s] * (n - 1), zc = z[s] * (n - 1);
        double rc = r[s] * (n - 1);
        double rc2 = rc * rc;

        double md = 0.5 * m + rc; // ceil( pow( ( wss * f[ s ][ 0 ] ) / ( maxRe
                                  // / ( ( M - MCore ) * 1000.0 ) ),  1 / p ) );
        double md2 = md * md;
        ...
        for (int zt = zL; zt <= zU; zt++)
          for (int yt = yL; yt <= yU; yt++)
            for (int xt = xL; xt <= xU; xt++) {
              ...
              // add atom s's contribution to gHat[ xt + n / 2, yt + n / 2, zt +
              // n / 2 ]
...
void griddingHbond(int M, double *x, double *y, double *z, float *r,
  ...
  for (int s = 0; s < M; s++) {
    if ((f[s][0] == 0) && (f[s][1] == 0))
      continue;
    //         if ( ( hbondType[ s ] != 'A' ) && ( hbondType[ s ] != 'D' ) )
    //         continue;
    ...
    if ((movingMol && (f[s][1] != 0)) || (!movingMol && (f[s][0] != 0)))
      rc += rExp;
...
/*
   ...
   It is an implementation of algorithm 1.2 in section 1.2 of the following
   paper:
   ...
   -1/2 + m / n >, < 1/2 - m / n, 1/2 - m / n, 1/2 - m / n > ), where m is large
   enough so that m > n * max_s{ r[ s ] }, n = alpha * N, alpha (>= 1) is the
   oversampling factor for NFFT, and N^3 is the number of frequencies we want to
   compute. The computed frequencies are returned in hHat, while gHat (of size
   n^3) and the 3D FFTW plan gHatPlan are used for intermediate computation
   (gridding and 3D FFT) by NFFT. The smoothingFunction object gives access to
   precomputed Gaussian bell function Phi (which is actually truncated to [-m,
   m], and is referred to as Psi in the paper), and its FFT PhiHat.
*/
...
bool getCenterFrequencies(int M, double *x, double *y, double *z, float *r,
  ...
  if ((!griddingDone && (!x || !y || !z || !f)) ||
      (smoothSkin && !smoothingFunction) || !hHat || !gHat)
    return false;
  ...
  if (!griddingDone) {
    // STEP 1: Compute gHat( l ) = \sum_{ s \in [0, M): l - m <= n * w_s <= l +
    // m }{ f_s * Psi( w_s - l / n ) },
    //         where l \in [ - n/2, n/2 )^3, w_s = < x[ s ], y[ s ], z[ s ] >,
    //         f_s = exp( - blobbiness * r[ s ] * r[ s ] ) * f[ s ], and Psi( )
    //         is the truncated [-m, m] Gaussian bell function used for
    //         smoothing. Actually, the gHat array computed below by the
    //         gridding function is shifted by ( n/2, n/2, n/2 ), that is, the
    //         origin is at ( n/2, n/2, n/2 ), and not at ( 0, 0, 0 )

    gridding(M, x, y, z, r, type, f, blobbiness, n, m, smoothSkin,
             smoothingFunction, gHat, spreadSkin);
  }

  // STEP 2: Compute 3D FFT of gHat: for t \in [ - N/2, N/2 )^3,
  //               cHat( t ) = ( 1 / n^3 ) * \sum_{ l \in I_n }{ gHat( l ) *
  //               exp( - 2 * \Pi * i * t * l / n ) }.
  //         We actually compute cHat( t ) for t \in [ - n/2, n/2 )^3, and
  //         retain only the entries for t \in [ - N/2, N/2 )^3 in step 3. The
  //         cHat array is computed inplace on gHat, but the origin of cHat is
  //         no longer at ( n/2, n/2, n/2 ). The function has period n in all 3
  //         directions, and the origin is at
  //           ( 0, 0, 0 ) = ( 0, 0, n - 1 ) = ( 0, n - 1, n - 1 ) = ( 0, n - 1,
  //           0 )
  //         = ( n - 1, 0, 0 ) = ( n - 1, 0, n - 1 ) = ( n - 1, n - 1, n - 1 ) =
  //         ( n - 1, n - 1, 0 ).

  if (gHatSparsePlan != NULL)
    sparse3DFFT(gHatSparsePlan, gHat, cHat);
  else
    FFTW_execute(gHatPlan);

  // STEP 3: Compute hHat( t ) = cHat( t ) / PhiHat( t ), for t \in [ - N/2, N/2
  // )^3.
  //         Observe that since the origin of cHat is at its eight corners, we
  //         only need to copy a cube of size (N/2)^3 from each corner of gHat
  //         (= cHat) to the corresponding corner of hHat.

  if (griddingDone) {
    for (int c = 0, d = 0, i = 0; i < n; i++)
      for (int j = 0; j < n; j++)
        for (int k = 0; k < n; k++, c++)
          if ((i < N / 2 || n - i <= N / 2) && (j < N / 2 || n - j <= N / 2) &&
              (k < N / 2 || n - k <= N / 2)) {
            if (smoothSkin) {
              double ijkPsiBar = smoothingFunction->getPhiHat(min(i, n - i)) *
                                 smoothingFunction->getPhiHat(min(j, n - j)) *
                                 smoothingFunction->getPhiHat(min(k, n - k));
...
bool getCenterElecFrequencies(int M, double *x, double *y, double *z, float *r,
  ...
  if (!griddingDone) {
    ...
    if (gHat == (FFTW_DATA_TYPE *)hHat)
      forInPlaceFFT = true;
    else
      forInPlaceFFT = false;

    griddingElec(M, x, y, z, r, type, f, blobbiness, n, elecRadiusInGrids, gHat,
                 forInPlaceFFT, movingMol);
  }
...
bool getCenterHbondFrequencies(int M, double *x, double *y, double *z, float *r,
  ...
  if (!griddingDone)
    griddingHbond(M, x, y, z, r, rExp, f, blobbiness, n, gHat, movingMol);
...
bool getCenterHydrophobicityFrequencies(
  ...
  if (!griddingDone)
    griddingHydrophobicity(M, x, y, z, r, f, blobbiness, n, gHat, hydroRadExt,
                           pairWise);
...
bool getCenterSimpleComplementarityFrequencies(
  ...
  if (!griddingDone)
    griddingSimpleComplementarity(M, x, y, z, r, f, blobbiness, n, gHat,
                                  simpleRadExt);
