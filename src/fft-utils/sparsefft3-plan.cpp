...
static void update_range(sparse3DFFT_range *r, int x) {
  ...
  while (cur_r) {
    if ((x >= cur_r->min) && (x <= cur_r->max))
      return; /* x is already in an element of r */
  ...
  if (cur_r) {
    ...
    /* need a new interval */
    sparse3DFFT_range new_r =
        (sparse3DFFT_range)FFTW_malloc(sizeof(sparse3DFFT_range_struct));
...
sparse3DFFT_time sparse3DFFT_gettimeofday_time_diff(sparse3DFFT_time t1,
                                                    sparse3DFFT_time t2) {
...
double time_sparse3DFFT(sparse3DFFT_plan p, FFTW_complex *data_in,
...
sparse3DFFT_create_plan(int nx, int ny, int nz, int dir, int flags,
  ...
  if ((nx < 1) || (ny < 1) || (nz < 1) || (data_in == NULL) ||
      (data_out == NULL))
    return NULL;

  sparse3DFFT_plan p =
      (sparse3DFFT_plan)FFTW_malloc(sizeof(sparse3DFFT_plan_struct));
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      if (j != i) {
        p->range1[i][j] = (sparse3DFFT_range *)FFTW_malloc(
            sizeof(sparse3DFFT_range) * p->n[i]);
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++) {
      for (int k = 0, d = 1; k <= MAX_LOG_HOWMANY; k++, d *= 2) {
        if ((d - 1) * p->nafter[j] + (p->n[i] - 1) * p->nafter[i] >=
            p->n[0] * p->n[1] * p->n[2])
          p->p[i][j][k] = NULL;
        else
          p->p[i][j][k] = FFTW_plan_many_dft(
              1, &(p->n[i]), d, data_out, NULL, p->nafter[i], p->nafter[j],
              data_out, NULL, p->nafter[i], p->nafter[j], dir, flags);
      }
    }
      ...
      for (int iz = 0; iz < nz; ++iz) {
        ...
        if (nonzero(x, nonzero_data)) {
          for (int i = 0; i < 3; ++i) {
            ...
            for (int j = 0; j < 3; ++j)
              if (j != i)
                update_range(&p->range1[i][j][x[i]], x[j]);
          }
        }
      }
  ...
  if ((flags & OPT_FFTW_SEARCH_TYPE) != FFTW_ESTIMATE) {
    int best_fft2_first = 0, best_first_dim = 0, best_second_dim = 1,
        best_third_dim = 2;
    double best_time = 1e20;

    for (int dim2 = 0; dim2 < nx * ny * nz; ++dim2)
      c_re(data_in[dim2]) = c_im(data_in[dim2]) = c_re(data_out[dim2]) =
          c_im(data_out[dim2]) = 0.0;
