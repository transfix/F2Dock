...
static void performFFT(sparse3DFFT_plan p, int dim1, int dim2, int howmany,
                       FFTW_complex *data) {
  int dist = 0;
...
/* Given the 3d array data conforming to the sparsity pattern,
   ...
   along plane_dim. */
static void fft2_planes(sparse3DFFT_plan p, int plane_dim, int first_dim,
                        int second_dim, FFTW_complex *data) {
  for (int iplane = 0; iplane < p->n[plane_dim]; ++iplane) {
    ...
    while (r) {
      performFFT(p, first_dim, second_dim, r->max - r->min + 1,
                 data + iplane * p->nafter[plane_dim] +
                     r->min * p->nafter[second_dim]);
      r = r->next;
    }

    if (p->range1[plane_dim][second_dim][iplane])
      performFFT(p, second_dim, first_dim, p->n[first_dim],
                 data + iplane * p->nafter[plane_dim]);
  }
}

/* inverse of fft2_planes. */
static void ifft2_planes(sparse3DFFT_plan p, int plane_dim, int first_dim,
                         int second_dim, FFTW_complex *data) {
  for (int iplane = 0; iplane < p->n[plane_dim]; ++iplane) {
    sparse3DFFT_range r = p->range1[plane_dim][second_dim][iplane];
...
static void fft1_planes(sparse3DFFT_plan p, int plane_dim, int first_dim,
                        int second_dim, FFTW_complex *data) {
  for (int iplane = 0; iplane < p->n[plane_dim]; ++iplane) {
    sparse3DFFT_range r = p->range1[plane_dim][second_dim][iplane];
...
static void fft2_planes2(sparse3DFFT_plan p, int plane_dim, int second_dim,
                         int third_dim, FFTW_complex *data) {
  for (int iplane = 0; iplane < p->n[plane_dim]; ++iplane) {
    ...
    while (r) {
      performFFT(p, second_dim, third_dim, r->max - r->min + 1,
                 data + iplane * p->nafter[plane_dim] +
                     r->min * p->nafter[third_dim]);
      r = r->next;
    }

    performFFT(p, third_dim, second_dim, p->n[second_dim],
               data + iplane * p->nafter[plane_dim]);
  }
}

/* inverse of fft2_planes2 */
static void ifft2_planes2(sparse3DFFT_plan p, int plane_dim, int second_dim,
                          int third_dim, FFTW_complex *data) {
  for (int iplane = 0; iplane < p->n[plane_dim]; ++iplane) {
    sparse3DFFT_range r = p->range2[third_dim];
...
static void fft1_all(sparse3DFFT_plan p, int which_dim, FFTW_complex *data) {
  ...
  if (which_dim == 0)
    performFFT(p, 0, 2, p->nafter[0], data);
  else if (which_dim == 2)
    performFFT(p, 2, 1, p->n[0] * p->n[1], data);
  else {
    ...
    for (int ifirst = 0; ifirst < p->n[first_dim]; ++ifirst)
      performFFT(p, which_dim, second_dim, p->n[second_dim],
...
void sparse3DFFT(sparse3DFFT_plan p, FFTW_complex *data_in,
  ...
  if (data_out != data_in)
    memcpy(data_out, data_in,
           (p->n[0]) * (p->n[1]) * (p->n[2]) * sizeof(FFTW_complex));
