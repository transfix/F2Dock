...
bool create3DFFTWisdom(FFTW_complex *dest, FFTW_complex *src, int dim1,
...
bool compute3DFFT(FFTW_complex *dest, FFTW_complex *src, int dim1, int dim2,
                  int dim3, int type, unsigned int flags, FILE *wisdomFP)

{
  ...
  p = FFTW_plan_dft_3d(dim1, dim2, dim3, src, dest, type,
...

{
  ...
  FFTW_complex *src =
      (FFTW_complex *)FFTW_malloc(size * size * size * sizeof(FFTW_complex));
  ...
  if (!create3DFFTWisdom(dest, src, size, size, size, FFTW_FORWARD,
                         OPT_FFTW_SEARCH_TYPE, fp)) {
    FFTW_free(src);
    if (!inPlace)
      FFTW_free(dest);
  ...
  if (!create3DFFTWisdom(dest, src, size, size, size, FFTW_BACKWARD,
                         OPT_FFTW_SEARCH_TYPE, fp)) {
    FFTW_free(src);
    if (!inPlace)
      FFTW_free(dest);
  ...
  for (int i = 0; i < iter; i++) {
    if (!compute3DFFT(dest, src, size, size, size, FFTW_FORWARD,
                      OPT_FFTW_SEARCH_TYPE, fp)) {
      FFTW_free(src);
      if (!inPlace)
        FFTW_free(dest);
    ...
    if (!compute3DFFT(dest, src, size, size, size, FFTW_BACKWARD,
                      OPT_FFTW_SEARCH_TYPE, fp)) {
      FFTW_free(src);
      if (!inPlace)
        FFTW_free(dest);
...
#else
  printf("# Measuring 3D FFTW performance on double precision floats (define "
         "FFTW_SINGLE_PRECISION and link to fftw3f for single precision "
         "floats)\n");
#endif

#if OPT_FFTW_SEARCH_TYPE == FFTW_MEASURE
  printf("# Using flag FFTW_MEASURE (redefine OPT_FFTW_SEARCH_TYPE for other "
         "options: FFTW_ESTIMATE, FFTW_PATIENT and FFTW_EXHAUSTIVE)\n");
#else
#if OPT_FFTW_SEARCH_TYPE == FFTW_PATIENT
  printf("# Using flag FFTW_PATIENT (redefine OPT_FFTW_SEARCH_TYPE for other "
         "options: FFTW_ESTIMATE, FFTW_MEASURE and FFTW_EXHAUSTIVE)\n");
#else
#if OPT_FFTW_SEARCH_TYPE == FFTW_EXHAUSTIVE
  printf("# Using flag FFTW_EXHAUSTIVE (redefine OPT_FFTW_SEARCH_TYPE for "
         "other options: FFTW_ESTIMATE, FFTW_MEASURE and FFTW_PATIENT)\n");
#else
  printf("# Using flag FFTW_ESTIMATE (redefine OPT_FFTW_SEARCH_TYPE for other "
         "options: FFTW_MEASURE, FFTW_PATIENT and FFTW_EXHAUSTIVE)\n");
#endif
#endif
#endif

#ifdef SAVE_WISDOM_TO_FILE
  printf("# Saving wisdom to %s (filename can be changed by redefining "
         "WISDOM_FILE)\n#\n",
         (char *)WISDOM_FILE);
#else
  printf("# Wisdom will not be saved (since SAVE_WISDOM_TO_FILE is not "
         "defined)\n#\n");
#endif
...
int computeEffGrid(int minSize, int maxSize) {
  ...
  if (errID == NO_ERROR)
    errID = rankFFTWSizesByTime(minSize, maxSize);
  if (errID != NO_ERROR)
    printError(errID);
