...
void fastBornRadius::printCurrentSettings(void) {
  ...
  printf((char *)"\tminRadius = %lf, maxLeafSize = %d, epsilon = %lf, "
                 "numThreads = %d\n\n",
         minRadius, maxLeafSize, epsilon, numThreads);
}

fastBornRadius::fastBornRadius(char *qPtsFile, char *atmsPQRFile,
                               bool printStat) {
  setDefaults();
...
fastBornRadius::fastBornRadius(int numQPoints, double *qPts, int numAtoms,
                               double *atms, bool printStat) {
  setDefaults();

  printStatus = printStat;

  if (!copyQPointsFromArray(numQPoints, qPts) ||
      !copyAtomsFromArray(numAtoms, atms)) {
    freeMemory();
    exit(1);
  }
...
fastBornRadius::fastBornRadius(char *qPtsFile, int numAtoms, double *atms,
                               bool printStat) {
  setDefaults();
...
int fastBornRadius::nextSubtreeRoot(void) {
  ...
  if (curSubtreeRoot < maxSubtreeRoot)
    nextRoot = curSubtreeRoot++;
...
bool fastBornRadius::readQPoints(char *qPtsFile) {
  ...
  while (fscanf(fp, (char *)"%lf %lf %lf %lf %lf %lf %lf", &qPt.x, &qPt.y,
                &qPt.z, &qPt.nx, &qPt.ny, &qPt.nz, &qPt.w) == 7)
    nQPoints++;
  ...
  if (printStatus)
    printf((char *)"done ( %lf sec, read %d quadrature points )\n",
...
bool fastBornRadius::copyQPointsFromArray(int numQPoints, double *qPts) {
  ...
  if (printStatus)
    printf((char *)"done ( %lf sec, copied %d quadrature points )\n",
           endT - startT, nQPoints);
...
bool fastBornRadius::readAtomsFromPQR(char *atomsFile) {
  ...
  if (printStatus)
    printf((char *)"done ( %lf sec, read %d atoms )\n", endT - startT, nAtoms);
...
bool fastBornRadius::readAtoms(char *atomsFile) {
  ...
  if ((l > 3) && (atomsFile[l - 4] == '.') &&
      (toupper(atomsFile[l - 3]) == 'P') &&
...
bool fastBornRadius::copyAtomsFromArray(int numAtoms, double *atms) {
  ...
  if (printStatus)
    printf((char *)"done ( %lf sec, copied %d atoms )\n", endT - startT,
           nAtoms);
...
void fastBornRadius::transformAtomsOctree(int nodeID, double *transMatrix) {
  ...
  atomsOctree[nodeID].cx = transMatrix[0] * cx + transMatrix[1] * cy +
                           transMatrix[2] * cz + transMatrix[3];
  atomsOctree[nodeID].cy = transMatrix[4] * cx + transMatrix[5] * cy +
                           transMatrix[6] * cz + transMatrix[7];
  atomsOctree[nodeID].cz = transMatrix[8] * cx + transMatrix[9] * cy +
                           transMatrix[10] * cz + transMatrix[11];
...
void fastBornRadius::transformAtoms(double *transMatrix) {
  ...
  for (int i = 0; i < nAtoms; i++) {
    ...
    atoms[i].x = transMatrix[0] * x + transMatrix[1] * y + transMatrix[2] * z +
                 transMatrix[3];
    atoms[i].y = transMatrix[4] * x + transMatrix[5] * y + transMatrix[6] * z +
                 transMatrix[7];
    atoms[i].z = transMatrix[8] * x + transMatrix[9] * y + transMatrix[10] * z +
                 transMatrix[11];
  }

  if (atomsOctreeBuilt)
    transformAtomsOctree(atomsOctreeRoot, transMatrix);

  double endT = getTime();

  if (printStatus)
    printf((char *)"done ( %lf sec )\n", endT - startT);
}

void fastBornRadius::countQPointsOctreeNodesAndSortQPoints(QPOINT *qPts,
                                                           int qPtsStartID,
                                                           int qPtsEndID,
                                                           QPOINT *qPtsT,
                                                           int *numNodes) {
...
int fastBornRadius::constructQPointsOctree(int qPtsStartID, int qPtsEndID,
                                           QPOINT *qPts) {
  ...
  qPointsOctree[nodeID].wnx = qPointsOctree[nodeID].wny =
      qPointsOctree[nodeID].wnz = 0;
  ...
  if ((qPtsEndID - qPtsStartID + 1 > maxLeafSize) &&
      (r2 > minRadius * minRadius) /*( cr > minRadius )*/) {
    ...
    for (int i = 0; i < 8; i++)
      if (qPtsCount[i] > 0) {
        int j = constructQPointsOctree(
            qPtsStartIndex[i], qPtsStartIndex[i] + qPtsCount[i] - 1, qPts);
...
bool fastBornRadius::buildQPointsOctree(void) {
  ...
  qPointsOctreeT = (QPOINTS_OCTREE_NODE *)malloc(numQPointsOctreeNodes *
                                                 sizeof(QPOINTS_OCTREE_NODE));

  if (qPointsOctreeT == NULL) {
    printError(std::format(
        "Unable to {} quadrature points octree - memory allocation failed!",
        (qPointsOctreeBuilt) ? "rebuild" : "build"));
    if (!qPointsOctreeBuilt)
      exit(1);
    return false;
  }
...
void fastBornRadius::countAtomsOctreeNodesAndSortAtoms(ATOM *atoms,
                                                       int atomsStartID,
                                                       int atomsEndID,
                                                       ATOM *atomsT,
                                                       int *numNodes) {
...
int fastBornRadius::constructAtomsOctree(int atomsStartID, int atomsEndID,
                                         ATOM *atoms) {
  int nodeID = nextFreeNode();
  ...
  if ((atomsEndID - atomsStartID + 1 <= maxLeafSize) || (cr <= minRadius))
    ...
    for (int i = 0; i < 8; i++)
      if (atomsCount[i] > 0) {
        int j = constructAtomsOctree(
            atomsStartIndex[i], atomsStartIndex[i] + atomsCount[i] - 1, atoms);
...
void fastBornRadius::fillAtomsSubtreeRootsArray(int nodeID,
                                                int maxNodesInLevel) {
  if (nodeID < 0)
    return;

  if (nodeID == atomsOctreeRoot)
    maxSubtreeRoot = 0;
  ...
  if (atomsOctree[nodeID].leaf || (maxNodesInLevel >= atomsSubtreeRootsSize))
    ...
    for (int i = 0; i < 8; i++)
      if (atomsOctree[nodeID].cPtr[i] >= 0)
        fillAtomsSubtreeRootsArray(atomsOctree[nodeID].cPtr[i],
                                   8 * maxNodesInLevel);
  }
}
...
bool fastBornRadius::buildAtomsOctree(void) {
  ...
  atomsOctreeT = (ATOMS_OCTREE_NODE *)malloc(numAtomsOctreeNodes *
                                             sizeof(ATOMS_OCTREE_NODE));

  if (atomsOctreeT == NULL) {
    printError(
        std::format("Unable to {} atoms octree - memory allocation failed!",
                    (atomsOctreeBuilt) ? "rebuild" : "build"));
    if (!atomsOctreeBuilt)
      exit(1);
    return false;
  }
...
void fastBornRadius::cleanupPriorComputation(void) {
  ...
  if (atomsOctreeBuilt)
    cleanupAtomsOctree(atomsOctreeRoot);
...
void fastBornRadius::approximateIntegrals(int nodeA, int nodeQ) {
  ...
  double wnx = qPointsOctree[nodeQ].wnx, wny = qPointsOctree[nodeQ].wny,
         wnz = qPointsOctree[nodeQ].wnz;
  bool farEnough = false;

  if ((sumRad2 > 0.25) && (d2 > 4 * sumRad2) &&
      (d2 > (sumRad2 / (epsilon * epsilon))))
    farEnough = true;

  if (farEnough) {
#ifdef BRR4
    atomsOctree[nodeA].invR += ((wnx * dx + wny * dy + wnz * dz) / (d2 * d2));
    atomsOctree[nodeA].invCR +=
        ((wnx * dx + wny * dy + wnz * dz) / (d2 * d2 * d2 * sqrt(d2)));
#else
    atomsOctree[nodeA].invR +=
        ((wnx * dx + wny * dy + wnz * dz) / (d2 * d2 * d2));
#endif
  } else {
    if (atomsOctree[nodeA].leaf && qPointsOctree[nodeQ].leaf) {
      for (int i = atomsOctree[nodeA].atomsStartID;
        ...
        for (int j = qPointsOctree[nodeQ].qPtsStartID;
          ...
          if (d2 < atoms[i].r * atoms[i].r)
            d2 = atoms[i].r * atoms[i].r;

#ifdef BRR4
          atoms[i].invR += ((w * (nx * dx + ny * dy + nz * dz)) / (d2 * d2));
          atoms[i].invCR +=
              ((w * (nx * dx + ny * dy + nz * dz)) / (d2 * d2 * d2 * sqrt(d2)));
#else
          atoms[i].invR +=
              ((w * (nx * dx + ny * dy + nz * dz)) / (d2 * d2 * d2));
#endif
        }
        ...
        if (atomsOctree[nodeA].cPtr[i] >= 0)
          for (int j = 0; j < 8; j++)
            if (qPointsOctree[nodeQ].cPtr[j] >= 0)
              approximateIntegrals(atomsOctree[nodeA].cPtr[i],
                                   qPointsOctree[nodeQ].cPtr[j]);
...
void fastBornRadius::pushIntegralsToAtoms(int nodeID, double intVal,
                                          double intCVal) {
  if (nodeID < 0)
    return;
  ...
  if (atomsOctree[nodeID].leaf) {
    for (int i = atomsOctree[nodeID].atomsStartID;
         i <= atomsOctree[nodeID].atomsEndID; i++) {
      atoms[i].invR += intVal;
      atoms[i].invCR += intCVal;
      if (atoms[i].invCR < 0.0)
        atoms[i].invCR = 0;
      double invR = COMBINE_INTEGRALS(atoms[i].invR, atoms[i].invCR);
      if (invR > maxBornRadiusInv)
        atoms[i].R = 1.0 / invR;
      else
        atoms[i].R = atoms[i].r; // maxBornRadius;
      if (atoms[i].R < atoms[i].r)
        atoms[i].R = atoms[i].r;
    }
  } else {
    for (int i = 0; i < 8; i++)
...
void fastBornRadius::pushIntegralsAndFillAtomsSubtreeRootsArray(
    int nodeID, double intVal, double intCVal, int maxNodesInLevel) {
  if (nodeID < 0)
    return;
  ...
  if (atomsOctree[nodeID].leaf || (maxNodesInLevel >= atomsSubtreeRootsSize)) {
    ...
    for (int i = 0; i < 8; i++)
      if (atomsOctree[nodeID].cPtr[i] >= 0)
        pushIntegralsAndFillAtomsSubtreeRootsArray(
            atomsOctree[nodeID].cPtr[i], intVal, intCVal, 8 * maxNodesInLevel);
  }
}

bool fastBornRadius::computeBornRadii(void) {
  if (!priorComputationCleared && (minRadius == minRadiusUsed) &&
      (maxLeafSize == maxLeafSizeUsed) && (epsilon == epsilonUsed))
    return false;
...
bool fastBornRadius::computeBornRadiiNaively(void) {
  ...
  for (int i = 0; i < nAtoms; i++) {
    ...
    if (invR > maxBornRadiusInv)
      atoms[i].naiveR = 1.0 / invR;
    else
      atoms[i].naiveR = maxBornRadius;

    if (atoms[i].naiveR < atoms[i].r)
      atoms[i].naiveR = atoms[i].r;
  }
...
bool fastBornRadius::getQPoints(int *numQPoints, double **qPts) {
  ...
  *qPts = (double *)malloc(7 * nQPoints * sizeof(double));
  ...
  if (printStatus)
    printf((char *)"done ( %lf sec, got %d quadrature points )\n",
           endT - startT, nQPoints);
...
bool fastBornRadius::getAtomsPQR(int *numAtoms, double **atomsPQR) {
  ...
  *atomsPQR = (double *)malloc(5 * nAtoms * sizeof(double));
  ...
  if (printStatus)
    printf((char *)"done ( %lf sec, got %d atoms )\n", endT - startT, nAtoms);
...
bool fastBornRadius::getAtomsPQRR(int *numAtoms, double **atomsPQRR,
  ...
  *atomsPQRR = (double *)malloc(6 * nAtoms * sizeof(double));
  ...
  for (int i = 0; i < nAtoms; i++) {
    ...
    if (getNaiveR)
      (*atomsPQRR)[6 * i + 5] = atoms[i].naiveR;
    else
      (*atomsPQRR)[6 * i + 5] = atoms[i].R;
  }
...
double *fastBornRadius::getBornRadii(void) {
  ...
  if (epsilon != epsilonUsed) {
    printError(std::format(
        "Born radii for epsilon = {:f} have not yet been computed!", epsilon));
    return NULL;
  }
...
double *fastBornRadius::getIntegrals(void) {
  ...
  if (epsilon != epsilonUsed) {
    printError(std::format(
        "Integrals for relative error {:f} have not yet been computed!",
        epsilon));
    return NULL;
  }
...
bool fastBornRadius::writePQRRFile(char *outFile) {
  ...
  for (int i = 0; i < nAtoms; i++)
    fprintf(fp, (char *)"%18.6lf %18.6lf %18.6lf %18.6lf %18.6lf %18.6lf\n",
            atoms[i].x, atoms[i].y, atoms[i].z, atoms[i].q, atoms[i].r,
            atoms[i].R);
...
bool fastBornRadius::writeFastAndNaiveBornRadiiToFile(char *outFile) {
  ...
  if (printStatus)
    printf(
        (char *)"\nwriting both fast and naively computed Born radii to %s... ",
        outFile);
  ...
  fprintf(fp,
          (char *)"Time for computing Born radii fast = %lf sec ( speed-up "
                  "factor = %0.2lf )\n",
          fastBRTime, (fastBRTime != 0) ? (naiveBRTime / fastBRTime) : 0);
  fprintf(fp,
          (char *)"( Parameters: minRadius = %lf, maxLeafSize = %d, epsilon = "
                  "%lf, numThreads = %d )\n\n",
  ...
  for (int i = 0; i < nAtoms; i++) {
    ...
    double errorPercent =
        (naiveBornR[i] != 0)
            ? (((BornR[i] - naiveBornR[i]) / naiveBornR[i]) * 100)
            : ((BornR[i] == 0) ? 0 : 1000000000);
#else
    double errorPercent =
        (naiveBornR[i] != 0)
            ? (((BornR[i] - naiveBornR[i]) / naiveBornR[i]) * 100)
            : ((BornR[i] == 0) ? 0 : 10000000000);
#endif

    fprintf(fp, (char *)"%12.6lf  %12.6lf  ( %9.3lf \% )\n", BornR[i],
            naiveBornR[i], errorPercent);
