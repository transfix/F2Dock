...
void fastGpol::printCurrentSettings(void) {
  ...
  printf((char *)"\n\tminRadius = %lf, maxLeafSize = %d, epsilon = %lf, "
                 "numThreads = %d, numAtoms = %d\n",
         minRad, maxLeafAtoms, epsilon, numThreads, numAtoms);
  printf((char *)"\tDielectric constants: ion = %lf, solvent = %lf\n", e_in,
         e_out);

  if (useApproxMath)
    printf((char *)"\tUse approximate math functions: YES\n");
  else
    printf((char *)"\tUse approximate math functions: NO\n");
...
fastGpol::fastGpol(char *pqrRFile) { fastGpol(pqrRFile, true); }
...
fastGpol::fastGpol(int nAtoms, double *pqrR) { fastGpol(nAtoms, pqrR, true); }
...
bool fastGpol::readFromPQRRFile(char *fname) {
  ...
  for (i = 0; i < numAtoms; i++) {
    ...
    if (fscanf(fp, (char *)"%lf %lf %lf %lf %lf %lf", &x, &y, &z, &q, &r, &R) !=
        6)
      break;
  ...
  if (i < numAtoms) {
    printError(
        std::format("Not enough data in input file {} (expected {}, found {})!",
                    fname, numAtoms, i));
    numAtoms = 0;
    fclose(fp);
    return false;
...
bool fastGpol::initDataStructures(void) {
  ...
  for (int i = 0; i < numAtoms; i++) {
    ...
    if (atoms[i].groupR > maxGroupR)
      maxGroupR = atoms[i].groupR;
  }
  ...
  countOctreeNodesAndSortAtoms(atoms, 0, numAtoms - 1, minRad * minRad,
                               maxLeafAtoms, atomsT, &numOctreeNodes);
  ...
  constructOctree(octree, 0, qSum, maxGroupR + 1, minRad, maxLeafAtoms, atoms,
                  &freeID);
...
void fastGpol::initJobServer(std::vector<JOB> &leafJobs,
...
inline float fastGpol::vectorGB(float rij2f, v4sf Ri, v4sf qi, v4sf Rj,
                                v4sf qj) {
  v4sf rij2, RiRj, RiRjx4, qiqj, val;
  V4SF val2;
...
void fastGpol::countOctreeNodesAndSortAtoms(ATOM *atom, int atomStartID,
                                            int atomEndID, double minRad2,
                                            int maxLeafAtoms, ATOM *atomT,
                                            int *numNodes) {
  double minX = atom[atomStartID].x, minY = atom[atomStartID].y,
         minZ = atom[atomStartID].z;
  double maxX = atom[atomStartID].x, maxY = atom[atomStartID].y,
         maxZ = atom[atomStartID].z;
  ...
  double r2 =
      (atom[atomStartID].x - centerX) * (atom[atomStartID].x - centerX) +
  ...
  *numNodes = 1;
  ...
  if ((atomEndID - atomStartID + 1 > maxLeafAtoms) && (r2 > minRad2)) {
    ...
    for (int i = 1; i < 8; i++)
      atomCurIndex[i] = atomStartIndex[i] =
          atomStartIndex[i - 1] + atomCount[i - 1];
    ...
    for (int i = 0; i < 8; i++)
      if (atomCount[i] > 0) {
        ...
        countOctreeNodesAndSortAtoms(atom, atomStartIndex[i],
                                     atomStartIndex[i] + atomCount[i] - 1,
                                     minRad2, maxLeafAtoms, atomT, &numNodesT);
...
void fastGpol::constructOctree(OCTREE_NODE *octree, int nodeID, double *qSum,
  ...
  double r2 = (atom[atomStartID].x - octree[nodeID].cx) *
                  (atom[atomStartID].x - octree[nodeID].cx) +
              (atom[atomStartID].y - octree[nodeID].cy) *
                  (atom[atomStartID].y - octree[nodeID].cy) +
              (atom[atomStartID].z - octree[nodeID].cz) *
                  (atom[atomStartID].z - octree[nodeID].cz);

  for (int i = atomStartID + 1; i <= atomEndID; i++) {
    double r2T =
        (atom[i].x - octree[nodeID].cx) * (atom[i].x - octree[nodeID].cx) +
  ...
  if ((atomEndID - atomStartID + 1 <= maxLeafAtoms) ||
      (octree[nodeID].cr <= minRad)) {
    ...
    for (int i = 0; i < 8; i++)
      if (atomCount[i] > 0) {
        ...
        constructOctree(octree, j, qSum, numGroupR, minRad, maxLeafAtoms, atom,
                        freeID);

        for (int k = 0; k < numGroupR; k++)
          qSum[qPtr + k] += qSum[numGroupR * j + k];
      } else
        octree[nodeID].cPtr[i] = -1;
  }
}

void fastGpol::generateJobs(OCTREE_NODE *octree, int nodeI, int nodeJ,
                            double eps, std::vector<JOB> &leafJobs,
                            std::vector<JOB> &nonLeafJobs) {
  if (octree[nodeI].leaf && octree[nodeJ].leaf) {
    ...
    double rij2 = (octree[nodeI].cx - octree[nodeJ].cx) *
                      (octree[nodeI].cx - octree[nodeJ].cx) +
                  (octree[nodeI].cy - octree[nodeJ].cy) *
                      (octree[nodeI].cy - octree[nodeJ].cy) +
                  (octree[nodeI].cz - octree[nodeJ].cz) *
                      (octree[nodeI].cz - octree[nodeJ].cz);
...
void fastGpol::recursiveFastGpol(OCTREE_NODE *octree, int nodeI, int nodeJ,
                                 double *qSum, double *approxR, int numGroupR,
                                 ATOM *atom, double eps, double *Gpol) {
  double sumRad = octree[nodeI].cr + octree[nodeJ].cr;
...
//              if ( i != j )
#ifdef USE_SSE
      {
        ...
        if (++k == 4) {
          *Gpol += vectorGB(xi.v, yi.v, zi.v, Ri.v, qi.v, xj.v, yj.v, zj.v,
                            Rj.v, qj.v);
          k = 0;
        }
      }
#else
      {
        ...
        if (rij2 <
            0.5 * 0.5 * (atom[i].r + atom[j].r) * (atom[i].r + atom[j].r))
        ...
        if (useApproxMath)
          *Gpol +=
              (qiqj *
               invSqrt((float)(rij2 +
                               RiRj * fastExp((float)(-rij2 / (4 * RiRj))))));
        else
          *Gpol += (qiqj / sqrt(rij2 + RiRj * exp(-rij2 / (4 * RiRj))));
      }
#endif
    ...
    while (k--) {
      ...
      if (useApproxMath)
        *Gpol +=
            (qiqj *
             invSqrt(
                 (float)(rij2 + RiRj * fastExp((float)(-rij2 / (4 * RiRj))))));
      else
        *Gpol += (qiqj / sqrt(rij2 + RiRj * exp(-rij2 / (4 * RiRj))));
    }
#endif
    ...
    if (rij > (2 / eps) * sumRad) //( rij2 > eps2 * sumRad2 )
    {
      ...
      if (!octree[nodeI].leaf && !octree[nodeJ].leaf) {
        for (int i = 0; i < 8; i++)
          if (octree[nodeI].cPtr[i] >= 0)
            for (int j = 0; j < 8; j++)
              if (octree[nodeJ].cPtr[j] >= 0) {
                GpolT = 0;
                recursiveFastGpol(octree, octree[nodeI].cPtr[i],
                                  octree[nodeJ].cPtr[j], qSum, approxR,
                                  numGroupR, atom, eps, &GpolT);
                *Gpol += GpolT;
              }
      } else {
        if (octree[nodeJ].leaf) {
        ...
        for (int j = 0; j < 8; j++)
          if (octree[nodeJ].cPtr[j] >= 0) {
            GpolT = 0;
            recursiveFastGpol(octree, nodeI, octree[nodeJ].cPtr[j], qSum,
                              approxR, numGroupR, atom, eps, &GpolT);
            *Gpol += GpolT;
          }
      }
    }
...
void fastGpol::computeThreadedFastGpol(int threadID, OCTREE_NODE *octree,
                                       double *qSum, double *approxR,
                                       int numGroupR, ATOM *atom, double eps,
                                       double *Gpol) {
  int nodeI, nodeJ;

  while (nextJob(nodeI, nodeJ))
    recursiveFastGpol(octree, nodeI, nodeJ, qSum, approxR, numGroupR, atom, eps,
                      Gpol);
}
...
void fastGpol::computeFastGpol(double *Gpol) {
  ...
  while (leafJobs.size() + nonLeafJobs.size() < numThreads * numThreads) {
    ...
    while (k--) {
      job = sourceJobs.back();
      generateJobs(octree, job.nodeI, job.nodeJ, epsilon, leafJobs,
                   nonLeafJobs);
      sourceJobs.pop_back();
    }
  }
