...
fastDispE::fastDispE(char *paramFile, int numStaticAtoms,
                     double *staticAtomsPQR, int numMovingAtoms,
                     double *movingAtomsPQR, int numThreads,
                     int numThreadsPerThread) {
  if (!getParamsFromFile(&params, paramFile))
    exit(1);
  ...
  getAtomsQuadsAndDispE(params.staticMoleculeQUAD, numStaticAtoms,
                        staticAtomsPQR, &staticMol);

  getAtomsQuadsAndDispE(params.movingMoleculeQUAD, numMovingAtoms,
                        movingAtomsPQR, &movingMol);

  numAtoms = staticMol.numAtoms + movingMol.numAtoms;
  numQPoints = staticMol.numQPoints + movingMol.numQPoints;

  atoms = (double *)malloc(params.numThreads *
                           (staticMol.numAtoms + movingMol.numAtoms) * 5 *
                           sizeof(double));
  qPoints = (double *)malloc(params.numThreads *
                             (staticMol.numQPoints + movingMol.numQPoints) * 7 *
                             sizeof(double));
...
fastDispE::~fastDispE() { freeMemory(); }

void fastDispE::getAtomsQuadsAndDispE(char *quadFile, int numAtoms,
                                      double *atomsPQR, MOLECULE_INFO *mol) {
...
bool fastDispE::getParamsFromFile(PARAMS_IN *p, char *paramFile) {
  ...
  while (fgets(s, 1999, fp) != NULL) {
    ...
    if (!strcasecmp(key, (char *)"staticMoleculeQUAD"))
    ...
    //               if ( v < 1 )
    //                 {
    //                   printError( (char *)"%s must be a positive integer!",
    //                   key ); fclose( fp ); return false;
    //                 }
    //
    //               p->numThreads = v;
    //             }
...
void fastDispE::transformAndCopyMovingMolecule(int threadID, double *transMat) {
  ...
  for (int i = 0; i < movingMol.numQPoints; i++, j++) {
    double x = movingMol.qPoints[7 * i + 0], y = movingMol.qPoints[7 * i + 1],
           z = movingMol.qPoints[7 * i + 2], nx = movingMol.qPoints[7 * i + 3],
           ny = movingMol.qPoints[7 * i + 4], nz = movingMol.qPoints[7 * i + 5],
...
double fastDispE::getDispE(int threadID, double *transMat) {
  ...
  fastGB::fastBornRadius fastBR(numQPoints, qPoints + threadID * numQPoints * 7,
                                numAtoms, atoms + threadID * numAtoms * 5,
                                false);
...
double fastDispE::getDelDispE(int threadID, double *transMat) {
  ...
  double delDispE =
      fastBR.getDispersionEnergy() - (staticMol.dispE + movingMol.dispE);
