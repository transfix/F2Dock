...
void fastLJ::printCurrentSettings(void) {
  ...
  printf("\tminRadius = %lf, maxLeafSize = %d, minInterAtomDist = %lf, epsilon "
         "= %lf, numThreads = %d\n",
...
fastLJ::fastLJ(int numStaticAtoms, double *stAtomsXYZ, char *stAtomsType,
               int numMovingAtoms, double *mvAtomsXYZ, char *mvAtomsType,
               int nThreads, bool printStat) {
  setDefaults();
  ...
  if (!copyAtomsFromArray(numStaticAtoms, stAtomsXYZ, stAtomsType, nStaticAtoms,
                          staticAtoms) ||
      !copyAtomsFromArray(numMovingAtoms, mvAtomsXYZ, mvAtomsType, nMovingAtoms,
                          movingAtomsOrig)) {
    freeMemory();
    exit(1);
  }
  ...
  for (int i = 0; i < numAtomType; i++) {
    movingAtoms[i] =
        (ATOM *)malloc(numThreads * nMovingAtoms[i] * sizeof(ATOM));
...
fastLJ::fastLJ(char *staticAtomsFile, char *movingAtomsFile, int nThreads,
               bool printStat) {
  setDefaults();
...
bool fastLJ::setMinRadius(double minRad) {
  ...
  for (int k = 0; k < numAtomType; k++) {
    if (staticAtoms[k] != NULL)
      staticAtomsLoaded = true;
    if (movingAtomsOrig[k] != NULL)
      movingAtomsLoaded = true;
  }
...
void fastLJ::transformMovingAtoms(int threadID, double *transMat) {
  for (int k = 0; k < numAtomType; k++)
    for (int i = 0, j = threadID * nMovingAtoms[k]; i < nMovingAtoms[k]; i++) {
      transform(movingAtomsOrig[k][i].x, movingAtomsOrig[k][i].y,
                movingAtomsOrig[k][i].z, transMat, movingAtoms[k][j + i].x,
                movingAtoms[k][j + i].y, movingAtoms[k][j + i].z);
...
bool fastLJ::getParamsFromFile(char *paramFile, char **staticAtomsFile,
                               char **movingAtomsFile) {
  char s[2000];
  ...
  while (fgets(s, 1999, fp) != NULL) {
    ...
    if (!strcasecmp(key, "staticMoleculePQR") ||
        !strcasecmp(key, "staticMoleculePDB"))
      (*staticAtomsFile) = strdup(val);
    else if (!strcasecmp(key, "movingMoleculePQR") ||
             !strcasecmp(key, "movingMoleculePDB"))
      (*movingAtomsFile) = strdup(val);
      ...
      if (v <= 0)
        printError(std::format("{} must be a positive float!", key));
      else
        setMinInterAtomDist(v);
      ...
      double v = atof(val);
      if (v <= 0)
        printError(std::format("{} must be a positive real value!", key));
      else
        vdWEqmRadScale = v;
  ...
  if (((*staticAtomsFile) == NULL) || ((*movingAtomsFile) == NULL)) {
    printError(std::format("Missing PDB/PQR file name for the {} molecule!",
                           ((*staticAtomsFile) == NULL) ? "static" : "moving"));
    freeMem((*staticAtomsFile));
    freeMem((*movingAtomsFile));
    return false;
  }
...
bool fastLJ::readAtomsFromPQR(char *atomsFile, int *numAtoms, ATOM **atms) {
  ...
  if (printStatus)
    printf("done ( %lf sec, read %d atoms )\n", endT - startT, nAtoms);
...
bool fastLJ::readAtomsFromFile(char *atomsFile, int *numAtoms, ATOM **atms) {
  ...
  else if ((l > 3) && (atomsFile[l - 4] == '.') &&
           (toupper(atomsFile[l - 3]) == 'P') &&
           (toupper(atomsFile[l - 2]) == 'Q') &&
           (toupper(atomsFile[l - 1]) == 'R'))
...
bool fastLJ::copyAtomsFromArray(int numAtomsSrc, double *atmsSrcXYZ,
                                char *atmsSrcType, int *numAtomsDest,
                                ATOM **atmsDest) {
  if (printStatus)
  ...
  if (printStatus)
    printf("done ( %lf sec, copied %d atoms )\n", endT - startT, numAtomsSrc);

  return true;
}

void fastLJ::countAtomsOctreeNodesAndSortAtoms(ATOM **atoms, int *atomsStartID,
                                               int *atomsEndID, ATOM **atomsT,
                                               int *numNodes) {

  // if (*atomsStartID > *atomsEndID) return;
  // for ( int k = 0; k < numAtomType; k++ )
  //   cout << "Enter " << k << " " << atomsStartID[k] << " " << atomsEndID[k]
  //   << endl;
  // cout << "NN " << *numNodes << endl;

  // double minX = atoms[ 0 ][ atomsStartID[ 0 ] ].x, minY = atoms[ 0 ][
  // atomsStartID[ 0 ] ].y, minZ = atoms[ 0 ][ atomsStartID[ 0 ] ].z; double
  // maxX = atoms[ 0 ][ atomsStartID[ 0 ] ].x, maxY = atoms[ 0 ][ atomsStartID[
  // 0 ] ].y, maxZ = atoms[ 0 ][ atomsStartID[ 0 ] ].z;
  ...
  //
  // double r2 = ( atoms[ 0 ][ atomsStartID[ 0 ] ].x - cx ) * ( atoms[ 0 ][
  // atomsStartID[ 0 ] ].x - cx )
  //          + ( atoms[ 0 ][ atomsStartID[ 0 ] ].y - cy ) * ( atoms[ 0 ][
  //          atomsStartID[ 0 ] ].y - cy )
  //          + ( atoms[ 0 ][ atomsStartID[ 0 ] ].z - cz ) * ( atoms[ 0 ][
  //          atomsStartID[ 0 ] ].z - cz );
  double r2 = 0.0;
  ...
  *numNodes = 1;
  ...
  // cout << "arand " << numAtoms << " " << cr << " " << maxLeafSize << " " <<
  // minRadius << endl;

  if ((numAtoms > maxLeafSize) && (cr > minRadius)) {
    int atomsCount[8][numAtomType],
        atomsCountAllTypes[8] = {0, 0, 0, 0, 0, 0, 0, 0};
    ...
    for (int k = 0; k < numAtomType; k++) {
      ...
      for (int i = 1; i < 8; i++)
        atomsCurIndex[i][k] = atomsStartIndex[i][k] =
            atomsStartIndex[i - 1][k] + atomsCount[i - 1][k];
    ...
    for (int i = 0; i < 8; i++)
      if (atomsCountAllTypes[i] > 0) {
        ...
        countAtomsOctreeNodesAndSortAtoms(atoms, atomsStartIndex[i],
                                          atomsEndIndex[i], atomsT, &numNodesT);
...
int fastLJ::constructAtomsOctree(int *atomsStartID, int *atomsEndID,
                                 ATOM **atoms, ATOMS_OCTREE_NODE *atomsOctree) {
  ...
  if ((numAtoms <= maxLeafSize) || (cr <= minRadius))
    ...
    for (int i = 0; i < 8; i++)
      if (atomsCountAllTypes[i] > 0) {
        ...
        int j = constructAtomsOctree(atomsStartIndex[i], atomsEndIndex[i],
                                     atoms, atomsOctree);
...
bool fastLJ::buildStaticAtomsOctree(void) {
  ...
  countAtomsOctreeNodesAndSortAtoms(staticAtoms, atomsStartIndex, atomsEndIndex,
                                    atomsT, &numStaticAtomsOctreeNodes);
  ...
  atomsOctreeT = (ATOMS_OCTREE_NODE *)malloc(numStaticAtomsOctreeNodes *
                                             sizeof(ATOMS_OCTREE_NODE));

  if (atomsOctreeT == NULL) {
    printError(std::format(
        "Unable to {} static atoms octree - memory allocation failed!",
        (staticAtomsOctreeBuilt) ? "rebuild" : "build"));
    if (!staticAtomsOctreeBuilt)
      exit(1);
    return false;
  }
  ...
  staticAtomsOctreeRoot = constructAtomsOctree(atomsStartIndex, atomsEndIndex,
                                               staticAtoms, staticAtomsOctree);
...
bool fastLJ::buildMovingAtomsOctree(void) {
  ...
  countAtomsOctreeNodesAndSortAtoms(movingAtomsOrig, atomsStartIndex,
                                    atomsEndIndex, atomsT,
                                    &numMovingAtomsOctreeNodes);
  ...
  atomsOctreeT = (ATOMS_OCTREE_NODE *)malloc(numMovingAtomsOctreeNodes *
                                             sizeof(ATOMS_OCTREE_NODE));

  if (atomsOctreeT == NULL) {
    printError(std::format(
        "Unable to {} moving atoms octree - memory allocation failed!",
        (movingAtomsOctreeBuilt) ? "rebuild" : "build"));
    if (!movingAtomsOctreeBuilt)
      exit(1);
    return false;
  }
  ...
  movingAtomsOctreeRoot = constructAtomsOctree(
      atomsStartIndex, atomsEndIndex, movingAtomsOrig, movingAtomsOctree);
...
inline float fastLJ::vectorLJ(v4sf xi, v4sf yi, v4sf zi, v4sf xj, v4sf yj,
                              v4sf zj, v4sf Aij, v4sf Bij) {
  v4sf d2, d6, v;
  V4SF val;
...
void fastLJ::approximatePotential(int threadID, double *transMat, int nodeS,
                                  int nodeM, double *LJPot) {
  double sumRad = staticAtomsOctree[nodeS].cr + movingAtomsOctree[nodeM].cr;
  double sumRad2 = sumRad * sumRad;
  double cxM, cyM, czM;

  transform(movingAtomsOctree[nodeM].cx, movingAtomsOctree[nodeM].cy,
            movingAtomsOctree[nodeM].cz, transMat, cxM, cyM, czM);
  ...
  if (/*( d2 > 4 * sumRad2 ) && */ (d2 > (sumRad2 / (epsilon * epsilon))))
    farEnough = true;
  ...
  if (farEnough) {
    ...
    for (int k = 0; k < numAtomType; k++)
      for (int l = 0; l < numAtomType; l++)
        pot += movingAtomsOctree[nodeM].numAtoms[k] *
               staticAtomsOctree[nodeS].numAtoms[l] *
               ((A[k][l] * d12) - (B[k][l] * d6));
  } else {
    if (staticAtomsOctree[nodeS].leaf && movingAtomsOctree[nodeM].leaf) {
      ...
      for (int k = 0; k < numAtomType; k++)
        for (int l = 0; l < numAtomType; l++) {
          ...
          for (int i = movingAtomsOctree[nodeM].atomsStartID[k];
               i <= movingAtomsOctree[nodeM].atomsEndID[k]; i++) {
            double mx = movingAtoms[k][s + i].x, my = movingAtoms[k][s + i].y,
                   mz = movingAtoms[k][s + i].z;
#ifdef USE_SSE
            if (useSSEFunctions) {
              for (int j = staticAtomsOctree[nodeS].atomsStartID[l];
                   j <= staticAtomsOctree[nodeS].atomsEndID[l]; j++) {
                xi.f[t] = mx;
                yi.f[t] = my;
                zi.f[t] = mz;
                ...
                if (++t == 4) {
                  pot += vectorLJ(xi.v, yi.v, zi.v, xj.v, yj.v, zj.v, Aij.v,
                                  Bij.v);
                  t = 0;
                }
              }
        ...
        if (staticAtomsOctree[nodeS].cPtr[i] >= 0)
          for (int j = 0; j < 8; j++)
            if (movingAtomsOctree[nodeM].cPtr[j] >= 0) {
              ...
              approximatePotential(threadID, transMat,
                                   staticAtomsOctree[nodeS].cPtr[i],
                                   movingAtomsOctree[nodeM].cPtr[j], &ljPot);
      ...
      for (int i = 0; i < 8; i++)
        if (staticAtomsOctree[nodeS].cPtr[i] >= 0) {
          ...
          approximatePotential(threadID, transMat,
                               staticAtomsOctree[nodeS].cPtr[i], nodeM, &ljPot);
      ...
      for (int j = 0; j < 8; j++)
        if (movingAtomsOctree[nodeM].cPtr[j] >= 0) {
          ...
          approximatePotential(threadID, transMat, nodeS,
                               movingAtomsOctree[nodeM].cPtr[j], &ljPot);
...
bool fastLJ::computePotential(int threadID, double *transMat, double *LJPot) {
  ...
  approximatePotential(threadID, transMat, staticAtomsOctreeRoot,
                       movingAtomsOctreeRoot, LJPot);
...
void fastLJ::computePotentialNaively(int threadID, double *transMat,
                                     double *LJPot) {
  if (printStatus)
