...
hbondFilter::hbondFilter(char *staticPQR, char *movingPQR, char *staticPSF,
...
bool hbondFilter::initializeFilter() {
  // building octrees
  if (!build_octree(&staticOctree,
                    max(60, ags1->nblst->npairs / staticAG->natoms), 6, 1.0,
                    staticAG)) {
    printf("Failed to build octree!\n");
    return false;
  }

  if (!build_octree(&movingOctree,
                    max(60, ags2->nblst->npairs / movingAG->natoms), 6, 1.0,
                    movingAG)) {
    printf("Failed to build octree!\n");
    return false;
  }
  ...
  complex_eng_params.eps = static_eng_params.eps = moving_eng_params.eps =
      eface;
  complex_eng_params.hdist_cutoff = static_eng_params.hdist_cutoff =
      moving_eng_params.hdist_cutoff = MAX_R;
  complex_eng_params.fixed_cull = static_eng_params.fixed_cull =
      moving_eng_params.fixed_cull = 1;
  complex_eng_params.trans = static_eng_params.trans = moving_eng_params.trans =
      NULL;
  complex_eng_params.engcat = static_eng_params.engcat =
      moving_eng_params.engcat = NULL;
  ...
  staticHbondEnergy = octree_accumulation_excluding_far(
      &staticOctree, &staticOctree, MAX_R, MAX_R, 1, NULL,
      (void *)(&static_eng_params), hbondeng_octree_single_mol);
  movingHbondEnergy = octree_accumulation_excluding_far(
      &movingOctree, &movingOctree, MAX_R, MAX_R, 1, NULL,
      (void *)(&moving_eng_params), hbondeng_octree_single_mol);

  return true;
}

bool hbondFilter::getEnergy(double *trans, double *en) {
  complexHbondEnergy = octree_accumulation_excluding_far(
      &staticOctree, &movingOctree, MAX_R, MAX_R, 1, trans,
      (void *)(&complex_eng_params), hbondeng_octree_single_mol);
