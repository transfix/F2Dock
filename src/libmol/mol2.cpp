  ...
  while (fgets(buffer, linesize, fp) != NULL) {
    if (strstr(buffer, "<TRIPOS>MOLECULE") != NULL) {
      ...
      if (strcmp(fname1, fname2)) {
        print_error("%s was generated from %s, not %s!", mol2file, fname2,
                    fname1);
        return 0;
      }
      ...
      if (nat != ag->natoms) {
        print_error("%s has %d atoms instead of %d atoms!", mol2file, nat,
                    ag->natoms);
        return 0;
      }
    ...
    if (strstr(buffer, "<TRIPOS>ATOM") != NULL) {
      ...
      for (int i = 0; i < ag->natoms; i++) {
        ...
        if (sscanf(buffer, "%d %s %lf %lf %lf %s", &atom_id, atom_name, &x, &y,
                   &z, atom_type) != 6) {
          print_error("Failed to read ATOM record from %s!", mol2file);
          return 0;
        }

        if ((x != ag->atoms[i].X) || (y != ag->atoms[i].Y) ||
            (z != ag->atoms[i].Z)) {
          print_error(
              "Coordinates of atom %d in %s do not match with those in %s!",
              atom_id, mol2file, pdbfile);
          return 0;
        }
        ...
        if (atom_type[1] == '.') {
          ...
          case '4':
          case '3':
            ag->atoms[i].hybridization = SP3_HYBRID;
            break;

          case '2':
            ag->atoms[i].hybridization = SP2_HYBRID;
            break;

          case '1':
            ag->atoms[i].hybridization = SP1_HYBRID;
            break;

          case 'a':
            if (atom_type[3] == 'r')
              ag->atoms[i].hybridization = RING_HYBRID;
            break;

          case 'c':
            if ((atom_type[3] == 'o') && (atom_type[4] == '2'))
              ag->atoms[i].hybridization = SP2_HYBRID;
            break;
          }
        }
        ...
        //               printf( "%d %s ( %d, %d )\n", i + 1, atom_type,
        //               ag->atoms[ i ].hprop, ag->atoms[ i ].hybridization );
      }
    }
  }
