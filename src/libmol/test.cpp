...
void print_usage(char *prog) {
  ...
  printf("\t--fpdb filename     : fixed part of the input PDB ( optional, "
         "default = filename ( -fixed.pdb ) derived from input PDB )\n");
  printf("\t--nofixed           : input PDB has no fixed part ( optional, "
         "default = has fixed part ( see --fpdb ) )\n");
  printf("\t--psf filename      : CHARMM PSF file for the input PDB ( "
         "optional, default = filename ( .psf ) derived from input PDB )\n");
  printf("\t--mol2 filename     : MOL2 file for the input PDB ( optional, for "
         "--hbond only, default = filename ( .mol2 ) derived from input PDB "
         ")\n\n");

  printf(
      "\t--outn filename     : output PDB file from nblists minimization ( "
      "optional, default = filename ( -outn.pdb ) derived from input PDB )\n");
  printf("\t--outo filename     : output PDB file from octree minimization ( "
         "optional, default = filename ( -outo.pdb ) derived from input PDB "
         ")\n\n");
  ...
  printf("\t--iter number       : maximum number of iterations of the "
         "minimizer ( optional, default = %d )\n\n",
         MAX_ITER);

  printf("\t--nonblist          : no nblists based minimization ( optional, "
         "default = perform nblists based minimization )\n");
  printf("\t--nooct             : no octree based minimization ( optional, "
         "default = perform octree based minimization )\n\n");

  printf("\t--vdw               : use van der Waals energy ( optional, default "
         "= do not use vdw )\n");
  printf("\t--elec              : use electrostatics ( optional, default = do "
         "not use electrostatics )\n");
  printf("\t--hbond             : use hbond energy ( optional, default = do "
         "not use hbond )\n\n");

  printf("\t--vdwcut value(s)   : distance cutoff(s) ( in angstroms ) for van "
         "der Waals energy ( optional, default = %.2lf )\n",
         DEFAULT_VDW_DIST_CUTOFF);
  printf("\t--eleccut value(s)  : distance cutoff(s) ( in angstroms ) for "
         "electrostatics ( optional, default = %.2lf )\n",
         DEFAULT_ELEC_DIST_CUTOFF);
  printf("\t--hbondcut value(s) : distance cutoff(s) ( in angstroms ) for "
         "hbond energy ( optional, default = %.2lf )\n\n",
         DEFAULT_HBOND_DIST_CUTOFF);

  printf("\t--vdwaprx value     : distance cutoff approximation factor ( in ( "
         "0, 1 ] ) for van der Waals energy ( optional, default = 1.0 )\n");
  printf("\t--elecaprx value    : distance cutoff approximation factor ( in ( "
         "0, 1 ] ) for electrostatics ( optional, default = 1.0 )\n\n");
...
int file_exists(char *fname, const char *msg) {
  ...
  if (fp == NULL) {
    print_error("Failed to open %s ( please specify %s filename )!", fname,
                msg);
    return 0;
  } else {
    fclose(fp);
...
int read_command_line(int argc, char *argv[], char **pdbFile,
                      char **pdbFixedFile, char **mol2File, char **psfFile,
                      char **outnFile, char **outoFile, char **prmFile,
                      char **rtfFile, char **aprmFile, int *maxIter, int *noOct,
                      int *noNblist, int *useVdw, int *useElec, int *useHbond,
                      double **dcVdw, double **dcElec, double **dcHbond,
                      int *ndc, double *aprxVdw, double *aprxElec) {
  if (argc <= 1) {
    print_usage(argv[0]);
    exit(0);
  }

  (*pdbFile) = (*pdbFixedFile) = (*mol2File) = (*psfFile) = (*outnFile) =
      (*outoFile) = (*prmFile) = (*rtfFile) = (*aprmFile) = NULL;
  *maxIter = MAX_ITER;
  *noOct = *noNblist = 0;
  ...
  *aprxVdw = *aprxElec = 1.0;
  ...
  for (i = 1; i < argc;) {
    ...
    if (!strcasecmp(argv[i], "--vdwcut")) {
      if ((i + 1 >= argc) || (argv[i + 1][0] == '-')) {
        print_error(
            "Missing distance cutoff for vdw ( specify --vdwcut value(s) )!");
        return 0;
      }
      ...
      while ((i < argc) && (argv[i][0] != '-')) {
        if (ndcVdw)
          (*dcVdw) =
              (double *)_mol_realloc(*dcVdw, (ndcVdw + 1) * sizeof(double));
        else
          (*dcVdw) = (double *)_mol_malloc((ndcVdw + 1) * sizeof(double));

        (*dcVdw)[ndcVdw] = atof(argv[i++]);

        if ((*dcVdw)[ndcVdw] <= 0) {
          print_error("Invalid distance cutoff for vdw ( %lf )!",
                      (*dcVdw)[ndcVdw]);
          return 0;
        }
    ...
    if (!strcasecmp(argv[i], "--eleccut")) {
      if ((i + 1 >= argc) || (argv[i + 1][0] == '-')) {
        print_error("Missing distance cutoff for electrostatics ( specify "
                    "--eleccut value(s) )!");
        return 0;
      }
      ...
      while ((i < argc) && (argv[i][0] != '-')) {
        if (ndcElec)
          (*dcElec) =
              (double *)_mol_realloc(*dcElec, (ndcElec + 1) * sizeof(double));
        else
          (*dcElec) = (double *)_mol_malloc((ndcElec + 1) * sizeof(double));

        (*dcElec)[ndcElec] = atof(argv[i++]);

        if ((*dcElec)[ndcElec] <= 0) {
          print_error("Invalid distance cutoff for electrostatics ( %lf )!",
                      (*dcElec)[ndcElec]);
          return 0;
        }
    ...
    if (!strcasecmp(argv[i], "--hbondcut")) {
      if ((i + 1 >= argc) || (argv[i + 1][0] == '-')) {
        print_error("Missing distance cutoff for hbond ( specify --hbondcut "
                    "value(s) )!");
        return 0;
      }
      ...
      while ((i < argc) && (argv[i][0] != '-')) {
        if (ndcHbond)
          (*dcHbond) =
              (double *)_mol_realloc(*dcHbond, (ndcHbond + 1) * sizeof(double));
        else
          (*dcHbond) = (double *)_mol_malloc((ndcHbond + 1) * sizeof(double));
    ...
    if (!strcasecmp(argv[i], "--vdwaprx")) {
      if ((i + 1 >= argc) || (argv[i + 1][0] == '-')) {
        print_error("Missing distance cutoff approximation factor for vdw ( "
                    "specify --vdwaprx value )!");
        return 0;
      }
      ...
      *aprxVdw = atof(argv[i + 1]);
      ...
      if ((*aprxVdw <= 0) || (*aprxVdw > 1)) {
        print_error(
            "Invalid distance cutoff approximation factor for vdw ( %lf )!",
            *aprxVdw);
        return 0;
      }
    ...
    if (!strcasecmp(argv[i], "--elecaprx")) {
      if ((i + 1 >= argc) || (argv[i + 1][0] == '-')) {
        print_error("Missing distance cutoff approximation factor for "
                    "electrostatics ( specify --vdwaprx value )!");
        return 0;
      }
      ...
      *aprxElec = atof(argv[i + 1]);
      ...
      if ((*aprxElec <= 0) || (*aprxElec > 1)) {
        print_error("Invalid distance cutoff approximation factor for "
                    "electrostatics ( %lf )!",
                    *aprxElec);
        return 0;
      }
    ...
    if (!strcasecmp(argv[i], "--fpdb")) {
      if ((i + 1 >= argc) || (argv[i + 1][0] == '-')) {
        print_error(
            "Missing fixed PDB file name ( specify --fpdb pdb-file-name )!");
        return 0;
      }
    ...
    if (!strcasecmp(argv[i], "--outn")) {
      if ((i + 1 >= argc) || (argv[i + 1][0] == '-')) {
        print_error("Missing output ( from nblists minimization ) PDB file "
                    "name ( specify --outn pdb-file-name )!");
        return 0;
      }
    ...
    if (!strcasecmp(argv[i], "--outo")) {
      if ((i + 1 >= argc) || (argv[i + 1][0] == '-')) {
        print_error("Missing output ( from octree minimization ) PDB file name "
                    "( specify --outo pdb-file-name )!");
        return 0;
      }
    ...
    if (!strcasecmp(argv[i], "--rtf")) {
      if ((i + 1 >= argc) || (argv[i + 1][0] == '-')) {
        print_error(
            "Missing output RTF file name ( specify --rtf rtf-file-name )!");
        return 0;
      }
    ...
    if (!strcasecmp(argv[i], "--aprm")) {
      if ((i + 1 >= argc) || (argv[i + 1][0] == '-')) {
        print_error(
            "Missing output APRM file name ( specify --aprm aprm-file-name )!");
        return 0;
      }
  ...
  if ((*prmFile) == NULL)
    (*prmFile) = strdup(DEFAULT_PRM);

  if ((*rtfFile) == NULL)
    (*rtfFile) = strdup(DEFAULT_RTF);

  if ((*aprmFile) == NULL)
    (*aprmFile) = strdup(DEFAULT_APRM);
  ...
  *ndc = ndcVdw;
  ...
  if (ndcVdw < *ndc) {
    if (ndcVdw)
      (*dcVdw) = (double *)_mol_realloc(*dcVdw, (*ndc) * sizeof(double));
    else
      (*dcVdw) = (double *)_mol_malloc((*ndc) * sizeof(double));
  ...
  if (ndcElec < *ndc) {
    if (ndcElec)
      (*dcElec) = (double *)_mol_realloc(*dcElec, (*ndc) * sizeof(double));
    else
      (*dcElec) = (double *)_mol_malloc((*ndc) * sizeof(double));
  ...
  if (ndcHbond < *ndc) {
    if (ndcHbond)
      (*dcHbond) = (double *)_mol_realloc(*dcHbond, (*ndc) * sizeof(double));
    else
      (*dcHbond) = (double *)_mol_malloc((*ndc) * sizeof(double));
  ...
  if (!file_exists(*pdbFile, "--pdb") ||
      (!nofixed && !file_exists(*pdbFixedFile, "--fpdb")) ||
      !file_exists(*psfFile, "--psf") ||
      (*useHbond && !file_exists(*mol2File, "--mol2")) ||
      !file_exists(*prmFile, "--prm") || !file_exists(*rtfFile, "--rtf") ||
      !file_exists(*aprmFile, "--aprm"))
    return 0;
...
int size_of_nblist(struct atomgrp *ag, struct agsetup *ags) {
  ...
  s += ag->natoms * (3 * sizeof(float) + 2 * sizeof(int) + sizeof(int *)) +
       nblst->npairs * sizeof(int);
...
void read_fix(char *ffile, int *nfix, int **fix) {
  ...
  *nfix = 0;
  ...
  if (ffile != NULL) {
    ...
    if (fp == NULL) {
      print_error("Failed to open fixed PDB %s ( use the --nofixed option if "
                  "the molecule has no fixed part )!",
                  ffile);
      exit(0);
    }
...
void my_en_grad(int n, double *inp, void *prms, double *en, double *grad) {
  ...
  *en = 0;
  ...
  if (octpar == NULL) {
    if (prm->useVdw)
      vdweng(mag, en, mags->nblst);

    if (prm->useElec)
      eleng(mag, prm->eface, en, mags->nblst);
    ...
    if (prm->useVdw)
      *en += octree_accumulation_excluding_far(
          octpar->octree_static, octpar->octree_moving, prm->dcVdw,
          prm->aprxdcVdw, octpar->fixed_cull, octpar->trans,
          octpar->proc_func_params, vdweng_octree_single_mol);
    if (prm->useElec)
      *en += octree_accumulation_excluding_far(
          octpar->octree_static, octpar->octree_moving, prm->dcElec,
          prm->aprxdcElec, octpar->fixed_cull, octpar->trans,
          octpar->proc_func_params, eleng_octree_single_mol);

    if (prm->useHbond)
      *en += octree_accumulation_excluding_far(
          octpar->octree_static, octpar->octree_moving, prm->dcHbond,
          prm->dcHbond, octpar->fixed_cull, octpar->trans,
...
int main(int argc, char *argv[]) {
  char *pdbFile, *pdbFixedFile, *psfFile, *mol2File, *outnFile, *outoFile,
      *prmFile, *rtfFile, *aprmFile;
  int maxIter;
  int noOct, noNblist;
  int useVdw, useElec, useHbond;
  double *dcVdw, *dcElec, *dcHbond;
  ...
  if (!read_command_line(argc, argv, &pdbFile, &pdbFixedFile, &mol2File,
                         &psfFile, &outnFile, &outoFile, &prmFile, &rtfFile,
                         &aprmFile, &maxIter, &noOct, &noNblist, &useVdw,
                         &useElec, &useHbond, &dcVdw, &dcElec, &dcHbond, &ndc,
                         &aprxVdw, &aprxElec))
    return 1;
  ...
  if (!useVdw && !useElec && !useHbond) {
    print_error("Nothing to do ( at least one of --vdw, --elec and --hbond "
                "must be set )!");
    return 1;
  }

  printf("Processing %s... ", aprmFile);
  fflush(stdout);
  struct prm *aprm = read_prm(aprmFile, "0.0.6"); // read in atomic parameters
  printf("done\n");

  printf("Processing %s... ", pdbFile);
  fflush(stdout);
  struct atomgrp *ag = read_pdb(
      pdbFile, aprm); // read in input file and it's forcefield parameters
  printf("done\n");

  double start, end, charmmTime;

  printf("Processing %s, %s and %s... ", psfFile, prmFile, rtfFile);
  fflush(stdout);
  start = clock();
  read_ff_charmm(psfFile, prmFile, rtfFile,
                 ag); // read in CHARMM forcefield parameters
  end = clock();
  charmmTime = ((double)(end - start)) / CLOCKS_PER_SEC;
  printf("%.3lf sec\n", charmmTime);

  if (useHbond) {
    printf("Processing %s... ", mol2File);
    fflush(stdout);
    if (!read_hybridization_states_from_mol2(mol2File, pdbFile, ag))
      return 1; // read in hybridization states
    printf("done\n");

    printf("Fixing acceptor bases... ");
    fflush(stdout);
  ...
  if (pdbFixedFile != NULL) {
    printf("Processing %s... ", pdbFixedFile);
    fflush(stdout);
  }
  ...
  for (k = 0; k < ndc; k++) {
    ...
    if (useVdw && (dcVdw[k] > ags->nblst->nbcof))
      ags->nblst->nbcof = dcVdw[k];
    if (useElec && (dcElec[k] > ags->nblst->nbcof))
      ags->nblst->nbcof = dcElec[k];
    if (useHbond && (dcHbond[k] > ags->nblst->nbcof))
      ags->nblst->nbcof = dcHbond[k];

    ags->nblst->nbcut = ags->nblst->nbcof + 1;

    if (!noNblist) {
      printf("Building nblists... ");
      fflush(stdout);
    ...
    double nblist_init_E = 0, octree_init_E = 0, nblist_E = 0, octree_E = 0,
           nblist_min = 0, octree_min = 0;
    ...
    printf("\tnmoving = %d  ( %.2lf \% )\n\n", ag->natoms - nfix,
           ((ag->natoms - nfix) * 100.0) / ag->natoms);
    ...
    if (!noNblist) {
      ...
      printf("\tsize = %d bytes ( %.2lf KB, %.2lf MB ), npairs = %d\n", snblist,
             snblist / 1024.0, snblist / (1024.0 * 1024.0), ags->nblst->npairs);
    ...
    if (!noOct) {
      ...
      printf("\tsize = %d bytes ( %.2lf KB, %.2lf MB )\n", soctree,
             soctree / 1024.0, soctree / (1024.0 * 1024.0));
      printf("\tinitial energy = %lf kcal/mol\n", octree_init_E);
      printf("\tfinal energy = %lf kcal/mol\n", octree_E);
      printf("\n");
    }
    ...
    if (!noNblist && !noOct) {
      ...
      if (octree_cons > 0)
        printf("\tconstruction-time( nblist ) / construction-time( octree ) = "
               "%.2lf\n",
      ...
      if (octree_min > 0)
        printf("\trunning-time( nblist ) / running-time( octree ) = %.2lf\n",
               nblist_min / octree_min);
      else
        printf("\trunning-time( nblist ) / running-time( octree ) = inf\n");

      if (soctree > 0)
        printf("\tsize( nblist ) / size( octree ) = %.2lf\n",
               (1.0 * snblist) / soctree);
      else
        printf("\tsize( nblist ) / size( octree ) = inf\n");
