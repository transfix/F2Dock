...
void aeng(struct atomgrp *ag, double *en) {
  ...
  for (i = 0; i < ag->nangact; i++) {
    ...
    a1 = ap->a1;
    a2 = ap->a2;
    //                if(a0->fixed == 1 && a1->fixed == 1 && a2->fixed ==
    //                1)continue;
    dx10 = a0->X - a1->X;
    dx12 = a2->X - a1->X;
    dy10 = a0->Y - a1->Y;
    ...
    if (sth < small) {
      fprintf(stderr, "angle %d (%d, %d, %d) is close to linear\n", i,
              a0->ingrp, a1->ingrp, a2->ingrp);
      fprintf(stderr, "accuracy of forces will be compromised\n");
      sth = small;
    }
...
void check_epeng_grads(struct atomgrp *ag, struct grid *pot, double d,
                       void (*efun)(double, struct atomgrp *, double *,
                                    struct grid *)) {
  int n = ag->natoms, i;
  double en, en1, t;
  double *fs = (double *)_mol_malloc(3 * n * sizeof(double));
...
void check_speng_grads(int nstart, int nend, struct atomgrp *ag, double d,
                       double stens, double *hx0, double *hy0, double *hz0,
                       int nx, int ny, int nz, double dcx, double dcy,
                       double dcz, double cx, double cy, double cz, double w,
  ...
  //      (*efun)(ag, &en);
  //        (*efun)(1.0,ag, &en, pot);
  (*efun)(1.0, stens, ag, &en, hx0, hy0, hz0, nx, ny, nz, dcx, dcy, dcz, cx, cy,
          cz, w);
  ...
  for (i = nstart; i <= nend; i++) {
    ...
    //              (*efun)(ag, &en1);
    //              (*efun)(1.0,ag, &en1, pot);
    (*efun)(1.0, stens, ag, &en1, hx0, hy0, hz0, nx, ny, nz, dcx, dcy, dcz, cx,
            cy, cz, w);
    ag->atoms[i].X = t;
    fs[3 * i] = (en - en1) / d;
    // y
