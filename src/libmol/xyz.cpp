...
struct atomgrp *read_xyz(const char *path, struct prm *prm) {
  ...
  read = getline(&line, &len,
                 fp); // get the header (the number of atoms in xyz file)
  if (read == -1) {
  ...
  while (getline(&line, &len, fp) != -1) {
    ...
    */
    ...
    if (sscanf(line, "%*d %*s %lf %lf %lf %d", &xyz->atoms[atomn].X,
               &xyz->atoms[atomn].Y, &xyz->atoms[atomn].Z,
...
struct atomgrp *read_oldxyz(const char *path, struct prm *prm) {
  ...
  while (getline(&line, &len, fp) != -1) {
    ...
    */
    ...
    if (sscanf(line, "%*d %*s %lf %lf %lf %f %d", &xyz->atoms[atomn].X,
               &xyz->atoms[atomn].Y, &xyz->atoms[atomn].Z, &sa,
               &xyz->atoms[atomn].atom_typen
               /*
               &xyz->atoms[atomn].bonds[0],
               &xyz->atoms[atomn].bonds[1],
...
void print_xyz(struct atomgrp *xyz, struct prm *prm) {
  ...
  for (atomn = 0; atomn < xyz->natoms; atomn++) {
    ...
    printf(
        "%-3.3s",
        prm->atoms[xyz->atoms[atomn].atom_typen]
            .typemin); // atom type (left aligned), only print first three chars
...
void fprint_oldxyz(struct atomgrp *xyz, struct prm *prm, const char *path) {
  ...
  for (atomn = 0; atomn < xyz->natoms; atomn++) {
    ...
    while (bi < 4 && xyz->atoms[atomn].bonds[bi] != -1)
    {
            fprintf (fp, "%6d", xyz->atoms[atomn].bonds[bi]+1); // atom type
    number bi++;
    }
    */
