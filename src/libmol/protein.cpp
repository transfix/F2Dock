...
struct atomgrp **extract_nitrogen_residues(struct atomgrp *ag,
                                           struct prm *prm) {
  char nitrogen[2] = {'N', '\0'};
  struct atomgrp **ress; // array of residues result

  // if no atoms or first atom is not N, return ag as index 0 and NULL as index
  // 1 of ress
  if (ag->natoms < 1 ||
      strcmp(nitrogen, prm->atoms[ag->atoms[0].atom_typen].typemin) != 0) {
  ...
  for (atomi = 0; atomi < ag->natoms;
       atomi++) // first count the number of residues
  {
    natoms++;
    if (strcmp(nitrogen, prm->atoms[ag->atoms[atomi].atom_typen].typemin) ==
        0) {
      nresidues++;
      if (natoms > maxnatoms) {
        maxnatoms = natoms - 1;
  ...
  if (nresidues ==
      0) // no res, return ag as index 0 and NULL as index 1 of ress
  {
    ress = (struct atomgrp **)_mol_malloc(2 * sizeof(struct atomgrp *));
    ress[0] = copy_atomgrp(ag);
    ress[1] = NULL; // flag the end with NULL
  } else {
    ress = (struct atomgrp **)_mol_malloc((nresidues + 1) *
    ...
    for (atomi = 0; atomi < ag->natoms; atomi++) // separate and store the ress
    {
      if (strcmp(nitrogen, prm->atoms[ag->atoms[atomi].atom_typen].typemin) ==
        ...
        if (resn != 0)
          ress[resn - 1]->natoms =
              resatomn; // attach the natoms for the prev res
        resatomn = 0;   // reset

        ress[resn] = (struct atomgrp *)_mol_calloc(1, sizeof(struct atomgrp));
        ress[resn]->atoms =
            (struct atom *)_mol_malloc(maxnatoms * sizeof(struct atom));
      }
...
struct atomgrp *around(int nlist, int *list, struct atomgrp *ag,
                       double distup) {
  int i, j;
