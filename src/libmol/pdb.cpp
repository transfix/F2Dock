...
struct atomgrp *read_pdb(const char *path, struct prm *prm) {
  ...
  while (getline(&line, &len, fp) != -1) {
    ...
    if (atomi + 1 > ag->natoms) {
      ag->natoms *= 2;
      ag->atoms = (struct atom *)_mol_realloc(ag->atoms,
    ...
    */
    ...
    if (ag->atoms[atomi].atom_typen == -1) // val not found
    {
      ...
      {
        fprintf(stderr,
                "error: in file %s line %s: atom type number of %s %s not "
                "defined in prm\n",
...
struct atomgrp *read_pdb_nopar(const char *path) {
  ...
  while (getline(&line, &len, fp) != -1) {
    ...
    */
    ...
    if (!strncmp(line + 13, "C ", 2) || !strncmp(line + 13, "CA ", 3) ||
        !strncmp(line + 13, "N ", 2) || !strncmp(line + 13, "O ", 2) ||
        !strncmp(line + 13, "H ", 2))
      ag->atoms[atomi].backbone = 1;
    else
      ag->atoms[atomi].backbone = 0;
...
struct atomgrp **read_pdb_models(const char *path, struct prm *prm,
  ...
  while (getline(&line, &len, fp) != -1) {
    ...
    if ((modeli + 1 > (*rmodels - 1)) &&
        ((*rmodels) !=
         -1)) // If were over requested models and we don't read everyhting
      break;
    if (modeli + 1 > nmodels) {
      printf("realloc\n");
      nmodels *= 2;
      ag_models = (struct atomgrp **)_mol_realloc(
          ag_models, nmodels * sizeof(struct atomgrp *));
    }
    modeli++;
    ag_models[modeli] =
        (struct atomgrp *)_mol_calloc(1, sizeof(struct atomgrp));
    ag_models[modeli]->natoms = 100; // just a guess, realloc as necessary
    ag_models[modeli]->atoms = (struct atom *)_mol_malloc(
        sizeof(struct atom) * ag_models[modeli]->natoms);
    int atomi = 0;
    while (getline(&line, &len, fp) != -1) {
      if (strncmp(line, "ENDMDL", 6) ==
          0) // if ENDMDL we finished group reading
      {
        ag_models[modeli]->natoms = atomi;
        ag_models[modeli]->atoms = (struct atom *)_mol_realloc(
      ...
      if (ag_models[modeli]->atoms[atomi].atom_typen == -1) // val not found
      {
        ...
        if (ag_models[modeli]->atoms[atomi].atom_typen ==
            -1) // val still not found
        {
          fprintf(stderr,
                  "error: in file %s line %s: atom type number of %s %s not "
                  "defined in prm\n",
  ...
  myfclose(fp);
  nmodels = modeli + 1;
  ag_models = (struct atomgrp **)_mol_realloc(ag_models,
...
void fprint_pdb(struct atomgrp *ag, struct prm *prm, const char *path) {
  ...
  // struct atomgrp** ags = (struct atomgrp**) _mol_malloc (2 * sizeof (struct
  // atomgrp*));
