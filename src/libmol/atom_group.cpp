...
void mol_atom_group_create_from_template(mol_atom_group *ag,
...
struct atomgrp *fullcopy_atomgrp(struct atomgrp *srcag) {
  ...
  destag->bonds =
      (atombond *)_mol_malloc(destag->nbonds * sizeof(struct atombond));
  for (i = 0; i < destag->nbonds; i++) {
    destag->bonds[i] = srcag->bonds[i];
  }

  destag->nbact = srcag->nbact;
  destag->bact = (struct atombond **)_mol_malloc(destag->nbact *
  ...
  destag->angs =
      (struct atomangle *)_mol_malloc(destag->nangs * sizeof(struct atomangle));
  for (i = 0; i < destag->nangs; i++) {
    destag->angs[i] = srcag->angs[i];
  }

  destag->nangact = srcag->nangact;
  destag->angact = (struct atomangle **)_mol_malloc(destag->nangact *
  ...
  destag->tors = (struct atomtorsion *)_mol_malloc(destag->ntors *
  ...
  destag->toract = (struct atomtorsion **)_mol_malloc(
      destag->ntoract * sizeof(struct atomtorsion *));
  for (i = 0; i < destag->ntoract; i++) {
    destag->toract[i] = srcag->toract[i];
  }

  destag->nimps = srcag->nimps;
  destag->imps = (struct atomimproper *)_mol_malloc(
      destag->nimps * sizeof(struct atomimproper));
  for (i = 0; i < destag->nimps; i++) {
    destag->imps[i] = srcag->imps[i];
  }

  destag->nimpact = srcag->nimpact;
  destag->impact = (struct atomimproper **)_mol_malloc(
      destag->nimpact * sizeof(struct atomimproper *));
...
struct atomgrp *exrm_type(struct atomgrp *ag, const char *type, struct prm *prm,
  ...
  struct atomgrp *exag = (struct atomgrp *)_mol_calloc(
      1, sizeof(struct atomgrp)); // resultant extracted atomgrp
  exag->atoms = (struct atom *)_mol_malloc(
      sizeof(struct atom) *
      ag->natoms); // allocate memory for the array of atoms, realloc later to
                   // make it tight

  natoms = 0;
  for (atomn = 0; atomn < ag->natoms; atomn++) {
    if ((direction == 0 &&
         strcmp(type, prm->atoms[ag->atoms[atomn].atom_typen].typemin) ==
             0) // extract type
        || (direction == 1 &&
            !(strcmp(type, prm->atoms[ag->atoms[atomn].atom_typen].typemin) ==
              0)) // rm type
  ...
  exag->atoms = (struct atom *)_mol_realloc(
      exag->atoms,
      sizeof(struct atom) * natoms); // realloc exag->atoms to make it tight
...
struct atomgrp *exrm_typemaj(struct atomgrp *ag, const char *typemaj,
  ...
  for (atomn = 0; atomn < ag->natoms; atomn++) {
    if ((direction == 0 &&
         strcmp(typemaj, prm->atoms[ag->atoms[atomn].atom_typen].typemaj) ==
             0) // extract typemaj
        ||
        (direction == 1 &&
         !(strcmp(typemaj, prm->atoms[ag->atoms[atomn].atom_typen].typemaj) ==
...
struct atomgrp *join_atomgrps(struct atomgrp **ags) {
  ...
  while (ags[agi] != NULL) {
    ...
    for (agsai = 0; agsai < ags[agi]->natoms; agsai++, agai++) {
      if (agai + 1 > ag->natoms) {
        ag->natoms *= 2;
        ag->atoms = (struct atom *)_mol_realloc(ag->atoms, sizeof(struct atom) *
                                                               ag->natoms);
      }
  ...
  ag->natoms = agai;
  ag->atoms =
      (struct atom *)_mol_realloc(ag->atoms, sizeof(struct atom) * ag->natoms);
