...
void test_nbgrads(struct atomgrp *ag, double d, struct nblist *nblst, int n03,
                  int *list03) {
  int n = ag->natoms, i;
...
void vdwengs03(double f, double rc, struct atomgrp *ag, double *ven, int n03,
               int *list03) {
  int i, i1, i2;
  ...
  for (i = 0; i < nblst->nfat; i++) {
    ...
    for (j = 0; j < n2; j++) {
      ...
      if (d2 < rc2) {
        ...
        (*ven) += eij * (Rd12 - 2 * Rd6 + Rr6 * (4.0 - 2 * dr6) +
                         Rr12 * (2 * dr6 - 3.0));
        dven = -eij * 12 * (-Rd12 + Rd6 + dr6 * (Rr12 - Rr6)) / d2;
        g = dven * dx;
        (a1->GX) += g;
...
void eleng03(double f, struct atomgrp *ag, double eps, double *een, int n03,
             int *list03) {
  int i, i1, i2;
  struct atom *a1, *a2;
...
void gen_nblist_orig(struct atomgrp *ag, struct cubeset *cust,
          ...
          if (d <= dda) {
            ...
            for (k1 = 0; k1 < clst->clusters[icl1].natoms; k1++) {
              ...
              for (k2 = 0; k2 < clst->clusters[icl2].natoms; k2++) {
                ...
                if (ag->atoms[ak1].fixed + ag->atoms[ak2].fixed == 2)
                  continue;
                if (ak1 < ak2) {
                  ka1 = ak1;
                  ka2 = ak2;
                ...
                if (ifat[ka1] == -1) {
                  ...
                  ifat[ka1] = nblst->nfat;
                  nblst->isat[nblst->nfat] =
                      (int *)_mol_malloc((natoms - ka1 - 1) * sizeof(int));
                  (nblst->nfat)++;
                }
                ip = ifat[ka1];
                p = nblst->isat[ip];
  ...
  if (nblst->nfat > 0) {
    ...
    nblst->isat =
        (int **)_mol_realloc(nblst->isat, (nblst->nfat) * sizeof(int *));
  }

  for (i = 0; i < nblst->nfat; i++) {
    ntotp += nblst->nsat[i];
    nblst->isat[i] =
        (int *)_mol_realloc(nblst->isat[i], (nblst->nsat[i]) * sizeof(int));
  }
  nblst->npairs = ntotp;
}
...
void gen_nblist(struct atomgrp *ag, struct cubeset *cust,
  ...
  for (i = 0; i < natoms; i++) {
    ...
    if (ip > -1) {
      if (nblst->nsat[ip] > 0)
        nblst->isat[ip] = (int *)_mol_malloc(nblst->nsat[ip] * sizeof(int));
      nblst->nsat[ip] = 0;
    }
    nblst->ifat[i] = -1;
    ifat[i] = -1;
...
void gen_clset(int natoms, struct clusterset *clst, int nclust, int *clust) {
  ...
  clst->clusters =
      (struct cluster *)_mol_malloc(nclust * sizeof(struct cluster));
  int i, j;
  for (i = 0; i < nclust; i++)
  ...
  for (i = 0; i < nclust; i++) {
    clst->clusters[i].iatom =
        (int *)_mol_malloc((clst->clusters[i].natoms) * sizeof(int));
    clst->clusters[i].natoms = 0;
  }
  for (i = 0; i < natoms; i++) {
...
void comp_list02(int natoms, int *na01, int **pna01, int *na02, int **pna02,
                 int *n02, int *list02) {
  int i, j, k, l, m, n, *p;
  int *list = list02;
...
void trim_list03(int natoms, int *na01, int **pna01, int *na02, int **pna02,
                 int *n03, int *list03) {
...
void excl_dims(int natoms, int *na01, int **pna01, int n02, int *list02,
               int n03, int *list03, int *nd1, int *nd2, int *ndm,
               int *atmind) {
  int i, j, k, l, *p;
...
void excl_tab(int natoms, int *na01, int **pna01, int n02, int *list02, int n03,
              int *list03, int nd1, int nd2, int ndm, int *atmind,
...
void init_nblst(struct atomgrp *ag, struct agsetup *ags) {
  ...
  excl_dims(ag->natoms, na01, pna01, n02, list02, n03, list03, &nd1, &nd2, &ndm,
            atmind);
  ...
  excl_tab(ag->natoms, na01, pna01, n02, list02, n03, list03, nd1, nd2, ndm,
           atmind, excl_list, pd1, pd2);
  ags->list02 = list02;
  ags->list03 = list03;
  ags->listf03 = listf03;
  ags->n02 = n02;
  ...
  struct clusterset *clst =
      (struct clusterset *)_mol_malloc(sizeof(struct clusterset));
...
void update_nblst(struct atomgrp *ag, struct agsetup *ags) {
  ...
  gen_nblist(ag, cust, ags->clst, ags->excl_list, ags->pd1, ags->pd2, ags->ndm,
             ags->nblst);
  free_cubeset(cust);
  free(cust);
}
...
int check_clusterupdate(struct atomgrp *ag, struct agsetup *ags) {
  ...
  for (i = 0; i < ag->nactives; i++) {
    j = ag->activelist[i];
    delta = (ags->nblst->crds[3 * j] - ag->atoms[j].X) *
                (ags->nblst->crds[3 * j] - ag->atoms[j].X) +
            (ags->nblst->crds[3 * j + 1] - ag->atoms[j].Y) *
                (ags->nblst->crds[3 * j + 1] - ag->atoms[j].Y) +
            (ags->nblst->crds[3 * j + 2] - ag->atoms[j].Z) *
                (ags->nblst->crds[3 * j + 2] - ag->atoms[j].Z);
    delta = delta * delta;
    if (delta > delta0) {
      flag = 1;
...
void give_012(struct atomgrp *ag, struct agsetup *ags, int *na012, int **la012,
              int *nf012, int **lf012) {
  struct atom *a0, *a1;
  int ia0, ia1;
  int n01 = ag->nbonds;
  ...
  if (n > 0) {
    ...
    *lf012 = (int *)_mol_malloc(2 * n * sizeof(int));
    ...
    if (na > 0)
      *la012 = (int *)_mol_realloc(*la012, 2 * na * sizeof(int));
    else
      free(*la012);
    if (nf > 0)
      *lf012 = (int *)_mol_realloc(*lf012, 2 * nf * sizeof(int));
    else
      free(*lf012);
  }
  *na012 = na;
...
inline int exclude(int a1, int a2, int *excl_list, int **pd1, int **pd2,
                   int ndm) {
  if (a1 <= a2) {
    int i = a2 - a1 - 1;
...
inline double get_pairwise_vdweng_near(int ai, mol_atom *atom_i, double xi,
                                       double yi, double zi, double e_i,
                                       double r_i, OCTREE *octree_static,
                                       int aj, double rc2, double irc2,
                                       struct agsetup *ags) {
  ...
  //   if ( ( d2 < 20 ) && exclude( ai, aj, ags->excl_list, ags->pd1, ags->pd2,
  //   ags->ndm ) ) return 0;
  ...
  double en =
      e_ij * (Rd12 - 2 * Rd6 + Rr6 * (4.0 - 2 * dr6) + Rr12 * (2 * dr6 - 3.0));
...
inline double get_pairwise_vdweng_far(int ai, mol_atom *atom_i, double xi,
                                      double yi, double zi, double e_i,
                                      double r_i, OCTREE *octree_static, int aj,
                                      double rc2, double irc2) {
  mol_atom *atom_j = &(octree_static->atoms[aj]);
...
double get_pairwise_vdweng_near_node(int ai, mol_atom *atom_i, double xi,
                                     double yi, double zi, double e_i,
                                     double r_i, OCTREE *octree_static,
                                     OCTREE_NODE *snode, double rc2,
                                     double irc2, struct agsetup *ags) {
  ...
  for (int j = 0; j < snode->n; j++) {
    ...
    //        if ( d2 < 20 )
    {
      ...
      if (ai < aj)
        k = exta(ai, aj, ags->excl_list, ags->pd1, ags->pd2, ags->ndm);
      else
        k = exta(aj, ai, ags->excl_list, ags->pd1, ags->pd2, ags->ndm);
    ...
    en += e_ij *
          (Rd12 - 2 * Rd6 + Rr6 * (4.0 - 2 * dr6) + Rr12 * (2 * dr6 - 3.0));
...
double get_pairwise_vdweng_far_node(int ai, mol_atom *atom_i, double xi,
                                    double yi, double zi, double e_i,
                                    double r_i, OCTREE *octree_static,
                                    OCTREE_NODE *snode, double rc2,
                                    double irc2) {
...
void vdweng_octree_single_mol(OCTREE_PARAMS *octpar, double *energy) {
    ...
    for (int i = mnode->nfixed; i < mnode->n; i++) {
      ...
      if (d2 < rc2) {
        ...
        for (int j = 0; j < snode->n; j++) {
          ...
          double dx = atom_j->X - x, dy = atom_j->Y - y, dz = atom_j->Z - z;
          ...
          en += e_ij * (Rd12 - 2 * Rd6 + Rr6 * (4.0 - 2 * dr6) +
                        Rr12 * (2 * dr6 - 3.0));
  ...
  printf("< %2d %2d %9lf > < %2d %2d %9lf > %2d %3d %15.12lf %9lf %9lf %d ( "
...
inline double get_pairwise_eleng_near(int ai, mol_atom *atom_i, double xi,
                                      double yi, double zi, double c_i,
                                      OCTREE *octree_static, int aj, double rc,
                                      double irc2, struct agsetup *ags) {
  mol_atom *atom_j = &(octree_static->atoms[aj]);
...
inline double get_pairwise_eleng_far(int ai, mol_atom *atom_i, double xi,
                                     double yi, double zi, double c_i,
                                     OCTREE *octree_static, int aj, double rc,
                                     double irc2) {
  mol_atom *atom_j = &(octree_static->atoms[aj]);
...
void eleng_octree_single_mol_bak(OCTREE *octree_static, int node_static,
                                 OCTREE *octree_moving, int node_moving,
                                 void *params, double dist_cutoff,
                                 double *trans_mat, double *energy) {
  ...
  if ((snode->nfixed == snode->n) && (mnode->nfixed == mnode->n))
    return;
  ...
  double sx = snode->lx + half_dim, sy = snode->ly + half_dim,
         sz = snode->lz + half_dim;
  double hd2 = (dist_cutoff + HALF_SQRT_THREE * snode->dim) *
               (dist_cutoff + HALF_SQRT_THREE * snode->dim);
  double ld2 = (4.5 + HALF_SQRT_THREE * snode->dim) *
               (4.5 + HALF_SQRT_THREE * snode->dim);
    ...
    if (d2 < hd2) {
      ...
      if (d2 < ld2) {
        for (int j = 0; j < snode->nfixed; j++) {
          int aj = snode->indices[j];
          (*energy) += get_pairwise_eleng_near(
              ai, atom_i, x, y, z, c_i, octree_static, aj, rc, irc2, ags);
        }
        ...
        for (int j = 0; j < snode->nfixed; j++) {
          int aj = snode->indices[j];
          (*energy) += get_pairwise_eleng_far(ai, atom_i, x, y, z, c_i,
                                              octree_static, aj, rc, irc2);
        }
