...
int *read_sasa(const char *path) {
  ...
  nsasas = i;
  sasas = (int *)_mol_realloc(sasas,
                              sizeof(int) * (nsasas + 1)); // one extra for -1
  sasas[nsasas] = -1;
...
/* convert float atomic surface arrays into 0/1 representation (sa in the
 * structure atom) */
...
/* sthresh   if as > sthresh sa=1
             if as<= sthresh sa=0   */
void baccs(struct atomgrp *ag, struct prm *prm, float r_solv, short cont_acc,
           short rpr, float sthresh) {
  int n_at = ag->natoms;
  int i;
...
/*This function does the same thing as baccs except it call accs1 (the
traditional surface area accesibility algorith) instead of accs (in-house
protein-protein concerned algorithm).*/
void baccs1(struct atomgrp *ag, int n_at, int *restat, double r_solv,
            short cont_acc, float sthresh) {
  int i;
...
void accs(struct atomgrp *ag, struct prm *prm, float r_solv, short cont_acc,
  ...
  for (i = 0; i < n_at; i++) {
    ...
    if (rpr) {
      ...
      if (an == 'C')
        ...
        ri = R_H;
      /*			{
                                      fprintf (stderr, "error: accs.c> hydrogens
         are not allowed with rpr=0\n"); exit(EXIT_FAILURE);
                              } */
      else
        ri = R_ELSE;
    }
  ...
  i = kjidim * sint;
  int *itab =
      (int *)_mol_malloc(i * sizeof(int)); /* number of atoms in each cube */
  for (i = 0; i < kjidim; i++)
    itab[i] = 0;
  ...
  i = n_at * sint;
  int *cube =
      (int *)_mol_malloc(i * sizeof(int)); /* cube number for each atom */
...
Ryan Brenke altered accs to better account for protein-protein docking
consideration. Dmitri Beglov apparently restored accs to its original form.*/
