...
void bracket(double *orig, double *dir, double step, int ndim, void *prms,
  ...
  *lc = *lb + G * (*lb - *la);
  ...
  while ((fc <= *fb)) {
    ...
    if (fabs(parabp1 - parabp2) > TooSmall) {
      lnew = *lb + ((*lb - *lc) * parabp2 - (*lb - *la) * parabp1) /
                       (2 * (parabp1 - parabp2));
...
void old_brent(double *orig, double *dir, double fb, double la, double lb,
               double lc, int ndim, void *prms,
               void (*egfun)(int, double *, void *, double *, double *),
...
void brent(double *orig, double *dir, double fb, double la, double lb,
           double lc, int ndim, void *prms,
           void (*egfun)(int, double *, void *, double *, double *), double tol,
           unsigned int maxtimes, double *min, double *fmim) {
...
void dirbrent(double *orig, double *dir, double fb, double la, double lb,
              double lc, int ndim, void *prms,
              void (*egfun)(int, double *, void *, double *, double *),
              double tol, int maxtimes, double *min, double *fmim,
              double *grad) {
  ...
  for (numIt = 0; numIt < maxtimes; numIt++) {
    ...
    if (fabs(distMoved2) > lim) {
      ...
      if (canUse1 != 0) {
        if (((lbound1 - lcurmin - sec1) * (lcurmin + sec1 - lbound2) <= 0) ||
            (dcurmin * sec1 > 0))
          canUse1 = 0;
      }

      if (canUse2 != 0) {
        if (((lbound1 - lcurmin - sec2) * (lcurmin + sec2 - lbound2) <= 0) ||
            (dcurmin * sec2 > 0))
          canUse2 = 0;
      }
...
void powell(double *orig, double *directions, unsigned int maxIt, double tol,
  ...
  int j, k,         // variables that will be used in for loops
      jmostdec = 0, // direction of  the biggest  decrease withing a single
                    // directions set
      numAct = ndim;
  unsigned int i;
  double mostdec = 0;
  ...
  double test;
  double *pcur = (double *)_mol_malloc(
      numAct * sizeof(double)); // point of current minimum
  double *prev =
      (double *)_mol_malloc(numAct * sizeof(double)); // previous minimum
  double *newdir = (double *)_mol_malloc(
      numAct * sizeof(double)); // average direction moved in one iteration
  ...
  for (i = 0; i < maxIt; i++) // loop through once for each direction set(break
                              // out if close enough)
  {
    fprev = val;
    mostdec = 0;
    ...
    for (j = 0; j < numAct; j++) // loop through once for each direction
    {
      ...
      brent(pcur, newdir, fbrac, la, lb, lc, ndim, prms, egfun, pow(10, -5),
            100, pcur, &val);
      if (fabs(fprev2 - val) >
          mostdec) // get direction of greatest decrease and greatest decrease
      {
    ...
    if (2 * fabs(fprev - val) <=
        tol * (fabs(fprev) +
               fabs(val))) // if you reach the desired tolerance end powell
    {
      for (j = 0; j < numAct; j++)
        min[j] = pcur[j];
    ...
    if (fprev2 < fprev) {
      test =
          2. * (fprev - 2. * val + fprev2) * pow((fprev - val - mostdec), 2) -
          mostdec * pow((fprev - fprev2), 2);
      if (test < 0) { // if it makes sence to the direction set
        bracket(pcur, newdir, 1, ndim, prms, egfun, &fbrac, &la, &lb,
                &lc); // minimize in new direction
        brent(pcur, newdir, fbrac, la, lb, lc, ndim, prms, egfun, pow(10, -5),
              100, pcur, &val);
        for (k = 0; k < numAct; k++) {
...
void dirMin(double *orig, unsigned int maxIt, double tol, int ndim, void *prms,
            void (*egfun)(int, double *, void *, double *, double *),
            double *min, double *fmim) {
  int numAct = ndim;
  int i, j;
  ...
  for (it = 0; it < maxIt; it++) {
    ...
    bracket(curmin, dir, 1, ndim, prms, egfun, &fbrac, &la, &lb, &lc);

    dirbrent(curmin, dir, fbrac, la, lb, lc, ndim, prms, egfun, pow(10, -10),
             100, mvec, &val, grad);
...
void limin(double *orig, double *dir, unsigned int maxIt, double tol, int ndim,
           void *prms, void (*egfun)(int, double *, void *, double *, double *),
           double *min, double *fmim) {
  double fbrac, la, lb, lc;
  bracket(orig, dir, 1, ndim, prms, egfun, &fbrac, &la, &lb, &lc);
  brent(orig, dir, fbrac, la, lb, lc, ndim, prms, egfun, tol, maxIt, min, fmim);
}
...
double dg = 0, dgm = 0, dginit = 0, dgtest = 0, dgx[1], dgxm[1], dgy[1],
       dgym[1], finit = 0, ftest1 = 0, fm = 0, fx[1], fxm[1], fy[1], fym[1],
       p5 = 0, p66 = 0, stx[1], sty[1], stmin = 0, stmax = 0, width = 0,
       width1 = 0, xtrapf = 0;
void linesearch(int n, double *x, double f, double *g, double *s, int is0,
...
double dg = 0, dgm = 0, dginit = 0, dgtest =
  ...
  while (1) {
    ...
    if (stage1 && f <= fx[0] && f > ftest1) {
      ...
      linestep(stx, fxm, dgxm, sty, fym, dgym, stp, fm, dgm, brackt, stmin,
               stmax, infoc);
      ...
      // and to compute the new step.
      linestep(stx, fx, dgx, sty, fy, dgy, stp, f, dg, brackt, stmin, stmax,
               infoc);
    }
 ...
 *iteration, <li> <code>iprint[0] &gt; 0</code>: output every
 ...
 *function evaluations, function value, norm of the gradient, and steplength,
 *		<li> <code>iprint[1] = 1</code>: same as
 *<code>iprint[1]=0</code>, plus vector of variables and  gradient vector at the
 *initial point, <li> <code>iprint[1] = 2</code>: same as
 *<code>iprint[1]=1</code>, plus vector of variables, <li> <code>iprint[1] =
 *3</code>: same as <code>iprint[1]=2</code>, plus gradient vector.
 *		</ul>
...
double stp1 = 0, ftol = 0, stp[1], ys = 0, yy = 0, sq = 0, yr = 0, beta = 0,
       xnorm = 0;
int point = 0, ispt = 0, iypt = 0, maxfev = 0, info[1], bound = 0, npt = 0,
    cp = 0, nfev[1], inmc = 0, iycn = 0, iscn = 0;

/* worker array w needs to be initialized*/
void lbfgs(int n, int m, double *x, double f, double *g, short diagco,
  ...
  if (iflag[0] == 0) {
    ...
    if (gtol <= 0.0001) {
      printf("LBFGS.lbfgs: gtol is less than or equal to 0.0001. It has been "
             "reset to 0.9.\n");
      gtol = 0.9;
    }
    ...
    if (diagco) {
      for (i = 1; i <= n; i += 1) {
        if (diag[i - 1] <= 0) {
          iflag[0] = -2;
          printf("The %d-th diagonal element of the inverse hessian "
                 "approximation is not positive. Error %d\n",
                 i, iflag[0]);
          return;
        }
      }
  ...
  while (1) {
    ...
    linesearch(n, x, f, g, w, ispt + point * n, stp, ftol, xtol, maxfev, info,
               nfev, diag);
    ...
    if (info[0] != 1) {
      iflag[0] = -1;
      if (iprint[1 - 1] >= 0) {
        printf("Line search failed. See documentation of routine mcsrch. Error "
               "return of line search: info = %d Possible causes: function or "
 ...
 */
void daxpy(int n, double da, double *dx, int ix0, int incx, double *dy, int iy0,
           int incy) {
  int i, ix, iy, m, mp1;
 ...
 * code is a straight translation from the Fortran.
 */
double ddot(int n, double *dx, int ix0, int incx, double *dy, int iy0,
            int incy) {
  double dtemp;
  int i, ix, iy, m, mp1;
...
int lbfgs_run(lbfgs_t *opt, double *x, double f, double *g) {
  lbfgs(opt->n, opt->m, x, f, g, opt->diagco, opt->diag, opt->iprint, opt->eps,
        opt->xtol, opt->w, &opt->iflag, &opt->niter, &opt->nfuns);
  //    printf("%d\n",opt->iflag);
  return opt->iflag;
}

// LBFGS minimizer main loop
void lbfgsMin(double *orig, unsigned int maxIt, double tol, int ndim,
              void *prms,
              void (*egfun)(int, double *, void *, double *, double *),
              double *minv, double *fmim) {
  lbfgs_t *opt;
  int i;
  unsigned int nsteps;
...
*/
void minimize_ag(mol_min_method min_type, unsigned int maxIt, double tol,
                 struct atomgrp *ag, void *minprms,
                 void (*egfun)(int, double *, void *, double *, double *)) {
  ...
  */
  if (min_type == MOL_LBFGS)
    lbfgsMin(xyz, maxIt, tol, ndim, minprms, egfun, minv, &fmim);
