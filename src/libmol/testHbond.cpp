...
void print_short_args(char *app_name) {
  fprintf(stderr,
          "usage: %s PDBFILE OUTPUTPDB PSF PRM RTF FIXEEDATOMS ATOMSPRMFILE "
          "MOL2FILE\n",
          app_name);
}
...
int main(int argc, char *argv[]) {
  ...
  if (!build_octree(&octree, max(60, ags->nblst->npairs / ag->natoms), 6, 1.0,
                    ag)) {
  ...
  printf("nblist_hond_energy_cat = < BB-BB-SR: %lf, BB-BB-LR: %lf, BB-SC: %lf, "
         "SC-SC: %lf >\n",
  ...
  for (k = 0; k < MAX_ITER; k++) {
    zero_grads(ag);
    en = octree_accumulation_excluding_far(&octree, &octree, MAX_R, MAX_R, 1,
                                           NULL, (void *)(&eng_params),
                                           hbondeng_octree_single_mol);
  }
  end = clock();
  elapsed = ((double)(end - start)) / CLOCKS_PER_SEC;
  ...
  get_categorized_hbondeng(engcat, &bb_bb_sr, &bb_bb_lr, &bb_sc, &sc_sc);
  printf("octree_hond_energy_cat = < BB-BB-SR: %lf, BB-BB-LR: %lf, BB-SC: %lf, "
         "SC-SC: %lf >\n",
         bb_bb_sr, bb_bb_lr, bb_sc, sc_sc);
