...
float calc_distance(struct tvector *va, struct tvector *vb) {
  float d = sqrtf((va->X - vb->X) * (va->X - vb->X) +
                  (va->Y - vb->Y) * (va->Y - vb->Y) +
                  (va->Z - vb->Z) * (va->Z - vb->Z));
...
struct tvector *negative_tvector(struct tvector *tv) {
  struct tvector *neg_tv =
      (struct tvector *)_mol_malloc(sizeof(struct tvector));
...
void center_atomgrp_on_center_of_extrema(struct atomgrp *ag,
                                         struct atomgrp *centered_ag) {
  struct tvector *neg_com = negative_center_of_extrema(ag);
  translate_atomgrp(ag, centered_ag, neg_com);
  free(neg_com);
}

void center_atomgrp_on_center_of_mass(struct atomgrp *ag,
                                      struct atomgrp *centered_ag) {
...
void translate_atomgrp(struct atomgrp *src, struct atomgrp *dest,
                       struct tvector *tvec) {
...
struct rotset *read_rotset(const char *path) {
  ...
  rotset->rmats =
      (struct rmatrix *)_mol_malloc(rotset->nrots * sizeof(struct rmatrix));
  ...
  while (getline(&line, &len, fp) != -1) {
    ...
    if (sscanf(
            line, "%d %f %f %f %f %f %f %f %f %f", &tmpi, &rotset->rmats[i].a11,
            &rotset->rmats[i].a12, &rotset->rmats[i].a13, &rotset->rmats[i].a21,
            &rotset->rmats[i].a22, &rotset->rmats[i].a23, &rotset->rmats[i].a31,
            &rotset->rmats[i].a32, &rotset->rmats[i].a33) < 10) {
...
struct axisset *read_axisset(const char *path) {
  ...
  struct axisset *axisset =
      (struct axisset *)_mol_malloc(sizeof(struct axisset));
  axisset->naxes = naxes; // save the value
  axisset->axes =
      (struct axis *)_mol_malloc(axisset->naxes * sizeof(struct axis));
  ...
  while (getline(&line, &len, fp) != -1) {
    ...
    if (sscanf(line, "%d %f %f %f", &tmpi, &axisset->axes[i].a,
               &axisset->axes[i].b, &axisset->axes[i].c) < 4) {
      fprintf(stderr, "read error on file %s\n", path);
...
void print_axisset(struct axisset *axisset) {
  ...
  for (i = 0; i < axisset->naxes; i++) {
    printf("%d:\t", i);
    printf("%8.4f\t%8.4f\t%8.4f\n", axisset->axes[i].a, axisset->axes[i].b,
           axisset->axes[i].c);
  }
}
...
struct rotset *create_sym_rotset(int nmonomers) {
  struct rotset *rotset = (struct rotset *)_mol_malloc(sizeof(struct rotset));
  rotset->nrots =
      2; // 2 rotations: 1 for cyclical symmetry and 1 for dihedral symmetry
  rotset->rmats =
      (struct rmatrix *)_mol_malloc(rotset->nrots * sizeof(struct rmatrix));
...
void rotate_atomgrp(struct atomgrp *src, struct atomgrp *dest,
  ...
  for (atomi = 0; atomi < src->natoms; atomi++) {
    ...
    dest->atoms[atomi].X =
        rmatrix->a11 * X + rmatrix->a12 * Y + rmatrix->a13 * Z;
    dest->atoms[atomi].Y =
        rmatrix->a21 * X + rmatrix->a22 * Y + rmatrix->a23 * Z;
    dest->atoms[atomi].Z =
        rmatrix->a31 * X + rmatrix->a32 * Y + rmatrix->a33 * Z;
  }
}

void move_atomgrp(struct atomgrp *ag, struct atomgrp *moved_ag,
                  struct rmatrix *rmatrix, struct tvector *tv) {
...
void print_rmatrix(struct rmatrix *rmatrix) {
  printf("rmatrix:");
  printf("%8.4f\t%8.4f\t%8.4f\t%8.4f\t%8.4f\t%8.4f\t%8.4f\t%8.4f\t%8.4f\n",
         rmatrix->a11, rmatrix->a12, rmatrix->a13, rmatrix->a21, rmatrix->a22,
         rmatrix->a23, rmatrix->a31, rmatrix->a32, rmatrix->a33);
}
