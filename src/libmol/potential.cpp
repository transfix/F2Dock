...
struct matrix2df *potential_matrix2df_rowcol_subatom_join(struct matrix2df *A,
  ...
  for (i = 0; i < A->ni; i++) {
    ...
    if (subi > prm->nsubatoms - 1) {
      ...
      fprintf(stderr, "in function potential_matrix2df_rowcol_subatom_join\n");
      fprintf(stderr, "in the first atom group\n");
      fprintf(stderr, "the argument atom prm subatom mapping of atom (row) %d",
              i);
    ...
    for (j = 0; j < A->nj; j++) {
      if (j < 0 || j > prm->natoms - 1) {
        ...
        fprintf(stderr, "the column %d does not have a corresponding atom type",
                j);
        fprintf(stderr, "in the argument atom prm\n");
        fprintf(stderr, "end error\n");
        exit(EXIT_FAILURE);
      ...
      if (subj > prm->nsubatoms - 1) {
        ...
        fprintf(stderr, "in the first atom group\n");
        fprintf(stderr,
                "the argument atom prm subatom mapping of atom (column) %d", j);
        fprintf(stderr, "is greater than the maximum subatom type index\n");
        fprintf(stderr, "end error\n");
        exit(EXIT_FAILURE);
      }
