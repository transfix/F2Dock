...
void read_ff_charmm(const char *psffile, char *prmfile, char *rtffile,
  ...
  read_psf(psffile, &nat, &atoms, &atom_names, &nb, &bonds, &nang, &angs,
           &ndih0, &dihs, &nimp, &imps, &cha, &nres, &ires, dres, &ndon, &dons,
           &nacc, &acpts);
  ...
  if (nres > 0) {
    ...
    ag->res_type =
        (enum mol_res_type *)_mol_malloc(nres * sizeof(enum mol_res_type));
  }
  for (i = 0; i < nres; i++) {
    ag->iares[i] = ires[i];
    ag->idres[i] = dres[i];
  ...
  ndih = ndih0;
  read_par(prmfile, nat, atnam, nb, bonds, &kbond, &lbond, nang, angs, &kang,
           &fang, &ndih, dihs, &kdih, &fdih, &pdih, &tdih, nimp, imps, &kimp,
           &fimp, &epa, &siga, &acevolumes);

  //        printf("DIHEDRAL ndih0, ndih %d %d\n", ndih0, ndih);
  if ((ag->natoms) != nat) {
    fprintf(stderr, "different number of atoms after reading protein structure "
                    "file (PSF)\n");
    exit(EXIT_FAILURE);
  }
  ...
  for (i = 0; i < nat; i++) {
    ...
    if (epa[2 * i] <= 0)
      ...
      printf("error on pdb atom %d: positive vdw eps for atom type %.4s", i,
             &atnam[4 * i]);
      exit(EXIT_FAILURE);
    }
  ...
  ag->nimps = nimp;
  ag->imps =
      (struct atomimproper *)_mol_malloc(nimp * sizeof(struct atomimproper));
  for (i = 0; i < nimp; i++) {
    i1 = imps[4 * i] - 1;
  ...
  // torsions
  ag->tors =
      (struct atomtorsion *)_mol_malloc(ndih * sizeof(struct atomtorsion));
  int ltors = 0;
  for (i = 0; i < ndih; i++) {
    ti = tdih[i];
    if (ti < 0) {
      ...
      printf("%d %d %d %d\n", dihs[4 * i], dihs[4 * i + 1], dihs[4 * i + 2],
             dihs[4 * i + 3]);
      continue;
    }
    i1 = dihs[4 * ti] - 1;
  ...
  for (i = 0; i < nat; i++) {
    ...
    ag->atoms[i].bonds =
        (struct atombond **)_mol_malloc(i1 * sizeof(struct atombond *));
    ag->atoms[i].nbonds = 0;
    i1 = ag->atoms[i].nangs;
    ag->atoms[i].angs =
        (struct atomangle **)_mol_malloc(i1 * sizeof(struct atomangle *));
    ag->atoms[i].nangs = 0;
    i1 = ag->atoms[i].ntors;
    ag->atoms[i].tors =
        (struct atomtorsion **)_mol_malloc(i1 * sizeof(struct atomtorsion *));
    ag->atoms[i].ntors = 0;
    i1 = ag->atoms[i].nimps;
    ag->atoms[i].imps =
        (struct atomimproper **)_mol_malloc(i1 * sizeof(struct atomimproper *));
    ag->atoms[i].nimps = 0;
  }
  struct atom *atm;
  // fill bonds
  ...
  for (atomsi = 0; atomsi < ag->natoms; atomsi++) {
    _mol_atom_create_bond_indices(&ag->atoms[atomsi], ag->atoms[atomsi].nbonds);
  }
...
void read_psf(const char *psffile, int *o_nat, int **o_atoms,
  ...
  int nat = 0, nb = 0, nang = 0, ndih = 0, nimp = 0, i, j, nres = 0, ndon = 0,
      nacc = 0;
  int *bonds = NULL, *atoms = NULL, *angs = NULL, *dihs = NULL, *imps = NULL,
      *dons = NULL, *acpts = NULL;
  ...
  while (fgets(buffer, linesize, fp) != NULL) {
    if (strstr(buffer, "NATOM") != NULL) {
      ...
      for (i = 0; i < nat; i++) {
        if (fgets(buffer, linesize, fp) == NULL ||
            nthv(&p1, buffer, ' ', 1) == 0 || nthv(&p, buffer, ' ', 2) == 0 ||
            nthv(&p2, buffer, ' ', 4) == 0 || nthv(&p, buffer, ' ', 5) == 0 ||
            (j = atoi(p)) == 0 || nthv(&p, buffer, ' ', 6) == 0) {
          printf("error: reading atoms from psf\n");
          exit(EXIT_FAILURE);
        }
        ...
        if (strcmp(seg, seg0)) {
          ires[nres] = i;
          strcpy(seg0, seg);
          o_dres[nres] = (char *)_mol_malloc(sizeof(char) * 16);
    ...
    if (strstr(buffer, "NBOND") != NULL) {
      ...
      for (i = 0; i < nb; i++) {
        fscanf(fp, "%d %d", &bonds[2 * i], &bonds[2 * i + 1]);
      }
      continue;
    }
    if (strstr(buffer, "NTHETA") != NULL) {
      ...
      for (i = 0; i < nang; i++) {
        fscanf(fp, "%d %d %d", &angs[3 * i], &angs[3 * i + 1],
               &angs[3 * i + 2]);
      }
      continue;
    }
    if (strstr(buffer, "NPHI") != NULL) {
      ...
      for (i = 0; i < ndih; i++) {
        fscanf(fp, "%d %d %d %d", &dihs[4 * i], &dihs[4 * i + 1],
    ...
    if (strstr(buffer, "NIMPHI") != NULL) {
      ...
      for (i = 0; i < nimp; i++) {
        fscanf(fp, "%d %d %d %d", &imps[4 * i], &imps[4 * i + 1],
               &imps[4 * i + 2], &imps[4 * i + 3]);
      }
      continue;
    }
    if (strstr(buffer, "NDON") != NULL) {
      ...
      for (i = 0; i < ndon; i++) {
        fscanf(fp, "%d %d", &dons[2 * i], &dons[2 * i + 1]);
      }
      continue;
    }
    if (strstr(buffer, "NACC") != NULL) {
      ...
      for (i = 0; i < nacc; i++) {
        fscanf(fp, "%d %d", &acpts[2 * i], &acpts[2 * i + 1]);
      }
      continue;
    }
  }
...
void read_par(char *prmfile, int nat, char *atnam, int nb, int *bonds,
              float **o_kbond, float **o_lbond, int nang, int *angles,
              float **o_kangle, float **o_fangle, int *ndih, int *dihs,
              float **o_kdih, float **o_fdih, int **o_pdih, int **o_tdih,
  ...
  int read_mode = 0;
  while (fgets(buffer, linesize, fp) != NULL) {
    if ((p = strstr(buffer, "e14fac")) || (p = strstr(buffer, "E14FAC")))
      efac = atof(p + 6);
    if ((p = strchr(buffer, '!')))
      *p = '\0';
    if ((strstr(buffer, "NBON") != NULL) || (strstr(buffer, "NONB") != NULL)) {
      read_mode = 5;
      continue;
    ...
    if (read_mode == 3)
      chkdih(buffer, ndih, ndih0, dihs, atnam, kdih, fdih, pdih, tdih, wdih);
    if (read_mode == 4)
...
void chkangle(char *buffer, int nang, int *angles, char *atnam, float *kangle,
              float *fangle) {
  int i;
...
void chkdih(char *buffer, int *ndih, int ndih0, int *dihs, char *atnam,
            float *kdih, float *fdih, int *pdih, int *tdih, int *wdih) {
    ...
    if (strstr(tag0, match) != NULL) {
      if (wdih[i] < 1) {
        ...
        if ((*ndih) < ndih03) {
          ...
          printf("error: need more memory for dihedrals\n");
          exit(1);
        }
      }
...
void chkimp(char *buffer, int nimp, int *imps, char *atnam, float *kimp,
            float *fimp) {
  int i;
  char *p, match[20] = "                   ";
  ...
  for (i = 0; i < nimp; i++) {
    ...
    if (strstr(tag0, match) != NULL || strstr(tag1, match) != NULL ||
        strstr(tag2, match) != NULL || strstr(tag3, match) != NULL ||
...
void fixed_update(struct atomgrp *ag, int nlist, int *list) {
  ...
  ag->nangact = m;
  if (m > 0)
    ag->angact = (struct atomangle **)_mol_realloc(
        ag->angact, m * sizeof(struct atomangle *));
  else
    free(ag->angact);
  // dihedrals
  n = ag->ntors;
  m = 0;
  ag->toract =
      (struct atomtorsion **)_mol_malloc(n * sizeof(struct atomtorsion *));
  for (i = 0; i < n; i++) {
    a0 = ag->tors[i].a0;
    a1 = ag->tors[i].a1;
    a2 = ag->tors[i].a2;
    a3 = ag->tors[i].a3;
    if (a0->fixed == 1 && a1->fixed == 1 && a2->fixed == 1 && a3->fixed == 1)
      continue;
    ag->toract[m++] = &(ag->tors[i]);
  }
  ag->ntoract = m;
  if (m > 0)
    ag->toract = (struct atomtorsion **)_mol_realloc(
        ag->toract, m * sizeof(struct atomtorsion *));
  else
    free(ag->toract);
  // impropers
  n = ag->nimps;
  m = 0;
  ag->impact =
      (struct atomimproper **)_mol_malloc(n * sizeof(struct atomimproper *));
  for (i = 0; i < n; i++) {
    a0 = ag->imps[i].a0;
    a1 = ag->imps[i].a1;
    a2 = ag->imps[i].a2;
  ...
  ag->nimpact = m;
  if (m > 0)
    ag->impact = (struct atomimproper **)_mol_realloc(
        ag->impact, m * sizeof(struct atomimproper *));
  else
    free(ag->impact);
}
...
void read_ff_charmm_light(const char *psffile, struct atomgrp *ag) {
  ...
  /*	read_par(prmfile, nat, atnam, nb, bonds, &kbond, &lbond,
                                          nang, angs, &kang, &fang,
                                          &ndih, dihs, &kdih, &fdih, &pdih,
     &tdih, nimp, imps, &kimp, &fimp, &epa, &siga,&acevolumes);
  */
  //        printf("DIHEDRAL ndih0, ndih %d %d\n", ndih0, ndih);
  if ((ag->natoms) != nat) {
    fprintf(
  ...
  // torsions
  ag->tors =
      (struct atomtorsion *)_mol_malloc(ndih0 * sizeof(struct atomtorsion));
  int ltors = 0;
  for (i = 0; i < ndih0; i++) {
    int i1 = dihs[4 * i] - 1;
    int i2 = dihs[4 * i + 1] - 1;
