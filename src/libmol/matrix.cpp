...
struct matrix2di *matrix2di_create(int ni, int nj) {
  // error checking
  if (ni < 1 || nj < 1) {
    fprintf(stderr, "error: in function matrix2di_create, argument matrix "
                    "size(s) is less than 1\n");
    exit(EXIT_FAILURE);
  }

  // allocate memory for the matrix
  struct matrix2di *A =
      (struct matrix2di *)_mol_malloc(sizeof(struct matrix2di));
...
struct matrix2df *matrix2df_create(int ni, int nj) {
  // error checking
  if (ni < 1 || nj < 1) {
    fprintf(stderr, "error: in function matrix2df_create, argument matrix "
                    "size(s) is less than 1\n");
    exit(EXIT_FAILURE);
  }
  ...
  // allocate memory for the matrix
  struct matrix2df *A =
      (struct matrix2df *)_mol_malloc(sizeof(struct matrix2df));
...
struct matrix2di *matrix2di_read(const char *path) {
  ...
  struct matrix2di *A =
      matrix2di_create(ni, nj); // create the new matrix of size ni x nj
...
struct matrix2df *matrix2df_read(const char *path) {
  ...
  struct matrix2df *A =
      matrix2df_create(ni, nj); // create the new matrix of size ni x nj
...
void matrix2df_scalar_mult(struct matrix2df *A, float val,
                           struct matrix2df *B) {
  if (matrix2df_size_diff(A, B)) {
    fprintf(stderr, "begin error\n");
