...
int init_free_node_server(OCTREE *octree) {
  ...
  octree->nodes =
      (OCTREE_NODE *)_mol_malloc(INIT_NUM_OCTREE_NODES * sizeof(OCTREE_NODE));
  //   octree->nodes = ( OCTREE_NODE * ) memalign( ALIGNMENT_BLOCK_SIZE,
  //   INIT_NUM_OCTREE_NODES * sizeof( OCTREE_NODE ) );
...
int next_free_node(OCTREE *octree) {
  if (octree->free_node_ptr == -1) {
    ...
    octree->nodes = (OCTREE_NODE *)_mol_realloc(
        octree->nodes, new_num_nodes * sizeof(OCTREE_NODE));
...
inline void compute_root_bounding_box(int node_id, OCTREE *octree,
...
inline void compute_non_root_bounding_box(int node_id, OCTREE *octree,
                                          int child_id) {
  OCTREE_NODE *node = &(octree->nodes[node_id]);
  ...
  double lx = pnode->lx, ly = pnode->ly, lz = pnode->lz;
...
inline void compute_leaf_attributes(int node_id, OCTREE *octree, int *indices,
                                    int start_id, int end_id) {
#ifdef ADD_ATTR
  OCTREE_NODE *node = &(octree->nodes[node_id]);
  mol_atom *atoms = octree->atoms;
...
inline int get_child_id(OCTREE_NODE *node, mol_atom *atom) {
  ...
  int k = (zeroIfLess(atom->Z, cz) << 2) + (zeroIfLess(atom->Y, cy) << 1) +
          (zeroIfLess(atom->X, cx));

  return k;
}

int expand_octree_node(int node_id, OCTREE *octree, int *indices,
                       int *indices_temp, int start_id, int end_id) {
  OCTREE_NODE *node = &(octree->nodes[node_id]);
  mol_atom *atoms = octree->atoms;
  ...
  if ((n <= octree->max_leaf_size) || (dim <= octree->max_leaf_dim)) {
    ...
    //      node->indices = ( int * ) memalign( ALIGNMENT_BLOCK_SIZE, 2 * n *
    //      sizeof ( int ) );
    ...
    for (int i = 0; i < 8; i++) {
      if (count[i] > 0) {
        ...
        if (!expand_octree_node(j, octree, indices_temp, indices,
                                start_index[i], start_index[i] + count[i] - 1))
          return 0;
...
int contract_octree_node(int node_id, OCTREE *octree) {
  ...
  //   node->indices = ( int * ) memalign( ALIGNMENT_BLOCK_SIZE, sizeof ( int )
  //   * 2 * n );
...
int build_octree(OCTREE *octree, int max_leaf_size, double max_leaf_dim,
  ...
  //   indices = ( int * ) memalign( ALIGNMENT_BLOCK_SIZE, sizeof ( int ) *
  //   ag->natoms );
  indices_temp = (int *)_mol_malloc(sizeof(int) * ag->natoms);
  //   indices_temp = ( int * ) memalign( ALIGNMENT_BLOCK_SIZE, sizeof ( int ) *
  //   ag->natoms );
  ...
  int built = expand_octree_node(octree_root, octree, indices, indices_temp, 0,
                                 ag->natoms - 1);
...
void print_octree(OCTREE *octree) { traverse_octree(0, octree); }
...
int get_octree_size(OCTREE *octree) { return get_subtree_size(0, octree); }
...
inline int remove_atom_from_leaf(int node_id, OCTREE *octree, int atom_id) {
  ...
  if (n <= (node->id_cap >> 2)) {
    node->indices =
        (int *)_mol_realloc(node->indices, (node->id_cap >> 1) * sizeof(int));
...
inline int add_upward_migrating_atom_to_non_leaf(int node_id, OCTREE *octree,
                                                 int atom_id) {
  OCTREE_NODE *node = &(octree->nodes[node_id]);
  ...
  if (!m) {
    ...
    //       node->indices = ( int * ) memalign( ALIGNMENT_BLOCK_SIZE,
    //       INIT_MIGRATION_ARRAY_SIZE * sizeof( int ) );
...
inline int remove_upward_migrating_atom_from_non_leaf(int node_id,
                                                      OCTREE *octree,
                                                      int index) {
  OCTREE_NODE *node = &(octree->nodes[node_id]);
...
inline int add_downward_migrating_atom_to_non_leaf(int node_id, OCTREE *octree,
                                                   int atom_id) {
  OCTREE_NODE *node = &(octree->nodes[node_id]);
  mol_atom *atom = &(octree->atoms[atom_id]);
...
inline int remove_downward_migrating_atom_from_non_leaf(int node_id,
                                                        OCTREE *octree,
                                                        int index) {
  OCTREE_NODE *node = &(octree->nodes[node_id]);
...
inline int add_atom_to_leaf(int node_id, OCTREE *octree, int atom_id) {
  ...
  if (node->n > (octree->max_leaf_size << 1)) {
    ...
    //       int *temp = ( int * ) memalign( ALIGNMENT_BLOCK_SIZE, node->n *
    //       sizeof( int ) );
    ...
    int done = expand_octree_node(node_id, octree, node->indices, temp, 0,
                                  node->n - 1);
    ...
    for (int i = 0; i < 8; i++) {
      if (node->c_ptr[i] >= 0) {
        ...
        if (inside_node(cnode, atom))
          return push_down(node->c_ptr[i], octree, atom_id);
      } else {
        double lx = node->lx, ly = node->ly, lz = node->lz;
        ...
        if (!((atom->X >= lx) && (atom->X < lx + hdim) && (atom->Y >= ly) &&
              (atom->Y < ly + hdim) && (atom->Z >= lz) &&
              (atom->Z < lz + hdim)))
          continue;
  ...
  if (node->leaf) {
    if (node->p_ptr >= 0) {
      for (int i = node->nfixed; i < node->n; i++) {
        ...
        if (!inside_node(node, atom)) {
          if (!remove_atom_from_leaf(node_id, octree, atom_id))
            return 0;
          if (!add_upward_migrating_atom_to_non_leaf(node->p_ptr, octree,
                                                     atom_id))
            return 0;
          i--;
        }
    ...
    for (int i = 0; i < 8; i++)
      if (node->c_ptr[i] >= 0) {
        ...
        if (!batch_pull_up(node->c_ptr[i], octree, &emp))
          return 0;
...
int batch_push_down(int node_id, OCTREE *octree) {
  ...
  for (int i = node->id_num - 1; i >= 0; i--) {
    ...
    if (!remove_downward_migrating_atom_from_non_leaf(node_id, octree, i))
      return 0;
    ...
    if (cnode->leaf) {
      ...
      if (!add_downward_migrating_atom_to_non_leaf(node->c_ptr[k], octree,
                                                   atom_id))
        return 0;
    }
  }
...
double accumulate_excluding_far(OCTREE_PARAMS *octpar) {
  ...
  if (octpar->trans == NULL) {
    if (!within_distance_cutoff(
            static_node->lx, static_node->ly, static_node->lz, static_node->dim,
            moving_node->lx, moving_node->ly, moving_node->lz, moving_node->dim,
            octpar->approx_cutoff))
      return 0;
  } else {
    ...
    double mx = moving_node->lx + half_dim, my = moving_node->ly + half_dim,
...
double octree_accumulation_excluding_far(
    OCTREE *octree_static, OCTREE *octree_moving, double dist_cutoff,
    double approx_cutoff, int fixed_cull, double *trans, void *proc_func_params,
