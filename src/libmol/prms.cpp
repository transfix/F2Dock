  ...
  while (getline(&line, &len, fp) != -1) {
    ...
    if (strncmp(idstr, ATOM_KEY, MAXLEN) == 0) // cmp id
    {
      ...
      if (sscanf(line, "%*s %d %19s %19s %d %f %f",
                 ...
                 prm->atoms[atomsi].typemaj, prm->atoms[atomsi].typemin,
                 &prm->atoms[atomsi].subid, &prm->atoms[atomsi].r,
...
/**
        ...
        of prm->atoms with only a single entry for each unique <typemaj,
   typemin> tuple in the PDB. This is mainly for compatibility with other parts
...
void read_typeinfo_from_pdb(struct prm *prm, const char *path) {
  ...
  while (getline(&line, &len, fp) != -1) {
    ...
    if (sscanf(line, "%*s %*d %4s %4s", prm->atoms[atomsi].typemin,
  ...
  prm->atoms = (struct prmatom *)_mol_realloc(
      (void *)prm->atoms, sizeof(struct prmatom) * prm->natoms);
}
...
void read_prmpwpot(struct prm *prm, const char *path) {
  ...
  prm->pwpot->Xs =
      (float *)_mol_malloc(sizeof(float) * prm->pwpot->k * prm->nsubatoms);
  ...
  while (getline(&line, &len, fp) != -1) {
    ...
    if (strncmp(idstr, PWPOT_KEY, MAXLEN) == 0) // cmp id
    {
      ...
      if (sscanf(tmpline, "%*s %f %n", &prm->pwpot->lambdas[lambdasi], &n) <
          1) {
        fprintf(stderr, "error in file %s:\n", path);
        fprintf(stderr, "wrong format for %s line\n", PWPOT_KEY);
        exit(EXIT_FAILURE);
      ...
      for (j = 0; j < prm->nsubatoms; j++) {
        if (sscanf(tmpline, "%f %n",
                   &prm->pwpot->Xs[(Xsi * prm->nsubatoms) + j], &n) < 1) {
          fprintf(stderr, "error in file %s:\n", path);
...
void read_prmbond(struct prm *prm, const char *path) {
  ...
  while (getline(&line, &len, fp) != -1) {
    ...
    if (strncmp(idstr, BOND_KEY, MAXLEN) == 0) // cmp id
    {
      if (sscanf(line, "%*s %d %d %f %f", &prm->bonds[bondsi].i,
                 &prm->bonds[bondsi].j, &prm->bonds[bondsi].k,
...
int atomid(struct prm *prm, const char *typemaj, const char *typemin) {
  ...
  struct prmatom *atomres = (prmatom *)bsearch(
      &atomkey, prm->atoms, prm->natoms, sizeof(struct prmatom), comp_prmatom);
...
struct prm *copy_prm(struct prm *iprm) {
  ...
  oprm->Xs = (float*) _mol_malloc (sizeof (float) * oprm->pwpot->k *
  oprm->pwpot->k);
...
void copy_prmatom(struct prmatom *iatom, struct prmatom *oatom) {
  size_t satypemaj =
      20 * sizeof(char); // size of atypemaj for malloc and strncpy
  size_t satypemin =
      20 * sizeof(char); // size of atypemin for malloc and strncpy
...
void print_readerr(enum ereaderr readerr, const char *path, char *line) {
  ...
  case ERR_NAMELEN:
    fprintf(stderr, "length of atom type name is too long (it should be less "
                    "than 40 chars)\n");
    break;
...
void print_prmpwpot(struct prm *prm) {
  ...
  for (i = 0; i < prm->pwpot->k; i++) {
    for (j = 0; j < prm->nsubatoms; j++) {
      // printf ("%02d,%02d: %08.4f  ", i, j,
      // prm->pwpot->Xs[(i*prm->pwpot->k)+j]);
...
void destroy_prmpwpot(struct prmpwpot *pwpot) {
  free(pwpot->lambdas);
  free(pwpot->Xs);
}

void free_prmpwpot(struct prmpwpot *pwpot) {
