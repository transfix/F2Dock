...
void ace_ini(struct atomgrp *ag, struct acesetup *ac_s) {
  ...
  double width = 1.2;
  double ri, ri2, rk, vk, rk2, alpha, alpha2, alpha4, prod2, prod4, ratio, tik2,
      qik, qterm, temp, fik, omik, s2ik, s3ik, uik, omgik;
  ac_s->ntypes = 255;
      ...
      for (j = 0; j < ag->nbonds; j++) {
        ...
        if ((bp.a0->atom_ftypen == ag->atoms[i].atom_ftypen)) {
          ac_s->rsolv[ag->atoms[i].atom_ftypen] =
              max(ac_s->rsolv[ag->atoms[i].atom_ftypen], bp.a1->rminh - bp.l0);
        }
        if (ag->bonds[j].a1->atom_ftypen == ag->atoms[i].atom_ftypen) {
          ac_s->rsolv[ag->atoms[i].atom_ftypen] =
              max(ac_s->rsolv[ag->atoms[i].atom_ftypen], bp.a0->rminh - bp.l0);
        }
      }
    }
  }
  ...
  for (i = 0; i < ac_s->ntypes; i++) {
    ac_s->hydr[i] = -4 * L_PI * 0.003 * ac_s->rsolv[i] *
                    (ac_s->rsolv[i] + 1.4) * (ac_s->rsolv[i] + 1.4);
    ri = ac_s->rsolv[i];
    ri2 = ri * ri;
    if (ri > 0) {
      for (k = 0; k < ac_s->ntypes; k++) {
        ...
        if (rk > 0) {
          ...
          if (k == i) {
            omik =
                -((vk * pi2) - c1 * (ri2 * ri)) * qterm / (alpha4 * ri2 * ri2);
            ac_s->lwace[i] = omik;
          }
          if (vk > 1.0e-8) {
...
int *compute_0123_list(struct atomgrp *ag, int *n0123, int *list03, int n03,
                       int *list02, int n02, int *na01, int **pna01) {
  int i, j, n2, ind;
  int *p;
...
void ace_eselfupdate(int i1, int i2, int it, int kt, int ij, double dx,
                     double dy, double dz, struct acesetup *ac_s, double *eself,
                     double *swarr, double *dswarr, double *darr, double *xf,
                     double *yf, double *zf, double *xsf, double *ysf,
                     double *zsf, double rul3, double rul12, double nb2cot,
                     double nb2cof, int nbsize) {
  ...
  if (r2 < nb2cof) {
    ...
    r4 = r2 * r2;
    if (ac_s->vsolv[kt] > 0) {
      expterm = ac_s->wace[it * ac_s->ntypes + kt] *
      ...
      ffk = ((-8 * term * (3 * u4ace - r4) / (r2 * rmu)) +
             (4 * expterm / ac_s->s2ace[it * ac_s->ntypes + kt])) *
                sw -
            temp * dsw;
      eself[i1] -= temp;
      xsf[ij] = ffk * dx;
    ...
    if (ac_s->vsolv[it] > 0) {
      expterm = ac_s->wace[kt * ac_s->ntypes + it] *
      ...
      eself[i2] -= temp;
      ffk = ((-8 * term * (3 * u4ace - r4) / (r2 * rmu)) +
             (4 * expterm / ac_s->s2ace[kt * ac_s->ntypes + it])) *
                sw -
            temp * dsw;
      int ind = ij + nbsize;
      xsf[ind] = -ffk * dx;
...
void ace_fixedupdate(struct atomgrp *ag, struct agsetup *ags,
  ...
  if (ac_s->list0123 != NULL)
    free(ac_s->list0123);
  list0123 = compute_0123_list(ag, &n0123, ags->listf03, ags->nf03, ags->list02,
                               ags->n02, na01, pna01);
...
void aceeng(struct atomgrp *ag, double *en, struct acesetup *ac_s,
  ...
  for (i = 0; i < ags->nblst->nfat; i++) {
    ...
    for (j = 0; j < n2; j++) {
      ...
      ace_eselfupdate(i1, i2, it, kt, ij++, dx, dy, dz, ac_s, eself, swarr,
                      dswarr, darr, xf, yf, zf, xsf, ysf, zsf, rul3, rul12,
                      nb2cot, nb2cof, nbsize);
  ...
  for (i = 0; i < ags->nblst->nfat; i++) {
    ...
    if (ag->atoms[i1].chrg != 0) {
      // if (1){
      for (j = 0; j < n2; j++) {
        double s, s2, brij, expo, fexp, rij2, rij, cij, fac2, sw, dsw, fac3,
            fac4, dij, dji, facc2, fac5, fx, fy, fz;
        i2 = p[j];
  ...
  // 1-2-3-4 second loop
  for (i = 0; i < n0123; i++) {
    double s, s2, brij, expo, fexp, rij2, rij, cij, fac2, sw, dsw, fac3, fac4,
  ...
  //	 printf("Energy screen %.5f coul %.5f hyd %.5f self
  //%.5f\n",etotal-etotalself,ecoul,ehydr,etotalself); 	 printf("Etotal
  //%.8f\n",etotal+ecoul+ehydr);
  ...
  *en = ecoul + etotal + ehydr;
  ...
  for (i = 0; i < ag->natoms; i++) {
    ...
    // ag->atoms[i].GX,ag->atoms[i].GY,ag->atoms[i].GZ); printf("DIARR %d %.8f
