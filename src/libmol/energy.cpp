...
float pairwise_potential_energy(struct atomgrp *agA, struct atomgrp *agB,
  ...
  for (Aatomi = 0; Aatomi < agA->natoms; Aatomi++) {
    ...
    if (Atypen < 0 || Atypen > prm->natoms - 1) {
      fprintf(stderr, "begin error\n");
      fprintf(stderr,
              "atom type number of atom index %d is not defined in the "
              "argument atom prm\n",
              Aatomi);
      fprintf(stderr, "end error\n");
      exit(EXIT_FAILURE);
    ...
    if (subA > prm->nsubatoms - 1) {
      fprintf(stderr, "begin error\n");
      fprintf(stderr, "the argument atom prm subatom mapping of atom %d",
              Atypen);
    ...
    for (Batomi = 0; Batomi < agB->natoms; Batomi++) {
      ...
      float rsq =
          (powf((AX - BX), 2.0) + powf((AY - BY), 2.0) + powf((AZ - BZ), 2.0));

      if (rsq >= r1sq && rsq < r2sq) // atom distance is within the bin
      {
        for (int ek = 0; ek < prm->pwpot->k; ek++) {
          E += prm->pwpot->lambdas[ek] *
               prm->pwpot->Xs[(ek * prm->nsubatoms) + subA] *
               prm->pwpot->Xs[(ek * prm->nsubatoms) + subB];
        }
      }
    }
  }
  ...
  for (Aatomi = 0; Aatomi < agA->natoms; Aatomi++) {
    ...
    for (Batomi = 0; Batomi < agB->natoms; Batomi++) {
      ...
      if (Btypen < 0 || Btypen > prm->natoms - 1) {
        fprintf(stderr,
                "atom type number of atom index %d is not defined in the atom "
                "prm\n",
                Batomi);
        exit(EXIT_FAILURE);
      }
