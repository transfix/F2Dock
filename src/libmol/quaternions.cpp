...
void quaternion_to_rmatrix(
    struct quaternion *q,
    struct rmatrix *U) { // This function builds the rotation matrix from the
                         // given quartenion.
  double Norm = pow(q->q0, 2) + pow(q->q1, 2) + pow(q->q2, 2) + pow(q->q3, 2);

  U->a11 =
      (pow(q->q0, 2) + pow(q->q1, 2) - pow(q->q2, 2) - pow(q->q3, 2)) / Norm;
...
void rmatrix_to_quaternion(
    struct rmatrix *U,
    struct quaternion *q) { // This function builds the quaternion from a given
                            // rotation matrix.
  // Determine q0:
  ...
  q->q0 =
      cbrt(sqrt(pow(A12, 2) * pow(A13, 2) * pow(A23, 2) / S12 / S13 / S23 / 8));
...
void quaternion_product(struct quaternion *qa, struct quaternion *qb,
  ...
  product->q0 =
      qa->q0 * qb->q0 - qa->q1 * qb->q1 - qa->q2 * qb->q2 - qa->q3 * qb->q3;
  product->q1 =
      qa->q0 * qb->q1 + qa->q1 * qb->q0 + qa->q2 * qb->q3 - qa->q3 * qb->q2;
  product->q2 =
      qa->q0 * qb->q2 + qa->q2 * qb->q0 + qa->q3 * qb->q1 - qa->q1 * qb->q3;
  product->q3 =
      qa->q0 * qb->q3 + qa->q3 * qb->q0 + qa->q1 * qb->q2 - qa->q2 * qb->q1;
}
