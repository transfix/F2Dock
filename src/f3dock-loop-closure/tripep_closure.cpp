...
void tripep_closure::solve_3pep_poly(
    double r_n1[3], double r_a1[3], double r_a3[3], double r_c3[3], 
...
//!-----------------------------------------------------------------------
//subroutine initialize_loop_closure(b_len, b_ang, t_ang)
void tripep_closure::initialize_loop_closure(double b_len[6], double b_ang[7], double t_ang[2])
{
  //!-----------------------------------------------------------------------
  ...
  //  a_max = min(b_ang(4) + (xi(2) + eta(2)), kPi)
  a_max = min((b_ang[3] + (xi[1] + eta[1])), kPi);

  //  ! min/max of base length
  //!  print*, 'len1, len3=', len_aa(2:3)
  //  printf("len1, len3= %9.5f %9.5f\n", len_aa[1], len_aa[2]);
  //!  print*, 'a_min, a_max=', a_min*kRad2Deg, a_max*kRad2Deg
  //  printf("a_min, a_max= %9.5f %9.5f\n", a_min*kRad2Deg, a_max*kRad2Deg);
...
void tripep_closure::get_input_angles(int *n_soln, double r_n1[3], double r_a1[3], 
                                      double r_a3[3], double r_c3[3])
{
  ...
  *n_soln = max_soln;
  ...
  sin_alpha[1] = sin(alpha[1]);
  //  alpha(3) = kPi - alpha(1) + alpha(2)
  alpha[2] = kPi - alpha[0] + alpha[1];
  ...
  if (print_level > 0)
  {
    //     write(*,'(a,3f9.4)') 'xi = ', xi(1:3)*kRad2Deg
    printf("xi = %9.4f%9.4f%9.4f\n", xi[0]*kRad2Deg, xi[1]*kRad2Deg, xi[2]*kRad2Deg);
    //     write(*,'(a,3f9.4)') 'eta = ', eta(1:3)*kRad2Deg
    printf("eta = %9.4f%9.4f%9.4f\n", eta[0]*kRad2Deg, eta[1]*kRad2Deg, eta[2]*kRad2Deg);
    //     write(*,'(a,3f9.4)') 'delta = ', delta(1:3)*kRad2Deg
    printf("delta = %9.4f%9.4f%9.4f\n", delta[1]*kRad2Deg, delta[2]*kRad2Deg, delta[3]*kRad2Deg);
    //     write(*,'(a,3f9.4)') 'theta = ', theta(1:3)*kRad2Deg
    printf("theta = %9.4f%9.4f%9.4f\n", theta[0]*kRad2Deg, theta[1]*kRad2Deg, theta[2]*kRad2Deg);
    //     write(*,'(a,3f9.4)') 'alpha = ', alpha(1:3)*kRad2Deg
    printf("alpha = %9.4f%9.4f%9.4f\n", alpha[0]*kRad2Deg, alpha[1]*kRad2Deg, alpha[2]*kRad2Deg);
    //  end if
  }
...
void tripep_closure::test_two_cone_existence_soln(
    double tt, double kx, double et, double ap, int *n_soln, char cone_type[2])
{
  //  implicit none
  ...
  int s1, s2, t1, t2;
  int complicated = 0;
  //  real(dp), parameter :: half_pi = 0.5d0*kPi
...
void tripep_closure::get_poly_coeff(double poly_coeff[max_soln+1])
{
  //  implicit none
...
void tripep_closure::poly_mul_sub2(
    double u1[5][5], double u2[5][5], double u3[5][5], double u4[5][5],
    int p1[2], int p2[2], int p3[2], int p4[2], double u5[5][5], int p5[2])
{
  //  implicit none
...
void tripep_closure::poly_mul2(double u1[5][5], double u2[5][5], 
                               int p1[2], int p2[2], 
...
void tripep_closure::poly_sub2(double u1[5][5], double u2[5][5], 
                               int p1[2], int p2[2], 
                               double u3[5][5], int p3[2])
{
  //  implicit none
...
void tripep_closure::poly_mul_sub1(
    double u1[17], double u2[17], double u3[17], double u4[17], 
...
void tripep_closure::poly_mul1(
    double u1[17], double u2[17], int p1, int p2, double u3[17], int *p3)
{
  //  implicit none
...
void tripep_closure::coord_from_poly_roots(
    int *n_soln, double roots[max_soln], 
    double r_n1[3], double r_a1[3], double r_a3[3], double r_c3[3], 
    double r_soln_n[max_soln][3][3], double r_soln_a[max_soln][3][3], 
    double r_soln_c[max_soln][3][3])
{
  ...
  for(i_soln=0;i_soln<*n_soln;i_soln++)
  {
    ...
    if (print_level > 0)
    {
      ...
      //        write(*,"('alpha1, alpha3 = ', 2f9.3)") (kPi-a3a1a2)*kRad2Deg, (kPi-a2a3a1)*kRad2Deg
      printf("alpha1, alpha3 = %9.3f%9.3f\n", (kPi-a3a1a2)*kRad2Deg, (kPi-a2a3a1)*kRad2Deg);
      ...
      //        write(*,"('ang_nac = ', 2f9.3)") b_ang0(1)*kRad2Deg, n1a1c1*kRad2Deg
      printf("ang_nac = %9.3f%9.3f\n", b_ang0[0]*kRad2Deg, n1a1c1*kRad2Deg);
      //        write(*,"('ang_nac = ', 2f9.3)") b_ang0(4)*kRad2Deg, n2a2c2*kRad2Deg 
      printf("ang_nac = %9.3f%9.3f\n", b_ang0[3]*kRad2Deg, n2a2c2*kRad2Deg);
      //        write(*,"('ang_nac = ', 2f9.3)") b_ang0(7)*kRad2Deg, n3a3c3*kRad2Deg
      printf("ang_nac = %9.3f%9.3f\n", b_ang0[6]*kRad2Deg, n3a3c3*kRad2Deg);
      ...
      //        write(*,"('t_ang1 = ', 2f9.3)") t_ang0(1)*kRad2Deg, a1c1n2a2*kRad2Deg
      printf("t_ang1 = %9.3f%9.3f\n", t_ang0[0]*kRad2Deg, a1c1n2a2*kRad2Deg);
      //        write(*,"('t_ang2 = ', 2f9.3)") t_ang0(2)*kRad2Deg, a2c2n3a3*kRad2Deg
      printf("t_ang2 = %9.3f%9.3f\n", t_ang0[1]*kRad2Deg, a2c2n3a3*kRad2Deg);
      //     end if
    }
...
//!-----------------------------------------------------------------------
//function calc_t2(t0)
double tripep_closure::calc_t2(double t0)
{
  //  implicit none
...
double tripep_closure::calc_t1(double t0, double t2)
{
  //  implicit none
...
void tripep_closure::calc_dih_ang(double r1[3], double r2[3], double r3[3], double *angle)
{
  //!-----------------------------------------------------------------------
...
void tripep_closure::calc_bnd_ang(double r1[3], double r2[3], double *angle)
{
  //!-----------------------------------------------------------------------
...
void tripep_closure::cross(double p[3], double q[3], double s[3])
{
  //!-----------------------------------------------------------------------
  //  implicit none
...
void tripep_closure::quaternion(double axis[3], double quarter_ang, double p[4])
{
  //!-----------------------------------------------------------------------
...
void tripep_closure::rotation_matrix(double q[4], double U[3][3])
{
  //!-----------------------------------------------------------------------
