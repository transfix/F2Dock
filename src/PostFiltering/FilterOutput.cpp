...
void readPQR(const char *filename, int &numMovingAtoms, double *&movingAtoms) {
  ...
  for (int i = 0, j = 0; i < lines.size(); i++) {
    if (sscanf(lines.at(i) + 30, "%lf %lf %lf %lf %lf", &x, &y, &z, &q, &r) ==
        5) {
...
void readRaw(const char *forbiddenVolFileName, int &numStaticAtoms,
             double *&staticAtoms) {

  std::cout << "Forbidden volume bounded by surface " << forbiddenVolFileName
            << std::endl;
  ...
  if ((staticAtoms == NULL)) {
    ...
    std::cerr << "Cannot allocate memory. Forbidden volume filter will not be "
                 "applied.\n";
    assert(false);
  }
...
void readTransformation(const char *filename,
  ...
  std::string dummy;
  ifile >> dummy;
  while (ifile >> T(0, 0) >> T(0, 1) >> T(0, 2) >> T(0, 3) >> T(1, 0) >>
         T(1, 1) >> T(1, 2) >> T(1, 3) >> T(2, 0) >> T(2, 1) >> T(2, 2) >>
         T(2, 3) >> T(3, 0) >> T(3, 1) >> T(3, 2) >> T(3, 3))
    transformations.push_back(T);
  ifile.close();
}

bool initForbiddenVolumeFilter(const char *forbiddenVolFileName,
                               const char *movingAtomsFileName,
                               clashFilter **cFilter) {
...
void applyFilter(clashFilter *fFilter,
                 const std::vector<Matrix4x4> &transformations,
                 const char *transformationFileName) {
  ...
  std::cout << "Number of accepted transformations in the top 2000 = "
            << count_2000 << std::endl;
  std::cout << "Total number of transformations = " << transformations.size()
            << std::endl;
}
...
int main(int argc, char **argv) {
  ...
  if (argc < 5) {
    ...
    std::cout << "Usage:./FilterOutput <Ligand PQR> <Forbidden Volume RAW> "
                 "<XForms File> <OutputFile>.\n";
    return -1;
  }
