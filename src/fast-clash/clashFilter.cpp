...
void clashFilter::printError(const std::string &msg) {
  std::cerr << "\nError: " << msg << "\n\n";
}
...
void clashFilter::printCurrentSettings(void) {
  ...
  printf(
      (char *)"\tclasfFrac = %lf, severeClashFrac = %lf, fuzzyFrac = %lf\n\n",
      clashFrac, severeClashFrac, fuzzyFrac);
}

clashFilter::clashFilter(int numStaticAtoms, double *stAtoms,
                         int numMovingAtoms, double *mvAtoms, bool printStat) {
  setDefaults();

  printStatus = printStat;

  if (!copyAtomsFromArray(numStaticAtoms, stAtoms, &nStaticAtoms,
                          &staticAtoms) ||
      !copyAtomsFromArray(numMovingAtoms, mvAtoms, &nMovingAtoms,
                          &movingAtoms)) {
    freeMemory();
    exit(1);
  }
...
clashFilter::clashFilter(int numStaticAtoms, double *stAtoms,
                         int numMovingAtoms, double *mvAtoms)
...
bool clashFilter::setProximityFactors(double clashFactor,
                                      double severeClashFactor,
                                      double fuzzyFactor) {
  if (clashFactor < 0) {
...
bool clashFilter::copyAtomsFromArray(int numAtomsSrc, double *atmsSrc,
                                     int *numAtomsDest, ATOM **atmsDest) {
  if (printStatus)
...
void clashFilter::countAtomsOctreeNodesAndSortAtoms(ATOM *atoms,
                                                    int atomsStartID,
                                                    int atomsEndID,
                                                    ATOM *atomsT,
                                                    int *numNodes) {
  ...
  double maxX = atoms[atomsStartID].x, maxY = atoms[atomsStartID].y,
         maxZ = atoms[atomsStartID].z, maxR = atoms[atomsStartID].r;
...
int clashFilter::constructAtomsOctree(int atomsStartID, int atomsEndID,
                                      ATOM *atoms,
...
void clashFilter::fillMovingAtomsSubtreeRootsArray(int nodeID,
                                                   int maxNodesInLevel) {
  if (nodeID < 0)
    return;
...
bool clashFilter::buildStaticAtomsOctree(void) {
  ...
  countAtomsOctreeNodesAndSortAtoms(staticAtoms, 0, nStaticAtoms - 1, atomsT,
                                    &numStaticAtomsOctreeNodes);
  freeMem(atomsT);
  ...
  staticAtomsOctreeRoot =
      constructAtomsOctree(0, nStaticAtoms - 1, staticAtoms, staticAtomsOctree);
...
bool clashFilter::buildMovingAtomsOctree(void) {
  ...
  countAtomsOctreeNodesAndSortAtoms(movingAtoms, 0, nMovingAtoms - 1, atomsT,
  ...
  movingAtomsOctreeRoot =
      constructAtomsOctree(0, nMovingAtoms - 1, movingAtoms, movingAtomsOctree);
...
void clashFilter::approximateInteractions(Matrix transMat, int nodeS, int nodeM,
    ...
    if (staticAtomsOctree[nodeS].leaf && movingAtomsOctree[nodeM].leaf) {
      for (int i = movingAtomsOctree[nodeM].atomsStartID;
        ...
        for (int j = staticAtomsOctree[nodeS].atomsStartID;
          ...
          if (d2 <= (clashFrac * rSum) * (clashFrac * rSum))
            clash = true; //( *nClashes )++;
          if (d2 <= (severeClashFrac * rSum) * (severeClashFrac * rSum))
            severeClash = true; //( *nSevereClashes )++;

          if (d2 < (fuzzyFrac * rSum) * (fuzzyFrac * rSum))
            d2 = (fuzzyFrac * rSum) * (fuzzyFrac * rSum);
          ...
          for (int j = 0; j < 8; j++)
            if (movingAtomsOctree[nodeM].cPtr[j] >= 0) {
              ...
              approximateInteractions(
                  transMat, staticAtomsOctree[nodeS].cPtr[i],
                  movingAtomsOctree[nodeM].cPtr[j], &nC, &nSC, &intVal);
      ...
      for (int j = 0; j < 8; j++)
        if (movingAtomsOctree[nodeM].cPtr[j] >= 0) {
          ...
          approximateInteractions(transMat, nodeS,
                                  movingAtomsOctree[nodeM].cPtr[j], &nC, &nSC,
                                  &intVal);
...
bool clashFilter::computeInteractions(Matrix transMat, int *nClashes,
                                      int *nSevereClashes,
                                      double *interactionValue) {
  ...
  if (numThreads == 1)
    approximateInteractions(transMat, staticAtomsOctreeRoot,
                            movingAtomsOctreeRoot, nClashes, nSevereClashes,
                            interactionValue);
  else {
    ...
    for (int i = 0; i < numThreads; i++) {
      ...
      pthread_create(&p[i], NULL, approximateInteractionsThread,
                     (void *)&threadResults[i]);
    }
...
bool clashFilter::computeInteractions(int *nClashes, int *nSevereClashes,
                                      double *interactionValue) {
  return computeInteractions(transMatrix, nClashes, nSevereClashes,
                             interactionValue);
}

bool clashFilter::computeInteractionsNaively(Matrix transMat, int *nClashes,
                                             int *nSevereClashes,
                                             double *interactionValue) {
  if (printStatus)
  ...
  *interactionValue = 0.0;
  ...
  //     printf( "\n< %lf, %lf, %lf, %lf, %lf >\n", staticAtoms[ i ].x,
  //     staticAtoms[ i ].y, staticAtoms[ i ].z, staticAtoms[ i ].q,
  //     staticAtoms[ i ].r );
  //
  //   printf( "\nnMovingAtoms = %d\n", nMovingAtoms );
  //
  ...
  //     printf( "\n< %lf, %lf, %lf, %lf, %lf >\n", movingAtoms[ i ].x,
  //     movingAtoms[ i ].y, movingAtoms[ i ].z, movingAtoms[ i ].q,
  //     movingAtoms[ i ].r );
  //
  //   printf( "\n\n" );
  ...
  for (int i = 0; i < nMovingAtoms; i++) {
    ...
    for (int j = 0; j < nStaticAtoms; j++) {
      ...
      //         if ( d2 < ( fuzzyFrac * rSum ) * ( fuzzyFrac * rSum ) ) d2 = (
      //         fuzzyFrac * rSum ) * ( fuzzyFrac * rSum );

      double minD2 = (frac * rSum) * (frac * rSum);

      if (d2 < (1 / (frac * frac)) * minD2)
        k++;
      ...
      (*interactionValue) += (qiqj / d2);
      //         ( *interactionValue ) += ( minD2 / d2 );// ( ( minD2 * minD2 *
      //         minD2 ) / ( d2 * d2 * d2 ) );
    }
...
bool clashFilter::computeInteractionsNaively(int *nClashes, int *nSevereClashes,
                                             double *interactionValue) {
  return computeInteractionsNaively(transMatrix, nClashes, nSevereClashes,
                                    interactionValue);
}
