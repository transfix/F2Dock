...
KDTree::KDTree(KDTreeArray &data_in, bool rearrange_in, int dim_in)
    : the_data(data_in), N(data_in.shape()[0]), dim(data_in.shape()[1]),
      sort_results(false), rearrange(rearrange_in), root(NULL), data(NULL),
      ind(N) {
  //
...
void KDTree::n_nearest_brute_force(std::vector<float> &qv, int nn,
...
void KDTree::n_nearest(std::vector<float> &qv, int nn,
                       KDTreeResultVector &result) {
...
void KDTree::r_nearest(std::vector<float> &qv, float r2,
                       KDTreeResultVector &result) {
