...
Icp::Icp(double *M, const int32_t M_num, const int32_t dim)
  ...
  if (dim != 2 && dim != 3) {
    cout << "ERROR: LIBICP works only for data of dimensionality 2 or 3"
         << endl;
    M_tree = 0;
    return;
  }
...
void Icp::fit(double *T, const int32_t T_num, Matrix &R, Matrix &t,
              const double indist) {
...
void Icp::fitIterate(double *T, const int32_t T_num, Matrix &R, Matrix &t,
