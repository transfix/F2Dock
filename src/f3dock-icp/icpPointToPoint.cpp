...
// Also see (3d part): "Least-Squares Fitting of Two 3-D Point Sets" (Arun,
// Huang and Blostein)
double IcpPointToPoint::fitStep(double *T, const int32_t T_num, Matrix &R,
...
std::vector<int32_t> IcpPointToPoint::getInliers(double *T, const int32_t T_num,
                                                 const Matrix &R,
                                                 const Matrix &t,
                                                 const double indist) {
  ...
  if (dim == 2) {
    ...
    for (int32_t i = 0; i < T_num; i++) {
      ...
      // transform point according to R|t
      query[0] = (float)(r00 * T[i * 3 + 0] + r01 * T[i * 3 + 1] +
                         r02 * T[i * 3 + 2] + t0);
      query[1] = (float)(r10 * T[i * 3 + 0] + r11 * T[i * 3 + 1] +
                         r12 * T[i * 3 + 2] + t1);
      query[2] = (float)(r20 * T[i * 3 + 0] + r21 * T[i * 3 + 1] +
                         r22 * T[i * 3 + 2] + t2);
